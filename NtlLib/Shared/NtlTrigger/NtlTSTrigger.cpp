...
void CNtlTSTrigger::AttachGroup( CNtlTSGroup* pGroup )
{
	if ( 0 == pGroup )
	{
		CNtlTSLog::Log( "Can not set the group. ParamInfo[%s]. [%s]", "NULL", TS_CODE_TRACE() );
		return;
	}

	if ( GetGroup( pGroup->GetID() ) )
	{
		CNtlTSLog::Log( "Is exists already. ParamInfo[%d]. [%s]", pGroup->GetID(), TS_CODE_TRACE() );
		return;
	}
...
void CNtlTSTrigger::AttachChildForScript( CNtlTSScrObject* pChild )
{
	if ( 0 == pChild )
	{
		CNtlTSLog::Log( "Can not attach the script object. ParamInfo[%s]. [%s]", "NULL", TS_CODE_TRACE() );
		return;
	}
	...
	{
		CNtlTSLog::Log( "Can not do type cast from CNtlTSScrObject to CNtlTSGroup. ParamInfo[%s]. [%s]", pChild->GetClassName(), TS_CODE_TRACE() );
	}
}
