...
enum eOPCODE_GQ
{
	...
	GQ_SCOUTER_CHIP_REMOVE_ALL_REQ,			// 스카우터 칩 모두 제거
...
END_PROTOCOL()
//------------------------------------------------------------------
BEGIN_PROTOCOL(GQ_PARTY_ACTIVATE_CHARM_REQ)
	PARTYID					partyId;
	TBLIDX					charmItemTblidx;		// Table Index in ItemTable
	sVICTIM_ITEM_DATA		charmItemData;
	BYTE					byVictimInfoCount;
	bool					bIsSuccess;				// Dice Success 인가?
	sDBO_VICTIM_DATA		aVictimInfo[NTL_MAX_MEMBER_IN_PARTY];
END_PROTOCOL()
//------------------------------------------------------------------
...
BEGIN_PROTOCOL( GQ_QUEST_ITEM_DELETE_REQ )
	...
	DWORD						dwTimeStamp;
	CHARACTERID					charId;
	BYTE						byRequestType;	// sub data : Delete Type [eQUEST_ITEM_REQUEST_TYPE]
	uQUEST_ITEM_UPDATE_SUB_DATA	uSubData;		// sub data;
	BYTE						byItemCount;
	sQUEST_ITEM_UPDATE_DATA		aItems[1];
END_PROTOCOL()
...
BEGIN_PROTOCOL( GQ_QUEST_COMPLETE_DATA_UPDATE_REQ )
	...
	//QUESTID						questID; // 퀘스트 아이디
	//BYTE						byCompleteStatus; // 퀘스트 완료정보
END_PROTOCOL()
//------------------------------------------------------------------
...
BEGIN_PROTOCOL( GQ_CHAR_PUNISH_REQ )
	...
	WCHAR						awchGMCharName[NTL_MAX_SIZE_USERID_UNICODE + 1];			// GM Char Name
...
BEGIN_PROTOCOL( GQ_RANKBATTLE_SCORE_UPDATE_REQ )
	...
	BYTE						byStraightWin;		// eRANKBATTLE_QUERY_UPDATE_TYPE : 연승 카운트
	BYTE						byStraightKOWin;	// eRANKBATTLE_QUERY_UPDATE_TYPE : KO 연승 카운트
	BYTE						byWin;				// eRANKBATTLE_QUERY_UPDATE_TYPE : 승리 카운트
	BYTE						byDraw;				// eRANKBATTLE_QUERY_UPDATE_TYPE : 무승부 카운트
	BYTE						byLose;				// eRANKBATTLE_QUERY_UPDATE_TYPE : 패배 카운트
	float						fPoint;				// Point
...
BEGIN_PROTOCOL( GQ_MAIL_EVENT_SEND_REQ )
	...
	DWORD						dwZenny;		// Req or Give Zenny
	BYTE						byDay;		// 유효만료날짜 최대 10일
	WCHAR 						wszSenderName[NTL_MAX_SIZE_CHAR_NAME_UNICODE + 1];	// 캐릭 이름
	BYTE						byTextSize;
	WCHAR						wszText[NTL_MAX_LENGTH_OF_MAIL_MESSAGE_IN_UNICODE + 1];	// 메일내용
END_PROTOCOL()
//------------------------------------------------------------------
...
BEGIN_PROTOCOL(GQ_BUDOKAI_TOURNAMENT_INDIVIDUAL_ADD_ENTRY_LIST_REQ)
	...
	// pc의 진행 상태를 업데이트 하기 위한 정보
	BYTE					byWinnerJoinResult;	// = BUDOKAI_JOIN_RESULT_ENTER32
	BYTE					byLoserResultCondition;	// JoinResult 값이 byLoserResultCondition인 모든 레코드의 JoinState를 byLoserJoinState로 바꾼는다.
	BYTE					byLoserJoinState;	// = BUDOKAI_JOIN_STATE_DROPOUT
END_PROTOCOL()
//------------------------------------------------------------------
...
END_PROTOCOL()
//------------------------------------------------------------------
BEGIN_PROTOCOL(GQ_SCOUTER_CHIP_REMOVE_ALL_REQ)
	HOBJECT					hPc;
	CHARACTERID				charId;
	BYTE					byChipCount;
...
BEGIN_PROTOCOL(GQ_HOIPOIMIX_ITEM_MAKE_REQ)			// 아이템 만들기
	...
	TBLIDX					recipeTblidx;	
	bool					bIsNormal;
	DWORD					dwMixExp;				// 변경될 Exp
	BYTE					byMixLevel;				// 변경될 Level
	bool					bIsNew;					// 아이템이 생성이면 true 중첩이면 false 
