...
VOID CStaticBox_Generic::AddText( const WCHAR* text )
{
	...
	if( m_pBuffer )
	{
		// peessi : unibuffer관련 CPToXSize가 수정되면 원래대로 돌려놓자.
		//INT nTextSize = m_pBuffer->GetTextSize();
...
VOID CStaticBox_Generic::StringDetect(VOID)
{
	...
	for(i = 0; i < nTextSize; i++)
	{
		...
		{
			// Height Check 2
			// 다음 블럭의 까지의 높이 체크 후 높이가 안된다면 현재 스트링을 수정해서 입력후 리턴.
			...
			// . Check
			// 계행되는 후의 첫 문자가 .일경우 . 다음 부터 계행하도록 바꾼다.
			if( !bLineFeed )
			{
