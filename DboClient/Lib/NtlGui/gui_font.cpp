...
{
	// 참조 카운트가 0 일때만 폰트를 생성하고 참조 카운트를 증가시킨다.
	// 참조 카운트가 1 이상일때는 참조 카운트만 증가시킨다.
	if(m_nRefCount)
	{
		AddRef();
		return true;
	...
	else
	{
		...
		lf.lfEscapement     = 0 ;
		lf.lfOrientation    = 0 ;
		lf.lfWeight         = nAttribute & EZ_ATTR_BOLD      ? 700 : 0 ;		///< BOLD가 선언되어 있다면 700 아니면 0
		lf.lfItalic         = nAttribute & EZ_ATTR_ITALIC    ?   1 : 0 ;		///< ITRALIC이 선언되어 있다면 1 아니면 0
		lf.lfUnderline      = nAttribute & EZ_ATTR_UNDERLINE ?   1 : 0 ;		///< UNDERLINE이 선언되어 있다면 1 아니면 0
		lf.lfStrikeOut      = nAttribute & EZ_ATTR_STRIKEOUT ?   1 : 0 ;		///< STRIKEOUT이 선언되어 있다면 1 아니면 0
		lf.lfCharSet        = DEFAULT_CHARSET ;									///< 기본 문자 세트
		lf.lfOutPrecision   = 0 ;
		lf.lfClipPrecision  = 0 ;
		lf.lfQuality        = ANTIALIASED_QUALITY ;								///< 기본 4
		lf.lfPitchAndFamily = DEFAULT_PITCH;//VARIABLE_PITCH  ;					///< 기본 0
