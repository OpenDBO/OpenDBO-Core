...
std::wstring CListBox_Generic::GetItemText( INT nIndex )
{
	...
	if( nIndex >= 0 && nIndex < GetItemCount() )
		retval = m_vecItems[nIndex]->m_pBuffer->GetBuffer();
...
VOID CListBox_Generic::ChangeItem( const WCHAR* szText, INT nIndex, DWORD dwData )
{
	...
	m_vecItems[nIndex]->m_pBuffer->SetText( szText );
...
// 출력될 아이템 선정(아이템 추가,삭제 등), 선택, Ellipsis추가
VOID CListBox_Generic::ReviseItem()
{
	...
	for( i = 0 ; i < m_nVisibleRowCount ; ++i, ++it )
	{
		...
		INT nTextLength = pItem->m_pBuffer->GetTextSize();
		INT	nPos, j;

		for( j = 1 ; j <= nTextLength ; j++ )
		{
			pItem->m_pBuffer->CPtoX( j, FALSE, &nPos );
		...
		if( bEllipsis )
		{
			...
			else
			{
				...
				{
					--nPrevChar;
					pItem->m_pBuffer->CPtoX( nPrevChar, FALSE, &nPrevCharPosX );
			...
			else
			{
				...
				memcpy( InsertBuffer, pItem->m_pBuffer->GetBuffer(), sizeof( WCHAR ) * nTextLength );
		...
		{
			nTextLength = j;
			memset( InsertBuffer, 0, sizeof( WCHAR ) * nTextLength );
			memcpy( InsertBuffer, pItem->m_pBuffer->GetBuffer(), sizeof( WCHAR ) * ( j - 1 ) );
		}			

		m_listDrawItems.push_back( NTL_NEW CListBox_Item( InsertBuffer, m_pFont, pItem->m_dwData, pItem->m_bSelected ) );
	}
}
...
VOID CListBox_Generic::DrawItem(VOID)
{
	...
	for( idx = 0, it = m_listDrawItems.begin() ; it != m_listDrawItems.end() ; ++it, ++idx )
	{
		...
		INT nPosX = GetStartX( pItem->m_pBuffer );
		...
		if( nPosY >= 0 )
		{
			...
			if( pItem->m_bSelected )
			{
				...
				rect = m_pFont->TextOutW( pItem->m_pBuffer->GetBuffer() );
			}
			...
			g_FontMgr.BitmapToTexture( m_TextSurface.m_pTexture, rtBound, m_TextColor, m_nEffectMode, m_EffectColor, m_nEffectValue, m_nBkMode, m_BkColor );
		}
	}
}
...
VOID CListBox_Generic::OnMouseMove( INT nkey, INT xPos, INT yPos )
{
	...
	INT nTextLength = pItem->m_pBuffer->GetTextSize();
	INT	nTextWidth;
	pItem->m_pBuffer->CPtoX( nTextLength, FALSE, &nTextWidth );
	if( nTextWidth > m_nRowWidth )
	{
		m_pListBox->SetToolTip( std::wstring( pItem->m_pBuffer->GetBuffer() ) );
