...
public:
	CComponentType_Flash() : CComponentType_Component()
	{
		...
		m_stlOption["initplay"].type = CComponentType::SOptionType::BOOL;		// Load후 바로 Play. Default = FLASE
		m_stlOption["autorewind"].type = CComponentType::SOptionType::BOOL;		// 무비가 끝난후 Replay. Default = FALSE
		m_stlOption["framecallback"].type = CComponentType::SOptionType::BOOL;	// 프래임이 끝난순간 콜백을 날려준다. Default = FALSE
