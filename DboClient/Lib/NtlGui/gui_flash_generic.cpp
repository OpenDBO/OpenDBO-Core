...
BOOL CFlash_Generic::Load( CONST CHAR* pFileName, BOOL bOriginalSize )
{
	...
	m_pFlashObject->GetCommandHandler()->LinkFSCallBack( this, &CFlash_Generic::OnFSCallBack );
	
	BOOL bRes =	m_pFlashObject->Create( strFilename.c_str() );				

	if( !bRes )
	{
		m_pFlashObject->GetCommandHandler()->UnLinkFSCallBack();
	}
