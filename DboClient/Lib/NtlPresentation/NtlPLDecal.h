...
struct SPLDecalCreateParam : public SPLEntityCreateParam
{
	...
	RwV3d  vSize;								///< Decal의 Size (X,Z값만 사용한다)
	RwReal fScale;
	RwReal fVisibleSquaredDist;					///< Decal이 보이는 시야 거리의 제곱값
	RwReal fYOffset;							///< Y Offset 값 (지면으로부터의 Decal 높이)
	RwTextureAddressMode rwWrapType;			///< Texture Adress Mode
	RwInt32	nFlagEffect;						///< 이펙트 플래그
