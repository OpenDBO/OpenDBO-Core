...
RwBool CNtlPLXMLScriptHelper::SaveTMQEvent(IXMLDOMElement* pElemEvent, const SEventTMQ* pEventTMQ)
{
    if(!pElemEvent || !pEventTMQ)
        return FALSE;

    SaveAttribute(pElemEvent, L"TYPE", pEventTMQ->eTMQEventType);

    return TRUE;
}

RwBool CNtlPLXMLScriptHelper::LoadTMQEvent(IXMLDOMNode* pNodeEvent, SEventTMQ* pEventTMQ)
{
    ...
    pEventTMQ->eTMQEventType = (ETMQEventType)atoi(szBuf);
