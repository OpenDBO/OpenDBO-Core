...
// 	if(strlen(pBoneName) >= MAX_ATTR_BONE_NAME)
// 	{		
// 		NtlLogFilePrintArgs("%s(%s) - AttachBone Name length MAX_ATTR_BONE_NAME over", pPLAttachEntity->GetName(), pBoneName);
// 		NTL_ASSERTE("AttachBone Name length MAX_ATTR_BONE_NAME over");
// 	}
...
void CNtlPLAttach::SetAttachParent(CNtlPLAttach *pPLAttachEntity)
{
    ...
    // 부모가 Hide 상태면 자식도 Hide
    if(pPLAttachEntity)
    {
        SetVisible(pPLAttachEntity->IsVisible());
    }
}
