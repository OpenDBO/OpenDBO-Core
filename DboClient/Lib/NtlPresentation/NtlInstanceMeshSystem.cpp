									   ...
									   const RwMatrix& matWorld)
{
	...
	if(CNtlEffectSystemFreeList::m_ClumpFileManager.empty())		// Client에서는 map에 파일 정보를 넣어두지 않는다(로딩 속도를 빠르게 하기 위해)
	{
		strPathName = PATH_EFFECT_RESOURCE;
	...
	else	// Venus는 폴더를 자유로이 설정할수 있기때문에 map에 파일 패스 정보를 넣어둔다.
	{
	...
	// Uv 애니메이션이 있을때는 Clump 를 Clone 로 생성하면 같은 Clump 를 쓰는 Mesh System 은 
	// Uv 애니메이션을 세팅하지 않아도 uv 가 적용된다. 현재 CNtlPLResourceManager 에서 clone 으로 생성하지 못하도록 하는
	// 옵션이 없기 때문에 여기서 임시로 하드 코딩한다.
	if (m_pUvAnimDict == NULL)
	{
		m_pClumpResource = CNtlPLResourceManager::GetInstance()->LoadClump(strPathName.c_str(), strTexturePath.c_str());
		if (m_pClumpResource == NULL) return FALSE;
	}
	else
	{
	...
	// Animaiton Data가 있는 경우 Clump에 Animation를 적용을 한다.
    ...
    // 툰 적용
    //CreateToonData(m_pClumpResource->GetClump(), m_pToonData);
