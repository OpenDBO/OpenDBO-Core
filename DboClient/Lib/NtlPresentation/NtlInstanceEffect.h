...
public:
    ...
    void			 RenderBoundingSphere();                                        ///< 바운딩 스피어의 모양을 렌더링한다. (Venus 전용)
    ...
    //-- Low Spec용 플래그 설정들
    static void     SetLowSpecEnable(RwBool bEnable) {m_bLowSpec = bEnable;}        ///< 로우 스펙 적용 유무를 설정한다.
    static RwBool   GetLowSpecEnable() {return m_bLowSpec;}
    static void     SetLowSpecRatio(RwReal fRatio) {m_fLowSpecRatio = fRatio;}      ///< 로우 스펙 비율을 적용한다.    
    static RwReal   GetLowSpecRatio() {return m_fLowSpecRatio;}
    static void     SetRenderMeshSystem(RwBool bEnable) {m_bRenderMeshSystem = bEnable;}    ///< 메시 시스템 렌더링 On/Off
    static RwBool   GetRenderMeshSystem() {return m_bRenderMeshSystem;}
    static void     SetRenderDecalSystem(RwBool bEnable) {m_bRenderDecalSystem = bEnable;}  ///< 데칼 시스템 렌더링 On/Off
...
protected:
    ...
    void    CalcBoundingSphere();                                                   ///< 바운딩 스피어를 계산한다.    
