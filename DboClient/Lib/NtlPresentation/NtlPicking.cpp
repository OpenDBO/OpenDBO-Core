...
bool CNtlPicking::Pick( RwCamera* pCamera, RpWorld* pWorld, RwV2d& v2ScrPos, RwReal fPickDist, SWorldPickInfo& sPickInfo )
{
	...
	// 기본 카메라는 반드시 파라미터로 넘어와야 한다
	if ( NULL == pCamera ) return false;

	// World는 반드시 존재해야 한다
	if ( NULL == pWorld ) return false;

	// 2D 상에서의 Picking한 위치가 클라이언트의 영역을 넘어서면 리턴한다
	...
	RwV3d PickDir;
	RwV3dSub( &PickDir, &PickRay.end, &PickRay.start );
	...
	//////////////////////////////////////////////////////////////////////////
	//
	// Bounding box check routine << Ray 와 충돌에 걸리는 캐릭터를 검색한다 >>
	//
	//////////////////////////////////////////////////////////////////////////

	// 이전에 피킹된 Atomic 리스트를 초기화 한다
	m_bPickCharacter = false;
	m_pPickEntityList->Clear( m_pPickEntityPool );
	...
	// Bound picking 된 entity가 존재하지 않는다면 picking된 객체가 없는 것임
	if ( m_pPickEntityList->IsEmpty() ) return false;

	// Picking object를 거리에 따라 sorting 한다
	m_pPickEntityList->DepthSort();

	// 최대로 Rendering 할 atomic 개수로 pick list를 clipping 한다
	m_pPickEntityList->ClippingRenderingEntity( MAX_NUM_OF_MAX_ATOMIC );

	RwUInt16 uiRenderID = 0xffff;

	// 캐릭터의 바운딩 피킹이 검출된 경우만 Rendering picking 을 수행한다
	if ( m_bPickCharacter )
	{
		//////////////////////////////////////////////////////////////////////////
		//
	...
	if ( 0xffff != uiRenderID )
	{
		for ( RwInt32 i = 0; i < iCnt; ++i )
		{
			...
			// Character 관련 엔티티들의 피킹인 경우
			else if ( pPickInfo->uiID == uiRenderID )
			{
				sPickInfo.pPLEntity = pPickInfo->pPLEntity;
				sPickInfo.fMinDist = pPickInfo->fDist;
				sPickInfo.pAtomic = pPickInfo->pAtomic;
...
const RwSphere* CNtlPicking::ResizeBoundingSphereCB( RpAtomic* pAtomic, void* pData )
{
	...
	// 캐릭터 관련 Picking을 제외해야 하는 경우는 구지 Bounding size를 resize 할 필요 없음.
...
RpAtomic* CNtlPicking::PickAtomicBoundBoxCB( RpIntersection* pInters, RpWorldSector* pWorldSector, RpAtomic* pAtomic, RwReal fDistance, void* pData )
{
	...
	if ( pPLEntity )
	{
		...
		if ( pPLEntity->GetClassType() == PLENTITY_OBJECT )
		{
			...
			// 우디 : 툴에서 배치와 픽킹을 동시하는 경우 문제가 발생해서 NTL_PLEFLAG_TOOL_PICK를 추가합니다.
			// 다른 방법으로는 모델툴 상에서 픽킹을 주지않고 툴에서 임의로 체크하는 방법도 있습니다.
		...
		// Object 이 외의 Polygon 검사를 실행 한다
		// 단, 캐릭터 피킹을 제외하는 경우는 polygon 검사를 수행하지 않는다
		else if ( !pPickObj->m_bCharacterExclusion )
		{
				 ...
				 pPLEntity->GetFlags() & NTL_PLEFLAG_PICKING )
			{
				...
				// 오브젝트가 아닌 경우 뒤에 존재하는 엔티티들에 대해 폴리곤 검사를 위해 리스트에 담아야 한다
				// 아래 처럼 전체 피킹 가능 거리를 줄이는 이유는 오브젝트의 경우 피킹 거리를 줄임으로써 속도
				// 향상을 시키기 위해서임...
				if ( fColDist <= s_fMinDist ) s_fMinDist = fColDist;
