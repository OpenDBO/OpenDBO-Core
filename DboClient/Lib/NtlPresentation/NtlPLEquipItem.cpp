...
void CNtlPLEquipItem::AddCItem(CNtlPLItem* _pItem, RwBool _bAttach, RwUInt32 _CBitSet)
{
	...
	if(_CBitSet & eCBSTS_JACKET)	RemoveItem(ITEM_EQUIP_JACKET);
	if(_CBitSet & eCBSTS_PANTS)		RemoveItem(ITEM_EQUIP_PANTS);
	if(_CBitSet & eCBSTS_BOOTS)		RemoveItem(ITEM_EQUIP_BOOTS);
	if(_CBitSet & eCBSTS_TWO_HAND)	RemoveItem(ITEM_EQUIP_TWO_HAND);
	if(_CBitSet & eCBSTS_HEAD)		RemoveItem(ITEM_EQUIP_HEAD);
	if(_CBitSet & eCBSTS_FACE)		RemoveItem(ITEM_EQUIP_FACE);
