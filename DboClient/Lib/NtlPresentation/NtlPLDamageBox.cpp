...
#define NTL_DAMAGEBOX_TEX_PATH		"texture\\effect\\text\\"
...
void CNtlPLDamageBox::CreateResource(void)
{
	...
	for(RwInt32 j = 0; j < MAX_DAMAGEBOX_COLOR; j++)
	{
		for(RwInt32 i = 0; i < MAX_DAMAGEBOX_IMAGE_COUNT; i++)
		{
			m_ppTexture[j][i] = GetNtlResourceManager()->LoadTexture(strTexName[j][i].c_str(), NTL_DAMAGEBOX_TEX_PATH, CNtlPLResourceManager::RES_TEX_EFFECT_DICT);
		}
	}
}
