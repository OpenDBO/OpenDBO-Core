...
RwBool CNtlPLWater::OnRender(CNtlWorldSector* pNtlWorldSector, RxD3D9InstanceData* pInstancedData, RxD3D9ResEntryHeader *pResEntryHeader)
{
	NTL_SPROFILE("CNtlPLWater::OnRender")
		...
		else
		{
			...
			// Probably for some feature added between 1.1 and 1.4 map versions.
			// For example river water has 1001 as Idx instead of 1 so if we don't do this it won't
			// be loaded.
			// 
			...
			if(pNtlWorldSector->m_pWater->_pDepthMap)
			{
				...
				RwD3D9SetTextureStageState(3, D3DTSS_ALPHAOP, D3DTOP_SELECTARG1); // CNtlPLWater::OnRender 占싫울옙占쏙옙 占쏙옙占쏙옙
			...
			else
			{
				...
				RwD3D9SetTextureStageState(3, D3DTSS_ALPHAOP, D3DTOP_SELECTARG2); // CNtlPLWater::OnRender 占싫울옙占쏙옙 占쏙옙占쏙옙
