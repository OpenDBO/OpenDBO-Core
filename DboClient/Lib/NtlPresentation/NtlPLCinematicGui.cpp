...
VOID CNtlPLCinematicGui::SetCinematicBalloon( void* pData )
{
	...
	if( pBalloonData->bShow )
	{
		...
		std::wstring wstrTextData = L"모든 정보를 표시하려면 클래스를 상속해야 합니다";
...
VOID CNtlPLCinematicGui::SetCinematicFlash( void* pData )
{
	...
	if( pFlashData->bShow )
	{
		...
		CNtlPLLuaLinkedFlash* pFlash = NTL_NEW CNtlPLLuaLinkedFlash( rect, pFlashData->pFilename, m_pCinematicObject->GetFlashFrame(),
