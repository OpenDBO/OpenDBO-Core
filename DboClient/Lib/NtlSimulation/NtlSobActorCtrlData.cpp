...
void CSobNormalCtrlData::Dump(CNtlDumpTarget &dump)
{
	...
	dump << "Normal Ctrl Data (server status) : " << m_byMoveStatus << "\n"; 
	dump.Dump();
...
void CSobNormalCtrlData::SetMoveStatus(RwUInt8 byMoveStatus)
{
	m_byMoveStatus = byMoveStatus;
}

RwUInt8 CSobNormalCtrlData::GetMoveStatus(void) const
{
	return m_byMoveStatus;
}
