...
VOID CActionMapManager::InitDefaultActionMap() 
{
	...
	SetSingleKey( 'R', ACTION_AVATAR_AUTORUN );
	SetSingleKey( 'T', ACTION_AVATAR_SCOUTER );
	SetSingleKey( 'Z', ACTION_AVATAR_CHARGE );
	SetSingleKey( 'E', ACTION_AVATAR_LOOTING );
	SetSingleKey( NTL_KEY_SHIFT, ACTION_AVATAR_BLOCKING );

	// Target 선택 및 단축키 설정
	SetSingleKey(NTL_KEY_WAVE, ACTION_TARGET_SELF);
	SetSingleKey(NTL_KEY_F5, ACTION_TARGET_1STPARTY);
	SetSingleKey(NTL_KEY_F6, ACTION_TARGET_2NDPARTY);
	SetSingleKey(NTL_KEY_F7, ACTION_TARGET_3RDPARTY);
	SetSingleKey(NTL_KEY_F8, ACTION_TARGET_4THPARTY);
	SetSingleKey(NTL_KEY_TAB, ACTION_TARGET_AUTO);
	SetSingleKey('Q', ACTION_TARGET_AUTOATK);
	SetSingleKey(NTL_KEY_INSERT, ACTION_TARGET_AUTOFOLLOW);
	...
	SetCombineKey(NTL_CTRL_BIT | '1', ACTION_QUICK_1_EX);
	SetCombineKey(NTL_CTRL_BIT | '2', ACTION_QUICK_2_EX);
	SetCombineKey(NTL_CTRL_BIT | '3', ACTION_QUICK_3_EX);
	SetCombineKey(NTL_CTRL_BIT | '4', ACTION_QUICK_4_EX);
	SetCombineKey(NTL_CTRL_BIT | '5', ACTION_QUICK_5_EX);
	SetCombineKey(NTL_CTRL_BIT | '6', ACTION_QUICK_6_EX);
	SetCombineKey(NTL_CTRL_BIT | '7', ACTION_QUICK_7_EX);
	SetCombineKey(NTL_CTRL_BIT | '8', ACTION_QUICK_8_EX);
	SetCombineKey(NTL_CTRL_BIT | '9', ACTION_QUICK_9_EX);
	SetCombineKey(NTL_CTRL_BIT | '0', ACTION_QUICK_0_EX);
	SetCombineKey(NTL_CTRL_BIT | NTL_KEY_DASH, ACTION_QUICK_MIN_EX);
	SetCombineKey(NTL_CTRL_BIT | NTL_KEY_EQUAL, ACTION_QUICK_PLUS_EX);
	...
	SetSingleKey(NTL_KEY_F9, ACTION_QUICK_2NDCAP);
	SetSingleKey(NTL_KEY_F10, ACTION_QUICK_3RDCAP);
	SetSingleKey(NTL_KEY_F11, ACTION_QUICK_4THCAP);
	SetSingleKey(NTL_KEY_F12, ACTION_QUICK_5THCAP);
	...
	SetSingleKey('U', ACTION_WINDOW_SCOUTER);
	SetSingleKey('P', ACTION_WINDOW_PARTY);
	SetSingleKey('G', ACTION_WINDOW_GUILD);
	SetSingleKey('F', ACTION_WINDOW_FRIEND);
	SetSingleKey('O', ACTION_WINDOW_OPTION);
	SetSingleKey(NTL_KEY_F1, ACTION_WINDOW_HELP);
	SetSingleKey(NTL_KEY_F2, ACTION_WINDOW_MAIN);
...
/**
* \brief 액션맵에 인풋 모드를 지정한다.
* \param wAction (WORD) 인풋 모드가 될 ACTION ID
* \return 성공여부
*/
BOOL CActionMapManager::SetInputAction( WORD wAction ) 
{
	// 만약 액션이 고정되어 있는 액션이라면
...
WORD CActionMapManager::InputHandlerUp( BYTE byKey, std::list< WORD >& listUpAction )
{
	...
	if( IsCombineKey( byKey ) )
	{
		// 현재의 조합키로 발동되는 모든 눌려진 액션을 Up 시켜줘야 한다.
...
VOID CActionMapManager::RegisterKeyType( VOID ) 
{
	...
	m_mapKeyType[NTL_KEY_CONTROL]	= TYPE_COMBINE;	// CTRL
	m_mapKeyType[NTL_KEY_MENU]		= TYPE_COMBINE; // ALT
}
