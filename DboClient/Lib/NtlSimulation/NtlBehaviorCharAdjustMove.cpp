...
void CNtlBehaviorCharGroundAdjustMove::Enter(void)
{
    ...
    // Run Speed의 2배보다 차이가 작은 경우에만 보간하고, 넘어서면 점프해버린다.
    if(fLen < fSpeed  * 2.0f)
    {
...
void CNtlBehaviorCharGroundAdjustMove::SetAnim() 
{
    ...
    if( m_MoveStuff.byMoveFlags == NTL_MOVE_F || m_MoveStuff.byMoveFlags == NTL_MOVE_F_TURN_L ||
        m_MoveStuff.byMoveFlags == NTL_MOVE_F_TURN_R )
    {
...
RwBool CNtlBehaviorCharGroundAdjustMove::IsNotAdjustMove() 
{
    ...
    if(pMoveStuff->byMoveFlags == NTL_MOVE_NONE ||
       ...
       (pMoveStuff->byMoveFlags >= NTL_MOVE_F_TURN_L_JUMP && pMoveStuff->byMoveFlags <= NTL_MOVE_B_TURN_R_JUMP))
    {
        return TRUE;
    }
    else if(pJumpStuff->eState != JUMPSTATE_NONE)       ///< 점프한 후에도 Adjust 하지 않는다.
    {
