...
CDboTSCQCtrl::CDboTSCQCtrl( void )
{
...
RwUInt32 CDboTSCQCtrl::GetTitle( void )
{
	if ( 0 == m_pTrigger )
	{
		return m_uiTitle;
		...
		case DBO_CONT_TYPE_ID_CONT_GCOND:
			{
				...
				UG_Avatar_TS_Confirm_Step( pCondCont->GetID(), pCondCont->GetYesLinkID(), 0xffffffff, GetEventType(), GetEventData() );
...
NTL_TSRESULT CDboTSCQCtrl::Cont_GCond( CNtlTSCont* pCont, bool bDoAction /*= true*/ )
{
	...
	if ( NTL_TSRESULT_TYPE_SUCCESS == tsResult )
	{
		...
		if ( bDoAction ) UG_Avatar_TS_Confirm_Step( tcID, pNextCont->GetID(), 0xffffffff, GetEventType(), GetEventData() );
	}
...
NTL_TSRESULT CDboTSCQCtrl::Cont_Start( CNtlTSCont* pCont, bool bDoAction /*= true*/ )
{
	...
	if ( NTL_TSRESULT_TYPE_SUCCESS == tsResult )
	{
		...
		// 서버에게 다음 단계로 진행해도 되는지 물어본다
		if ( bDoAction ) UG_Avatar_TS_Confirm_Step( tcID, pNextCont->GetID(), IsQuestShareMode() ? 0 : 0xffffffff, GetEventType(), GetEventData() );
	}
...
NTL_TSRESULT CDboTSCQCtrl::Cont_End( CNtlTSCont* pCont, bool bDoAction /*= true*/ )
{
	...
	// 서버에게 다음 단계로 진행해도 되는지 물어본다
	if ( bDoAction ) UG_Avatar_TS_Confirm_Step( tcID, NTL_TS_TC_ID_INVALID, 0xffffffff, GetEventType(), GetEventData() );
...
void CDboTSCQCtrl::ChangeTSState( unsigned int uiChangFlag )
{
	...
	else if ( eTS_PROG_STATE_EXIT == uiChangFlag )
	{
		if ( IsExitState() )
		{
			...
			// 클라이언트 UI 업데이트
			((CDboTSCQAgency*)GetParent())->TU_UnregQuestInfoNfy( GetTrigger()->GetID() );

			// 클라이언트에게 퀘스트 종료를 알림
			((CDboTSCQAgency*)GetParent())->TU_FinishQuest( TS_TYPE_QUEST_CS, GetTrigger()->GetID() );
		}
	}
...
void CDboTSCQCtrl::LoadQuestProgressInfo_V0( const sPROGRESS_QUEST_INFO::uDATA& uData )
{
	...
	// 로딩 시점에서 퀘스트의 등록 정보가 없으면서
	// 에러나 실패 상태인 경우 퀘스트를 제거한다
	if ( (NTL_TS_TC_ID_INVALID == m_tcQuestInfo || NTL_TS_TA_ID_INVALID == m_taQuestInfo) &&
		 (IsError() || IsFailed()) )
	{
		SetExitState();
	...
	if ( NTL_TS_TC_ID_INVALID != m_tcQuestInfo && NTL_TS_TA_ID_INVALID != m_taQuestInfo )
	{
		...
		if ( pCont )
		{
			...
			if ( pEntity && pEntity->IsDerivedClass( "CDboTSActRegQInfo" ) )
			{
				...
				{
					m_pDefRwd = (sREWARD_INFO*)&((CDboTSContReward*)pContNew)->GetDefRewardInfo( 0, API_GetTableContainer()->GetQuestRewardTable() );
					m_pSelRwd = (sREWARD_INFO*)&((CDboTSContReward*)pContNew)->GetSelRewardInfo( 0, API_GetTableContainer()->GetQuestRewardTable() );
				}
				else
				{
					m_pDefRwd = 0;
	...
	if ( pCurTSP && pCurTSP->GetEntityType() == DBO_CONT_TYPE_ID_CONT_NARRATION )
	{
		...
		{
			UG_Avatar_TS_Confirm_Step( pCurTSP->GetID(), tcNextID, 0xffffffff, GetEventType(), GetEventData() );
		}
		else
		{
...
void CDboTSCQCtrl::GU_Avatar_TS_Confirm_Step( WORD wResultCode, NTL_TS_TC_ID tcCurId, NTL_TS_TC_ID tcNextId )
{
	...
	if ( GAME_TS_ERROR_BEGIN <= wResultCode && wResultCode < GAME_TS_ERROR_END )
	{
		SetError();
		CNtlTSLog::Log( "Quest confirm step error. Info[%d,%d,%d]. [%s]", wResultCode, tcCurId, tcNextId, TS_CODE_TRACE() );
		return;
	}
	else if ( GAME_TS_WARNING_BEGIN <= wResultCode && wResultCode < GAME_TS_WARNING_END )
	{
		...
		if ( pCurCont->GetEntityType() == DBO_CONT_TYPE_ID_CONT_GACT )
		{
			...
			if ( pErrorCont )
			{
				...
				UG_Avatar_TS_Confirm_Step( tcCurId, pErrorCont->GetID(), 0xffffffff, GetEventType(), GetEventData() );
...
void CDboTSCQCtrl::UT_Party( void )
{
	if ( IsError() ) return;
	if ( IsFailed() ) return;
	if ( IsCSComunication() ) return;
...
void CDboTSCQCtrl::UT_ShowProposalDialog( sTS_KEY& sKey, bool bAccept )
{
	...
	UG_Avatar_TS_Confirm_Step( sKey.tcID, tcNextID, 0xffffffff, GetEventType(), GetEventData() );
}
...
void CDboTSCQCtrl::UT_ShowRewardDialog( sTS_KEY& sKey, int nSelRwdIdx, bool bCancel )
{
	...
	if ( bCancel )
	{
		...
		if ( pCancCont )
		{
			UG_Avatar_TS_Confirm_Step( m_pCurTSP->GetID(), pCancCont->GetID(), 0xffffffff, GetEventType(), GetEventData() );
		}
		else
		{
			// NPC 강제 카메라 이동에 대한 처리
	...
	else
	{
		...
		UG_Avatar_TS_Confirm_Step( m_pCurTSP->GetID(), pNextCont->GetID(), nSelRwdIdx, GetEventType(), GetEventData() );
	}
}
...
void CDboTSCQCtrl::UT_ShowUserSelectionDialog( sTS_KEY& sKey, NTL_TS_TC_ID tcSelId, bool bCancel )
{
	...
	if ( bCancel )
	{
		...
		UG_Avatar_TS_Confirm_Step( sKey.tcID, pNextCont->GetID(), 0xffffffff, GetEventType(), GetEventData() );
	}
	else if ( ((CDboTSContUsrSel*)m_pCurTSP)->IsNextLink( tcSelId ) )
	{
		...
		UG_Avatar_TS_Confirm_Step( sKey.tcID, tcSelId, 0xffffffff, GetEventType(), GetEventData() );
	}
	else
	{
		CNtlTSLog::Log( "Must have the next container. Info[%d,%d,%d,%d]. [%s]", sKey.tID, sKey.tgID, sKey.tcID, tcSelId, TS_CODE_TRACE() );
...
void CDboTSCQCtrl::TU_RegisterQuestInfo( sTS_KEY& sKey, CDboTSActRegQInfo* pAct )
{
	...
	{
		m_pDefRwd = (sREWARD_INFO*)&((CDboTSContReward*)pCont)->GetDefRewardInfo( 0, API_GetTableContainer()->GetQuestRewardTable() );
		m_pSelRwd = (sREWARD_INFO*)&((CDboTSContReward*)pCont)->GetSelRewardInfo( 0, API_GetTableContainer()->GetQuestRewardTable() );
	}
	else
	{
		m_pDefRwd = 0;
