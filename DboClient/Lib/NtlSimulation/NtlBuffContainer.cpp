...
void CNtlBuffContainer::CreateEventHandler(RWS::CMsg &pMsg)
{
	for( RwInt32 i = 0 ; i < GetNtlSLGlobal()->GetAvatarInfo()->byBuffCount ; ++i )
	{
		...
		CNtlSobBuff *pSobBuff = reinterpret_cast<CNtlSobBuff*>( CNtlSLEventGenerator::SobBuffCreate(SLCLASS_BUFF, INVALID_SERIAL_ID, pBuffInfo->bySourceType, pBuffInfo->sourceTblidx, pBuffInfo->dwTimeRemaining, pBuffInfo->dwInitialDuration, pBuffInfo->afEffectValue) );
...
void CNtlBuffContainer::AddEventHandler(RWS::CMsg &pMsg)
{
	...
	CNtlSobBuff *pSobBuff = reinterpret_cast<CNtlSobBuff*>( CNtlSLEventGenerator::SobBuffCreate(SLCLASS_BUFF, INVALID_SERIAL_ID, pBuffAdd->byBuffType, pBuffAdd->uiTblId, pBuffAdd->uiRemainTime, pBuffAdd->uiKeepTime, pBuffAdd->afEffectValue) );
	pSobBuff->SetOwnerID(m_pSobObj->GetSerialID());
	m_listBuff.push_back(pSobBuff);
...
void CNtlBuffContainer::DropEventHandler(RWS::CMsg &pMsg)
{
	...
	for(it = m_listBuff.begin(); it != m_listBuff.end(); ++it)
	{
		...
		if(pSobBuffAttr->GetTblIdx() == pBuffDrop->uiTblId)
		{
			CNtlSLEventGenerator::SobDelete(pSobBuff->GetSerialID());
			m_listBuff.erase(it);
			break;
...
void CNtlBuffContainer::RefreshAllEventHandler(RWS::CMsg &pMsg)
{
	...
	{
		pSobBuff = reinterpret_cast<CNtlSobBuff*>( CNtlSLEventGenerator::SobBuffCreate(SLCLASS_BUFF, INVALID_SERIAL_ID, pBuffRefreshAll->pBuffInfo[i].bySourceType, pBuffRefreshAll->pBuffInfo[i].sourceTblidx, pBuffRefreshAll->pBuffInfo[i].dwTimeRemaining, pBuffRefreshAll->pBuffInfo[i].dwInitialDuration, pBuffRefreshAll->pBuffInfo[i].afEffectValue) );
		pSobBuff->SetOwnerID(m_pSobObj->GetSerialID());
		m_listBuff.push_back(pSobBuff);
	}
...
void CNtlFakeBuffContainer::AddBuff(eDBO_OBJECT_SOURCE eType, RwUInt32 uiTableIndex, RwReal fRemainTime, RwReal* pfEffectValue /* = NULL */)
{
	...
	if(pNewFakeBuff->eType == DBO_OBJECT_SOURCE_SKILL)
	{
		...
		for( RwInt32 i = 0 ; i < NTL_MAX_EFFECT_IN_SKILL ; ++i )
		{
			...
			if( pfEffectValue )
				pNewFakeBuff->afEffectValue[i] = pfEffectValue[i];
		}	
	}
	else if(pNewFakeBuff->eType == DBO_OBJECT_SOURCE_ITEM)
	{
		...
		pNewFakeBuff->fMaxTime = (RwReal)pUseItemData->wKeep_Time;
...
void CNtlFakeBuffContainer::DropBuff(eDBO_OBJECT_SOURCE eType, RwUInt32 uiTableIndex)
{
	sNtlFakeBuff* pFakeBuff;
	LIST_FAKE_BUFF_ITER it = m_listBuff.begin();
	for( ; it != m_listBuff.end() ; ++it )
	{
		...
		{
			if( pFakeBuff->pSkillTbl->tblidx == uiTableIndex )
				break;
		}
		else if(pFakeBuff->eType == DBO_OBJECT_SOURCE_ITEM)
		{
			if( pFakeBuff->pItemTbl->tblidx == uiTableIndex )
				break;
		}
	}
...
void CNtlFakeBuffContainer::HandleEvents(RWS::CMsg &pMsg)
{
	...
	if(pMsg.Id == g_EventSobFakeBuffAdd)
	{
		...
		{
			AddBuff((eDBO_OBJECT_SOURCE)pBuffAdd->byBuffType, pBuffAdd->uiTblId, (RwReal)pBuffAdd->uiRemainTime / 1000.0f, pBuffAdd->afEffectValue);
	...
	else if(pMsg.Id == g_EventSobFakeBuffDrop)
	{
		...
		{
			DropBuff((eDBO_OBJECT_SOURCE)pBuffDrop->byBuffType, pBuffDrop->uiTblId);
			CNtlSLEventGenerator::SobUpdate(m_hOwner, EVENT_AIUT_BUFF, 0);
		}		
	}
	else if(pMsg.Id == g_EventSobFakeBuffRefreshAll)
	{
		...
		if( pBuffRefreshAll->hSerialId == m_hOwner )
		{
			for( RwInt32 i = 0 ; i < pBuffRefreshAll->byCount ; ++i )
			{
				AddBuff((eDBO_OBJECT_SOURCE)pBuffRefreshAll->pBuffInfo[i].bySourceType,
					    pBuffRefreshAll->pBuffInfo[i].sourceTblidx,
					    (RwReal)pBuffRefreshAll->pBuffInfo[i].dwTimeRemaining / 1000.0f, 
						pBuffRefreshAll->pBuffInfo->afEffectValue);
			}
