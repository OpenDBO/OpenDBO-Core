...
RwBool CNtlSobItemAttr::IsScouterPartItem(void)
{
	if(m_pItemTbl)
	{
		if(m_pItemTbl->byItem_Type == ITEM_TYPE_SCOUTER_PART)
			return TRUE;	
	}
...
void CNtlSobItemAttr::HandleEvents(RWS::CMsg &pMsg)
{
	if(pMsg.Id == g_EventSobCreate)
	{
		...
		if( !pSobItemCreate->bNeedToIdentify )
		{
			...
			NTL_ASSERT( m_pItemTbl, "CNtlSobItemAttr::HandleEvents is table id invalid(" <<  pSobItemCreate->uiItemTblId << ")" );
            if(!m_pItemTbl)
                return;
			...
			m_uiOptionIdx1 = pSobItemCreate->uiOptionTblId1;
			m_uiOptionIdx2 = pSobItemCreate->uiOptionTblId2;
			m_ucBattleAttribute = (RwUInt8)pSobItemCreate->byBattleAttribute;
			m_ucRestrictType = pSobItemCreate->byRestrictType;
	...
	else if( pMsg.Id == g_EventSobItemUpdate)
	{
		...
		if( !pSobItemUpdate->bNeedToIdentify )
		{
			...
			m_byMaxDur = m_pItemTbl->byDurability;
			m_uiOptionIdx1 = pSobItemUpdate->uiOptionTblId1;
			m_uiOptionIdx2 = pSobItemUpdate->uiOptionTblId2;
			m_ucBattleAttribute = pSobItemUpdate->byBattleAttribute;
			m_ucRestrictType = pSobItemUpdate->byRestrictType;
