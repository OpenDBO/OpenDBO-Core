...
{
	// peessiItemAttr : 현재 창고에서는 시간제한 아이템을 보관할 수 없으나, 소유권 있는 아이템은 옮겨갈 수 있다.
	void* pSobObj = CNtlSLEventGenerator::SobSlotItemCreate(pParentItem, SLCLASS_SLOT_ITEM,
		...
		pItemData->aOptionTblidx[0], pItemData->aOptionTblidx[1],
		byPlace, byPos,
		...
		pItemData->byBattleAttribute, ITEM_RESTRICT_TYPE_NONE, NULL, eDURATIONTYPE_NORMAL, 0, 0);   
...
void CNtlGuildWarehouse::CreateGuildWarehouseItem(RwUInt8 byIndex, RwUInt8 byPos, SERIAL_HANDLE hSrcItem)
{
	...
	aWarehouseSlot[byIndex][byPos].aOptionTblidx[0]		= pSobItemAttr->GetOptionIdx1();
	aWarehouseSlot[byIndex][byPos].aOptionTblidx[1]		= pSobItemAttr->GetOptionIdx2();
}
...
{	
	...
	else if( pMsg.Id == g_EventGuildWarehouseItemInfo )
	{
		...
		for( RwUInt8 i = 0 ; i < pEvent->byItemCount ; ++i )
		{
			...
			for(RwUInt8 j = 0 ; j < NTL_MAX_OPTION_IN_ITEM ; ++j )
				aWarehouseSlot[byIndex][pITEM_PROFILE->byPos].aOptionTblidx[j] = pITEM_PROFILE->aOptionTblidx[j];
...
void CNtlGuildWarehouse::ItemMoveEventHandler(RWS::CMsg &pMsg)
{
	...
	if( CONTAINER_TYPE_GUILD_FIRST <= pEvent->bySrcPlace && pEvent->bySrcPlace <= CONTAINER_TYPE_GUILD_LAST )
	{
		...
		else if( CONTAINER_TYPE_SCOUT == pEvent->byDestPlace )
		{
			CNtlInventory*	pInventory		= pAvatar->GetInventory();
			CNtlSobItem*	pScouterItem	= reinterpret_cast<CNtlSobItem*>( GetNtlSobManager()->GetSobObject( pInventory->GetScouterItem() ) );

			if( !pScouterItem )
			{
				DBO_FAIL("CONTAINER_TYPE_SCOUT == pEvent->byDestPlace , Not exist equip scouter of handle : " << pInventory->GetScouterItem());
				NTL_RETURNVOID();
			}
			...
			CreateSobItem(pScouterItem, pAvatar->GetSerialID(), &tempSrcSlotData,
				CONTAINER_TYPE_SCOUT, pEvent->byDestPos);
	...
	else if( CONTAINER_TYPE_GUILD_FIRST <= pEvent->byDestPlace && pEvent->byDestPlace <= CONTAINER_TYPE_GUILD_LAST )
	{
		...
		else if( CONTAINER_TYPE_SCOUT == pEvent->bySrcPlace )
		{
			CNtlInventory* pInventory = pAvatar->GetInventory();
			CNtlSobItem* pParentItem = reinterpret_cast<CNtlSobItem*>( GetNtlSobManager()->GetSobObject(pInventory->GetScouterItem()) );

			if( !pParentItem )
			{
				DBO_FAIL("Not exist sob item of handle : " << pInventory->GetScouterItem());
				NTL_RETURNVOID();
			}			
			...
			if( pEvent->hDestItem != INVALID_SERIAL_ID )
			{
				...
				if( !pDestSobItem )
				{
					DBO_FAIL("CONTAINER_TYPE_SCOUT == pEvent->bySrcPlace, Can not create sob item of handle : " << aWarehouseSlot[byDestIndex][pEvent->byDestPos].hHandle);
					NTL_RETURNVOID();
				}				
			}
		}		
		...
		if( CONTAINER_TYPE_GUILD_FIRST <= pEvent->bySrcPlace && pEvent->bySrcPlace <= CONTAINER_TYPE_GUILD_LAST )
		{
			...
			else
			{
				...
				CNtlSLEventGenerator::SobItemAdd( GetNtlSLGlobal()->GetSobAvatar()->GetSerialID(),
					pEvent->hDestItem, rDestItem.pITEM_TBLDAT->tblidx, rDestItem.aOptionTblidx[0], rDestItem.aOptionTblidx[1],
					...
					rDestItem.bNeedToIdentify, rDestItem.byBattleAttribute, ITEM_RESTRICT_TYPE_NONE, NULL, eDURATIONTYPE_NORMAL, 0, 0 );
