...
CNtlFSMPlayerController::CNtlFSMPlayerController()
{
    ...
    m_mapEventFunc[g_EventActionMapScouterUse.Get_pEventId()]  			= &CNtlFSMPlayerController::ActionMapScouterUseHandler;
	...
	case CHARSTATE_STANDING:
		{
			...
			else if(byServerOldState == CHARSTATE_DIRECT_PLAY ||
                    byServerOldState == CHARSTATE_RIDEON)
			{
				return NTL_FSM_EVENTRES_PASS;
			}            
		}
...
RwUInt32 CNtlFSMPlayerController::ActionMapScouterUseHandler( RWS::CMsg &pMsg ) 
{
    CNtlFSMCharActAgent *pCharActAgent = reinterpret_cast<CNtlFSMCharActAgent*>(m_pAgent);

    if(pCharActAgent->IsDirectMode() || pCharActAgent->IsDie() ||
       pCharActAgent->IsFightingMode() || pCharActAgent->IsSitDown())
    {
        NTL_RETURN(NTL_FSM_EVENTRES_BLOCK);
    }
...
void CNtlFSMPlayerController::NotifyOldUpdateStateProc(RwUInt32 uiOldState, RwUInt32 uiNextState)
{
	...
	if(uiOldState == NTL_FSMSID_FOLLOW)
	{
		...
		if(pTarActor)
		{
			if(Logic_CanActorCommunity(m_pActor, pTarActor, pCtrlStuff->fFollowRange))
			{
	...
	else if(uiOldState == NTL_FSMSID_JUMP)
	{
        ...
        CNtlFSMCharActAgent *pCharActAgent = reinterpret_cast<CNtlFSMCharActAgent*>(m_pAgent);
        if(pCharActAgent->IsAutoRun()) // 자동 달리기 도중에 점프 했을때
        {
...
void CNtlFSMPlayerController::NotifyNextUpdateStateChaseFollowProc(RwUInt32 uiOldState)
{
	...
	API_GetSLPacketGenerator()->SendCharFollowMove(pCtrlStuff->hTargetSerialId, pCtrlStuff->fFollowRange, NTL_FOLLOW_CHASE, m_pActor->GetServerSyncAvatarType());
}
...
void CNtlFSMPlayerController::NotifyNextUpdateStateSkillFollowProc(RwUInt32 uiOldState)
{
	...
	API_GetSLPacketGenerator()->SendCharFollowMove(pCtrlStuff->hTargetSerialId, pCtrlStuff->fFollowRange, NTL_FOLLOW_SKILL, m_pActor->GetServerSyncAvatarType());
}
...
void CNtlFSMPlayerController::NotifyNextUpdateStateFallingProc(RwUInt32 uiOldState)
{
	...
	RwUInt8 byMoveFlags = pCtrlStuff->uExtra.sFalling.byMoveFlags;
	if(byMoveFlags == NTL_MOVE_NONE || byMoveFlags >= NTL_MOVE_MOUSE_MOVEMENT)
		byMoveFlags = NTL_MOVE_F;

	API_GetSLPacketGenerator()->SendFalling(TRUE, vPos, vDir, byMoveFlags);
}
...
void CNtlFSMPlayerController::NotifyActionMapJumpEventHandler(RWS::CMsg &pMsg, RwUInt32 uiEventResult, RwUInt32 uiParam1, RwUInt32 uiParam2)
{
	...
	API_GetSLPacketGenerator()->SendJump(vDir);
...
void CNtlFSMPlayerController::NotifyActionMapSkillUseEventHandler(RWS::CMsg &pMsg, RwUInt32 uiEventResult, RwUInt32 uiParam1, RwUInt32 uiParam2)
{
	...
	{
		API_GetSLPacketGenerator()->SendCharFollowMove(pActionMapSkillUse->hTargetSerialId, pActionMapSkillUse->fFollowRange, NTL_FOLLOW_SKILL, m_pActor->GetServerSyncAvatarType());
...
void CNtlFSMPlayerController::NotifyActionMapHTBUseEventHandler(RWS::CMsg &pMsg, RwUInt32 uiEventResult, RwUInt32 uiParam1, RwUInt32 uiParam2)
{
	...
	if(uiNextState == NTL_FSMSID_HTB_FOLLOW)
	{
		...
		API_GetSLPacketGenerator()->SendCharFollowMove(pActionMapHTBUse->hTargetSerialId, pActionMapHTBUse->fFollowRange, NTL_FOLLOW_HTB, m_pActor->GetServerSyncAvatarType());
	}
}
...
void CNtlFSMPlayerController::NotifyActionMapItemUseEventHandler(RWS::CMsg &pMsg, RwUInt32 uiEventResult, RwUInt32 uiParam1, RwUInt32 uiParam2)
{
	...
	SNtlEventActionMapItemUse *pActionMapItemUse = reinterpret_cast<SNtlEventActionMapItemUse*>(pMsg.pData); 
	API_GetSLPacketGenerator()->SendCharItemUseReq(pActionMapItemUse->hTargetSerialId, pActionMapItemUse->byPlace, pActionMapItemUse->byPos);
}
...
void CNtlFSMPlayerController::NotifyKeyboardMoveEventHandler(RWS::CMsg &pMsg, RwUInt32 uiEventResult, RwUInt32 uiParam1, RwUInt32 uiParam2)
{
	...
	if(uiCurrState == NTL_FSMSID_JUMP || uiCurrState == NTL_FSMSID_FALLING)
	{
        ...
        if(pMoveStuff->byMoveFlags == NTL_MOVE_F || pMoveStuff->byMoveFlags == NTL_MOVE_B)
        {
...
void CNtlFSMPlayerController::NotifyKeyboardMoveStopEventHandler(RWS::CMsg &pMsg, RwUInt32 uiEventResult, RwUInt32 uiParam1, RwUInt32 uiParam2)
{
	...
	if(uiCurrState == NTL_FSMSID_JUMP)
	{
		...
		API_GetSLPacketGenerator()->SendKeyboardMove(vPos, vDir, pMoveStuff->byMoveFlags, m_pActor->GetServerSyncAvatarType());
	}
	else
		API_GetSLPacketGenerator()->SendKeyboardMove(vPos, vDir, NTL_MOVE_NONE, m_pActor->GetServerSyncAvatarType());
}
...
{
    m_pSyncMsgAttacKBegin->SetAttackBeginFlag(FALSE);
    API_GetSLPacketGenerator()->SendCharAttackEnd(m_pSyncMsgAttacKBegin->GetAvatarControlId(), m_pActor->GetServerSyncAvatarType());
