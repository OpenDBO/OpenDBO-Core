...
void CNtlFSMSyncMsgAttackBegin::Update(CNtlSobActor *pActor, RwReal fElapsed)
{
	...
	else
	{
		...
		if(m_bAttackBegin)
		{
			if(fSubDist > Logic_GetAttackRange(pActor, m_hTargetSerialId) * 1.3f || 
				!pActor->IsFightingMode())
			{
				m_bAttackBegin = FALSE;
				API_GetSLPacketGenerator()->SendCharAttackEnd(m_byAvatarControlId, pActor->GetServerSyncAvatarType());
			}
		...
		else
		{
			...
			if(pActor->IsFightingMode() && (fSubDist - 0.1f) <= Logic_GetAttackRange(pActor, m_hTargetSerialId) && 
				!pTarActor->IsDie())
			{
				m_bAttackBegin = TRUE;
