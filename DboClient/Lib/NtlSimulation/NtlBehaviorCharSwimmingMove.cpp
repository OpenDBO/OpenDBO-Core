...
void CNtlBehaviorCharSwimmingMove::SetAnim(void)
{
    ...
    {
        if( m_MoveStuff.byMoveFlags == NTL_MOVE_F || m_MoveStuff.byMoveFlags == NTL_MOVE_F_TURN_L ||
            m_MoveStuff.byMoveFlags == NTL_MOVE_F_TURN_R || m_MoveStuff.byMoveFlags == NTL_MOVE_F_TURN_L)
        {
            uiNextAnimKey = SWIM_FRONT;
        }
        else if( m_MoveStuff.byMoveFlags == NTL_MOVE_B || m_MoveStuff.byMoveFlags == NTL_MOVE_B_TURN_L ||
            m_MoveStuff.byMoveFlags == NTL_MOVE_B_TURN_R || m_MoveStuff.byMoveFlags == NTL_MOVE_B_TURN_L )
        {
...
void CNtlBehaviorCharSwimmingMove::CreateSwimFrontEffect(void)
{
    ...
    if(m_MoveStuff.byMoveFlags == NTL_MOVE_B || m_MoveStuff.byMoveFlags == NTL_MOVE_B_TURN_L ||
        m_MoveStuff.byMoveFlags == NTL_MOVE_B_TURN_R)
...
RwBool CNtlBehaviorCharSwimmingMove::UpdateDirectionMove(RwReal fElapsed)
{
    ...
    NtlGetDestination_Keyboard(vDir.x, vDir.z, fSpeed, vPos.x, vPos.y, vPos.z, m_MoveStuff.byMoveFlags, (DWORD)(fElapsed*1000.f), 0.5f, &vHeading, &vDest);
...
RwBool CNtlBehaviorCharSwimmingMove::UpdateLocationMove(RwReal fElapsed)
{
    RwV3d vPos = m_pActor->GetPosition();
    RwV3d vDestPos;
    RwV3dAssign(&vDestPos, &m_MoveStuff.vDest); 
