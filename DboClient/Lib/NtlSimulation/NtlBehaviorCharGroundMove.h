...
public:
    ...
    void     SetNotAnimChange(RwBool bNotChange) {m_bNotAnimChange = bNotChange;}       ///< 애니메이션을 변경하지 않는다. (회전공격용)
...
protected:
    ...
    RwBool				m_bSlowMove;			/** 이동이 처음 시작할 때 타켓하고 거리가 가까울 경우운 경우... 아주 천천히 이동하는 보정을 한다 */
    ...
    RwBool              m_bNotAnimChange;            ///< 애니메이션을 변경하지 않는 플래그
};
