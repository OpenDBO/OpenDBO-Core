...
NTL_TSRESULT CDboTSCTCtrl::Cont_GCond( CNtlTSCont* pCont )
{
	...
	if ( NTL_TSRESULT_TYPE_SUCCESS == tsResult )
	{
		...
		UG_Avatar_TS_Confirm_Step( tcID, pNextCont->GetID(), 0xffffffff, GetEventType(), GetEventData() );
	}
...
NTL_TSRESULT CDboTSCTCtrl::Cont_End( CNtlTSCont* pCont )
{
	...
	// 서버에게 다음 단계로 진행해도 되는지 물어본다
	UG_Avatar_TS_Confirm_Step( tcID, NTL_TS_TC_ID_INVALID, 0xffffffff, GetEventType(), GetEventData() );
