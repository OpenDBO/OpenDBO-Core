...
void CNtlBehaviorSmoothHeading::Enter(void)
{
	...
	SetAnim(pCtrlStuff->uExtra.sRotate.fDeltaAngle);
...
{
	// 1초에 120도를 돈다.
	RwReal fDeltaAngle = m_fUpdateAngle * fElapsed;

	if(m_fDeltaAngle < 0.0f)
	{
		if(m_fDeltaAngle + fDeltaAngle > 0.0f)
		{
			m_pActor->SetDeltaAngleY(m_fDeltaAngle);
			Finish();
		}
		else
		{
			m_pActor->SetDeltaAngleY(-fDeltaAngle);
			m_fDeltaAngle += fDeltaAngle;
		}
	}
	else
	{
...
void CNtlBehaviorSmoothHeading::SetAnim(RwReal fDeltaAngle)
{
	...
	RwReal fAbsDeltaAngle = (RwReal)fabs(m_fDeltaAngle);
		
	if(m_fDeltaAngle < 0.0f)
	{
		if(m_fDeltaAngle < -SMOOTH_TURN_LIMIT_ANGLE)
		{
			m_pActor->SetDeltaAngleY(m_fDeltaAngle + SMOOTH_TURN_LIMIT_ANGLE);
			m_fDeltaAngle = -SMOOTH_TURN_LIMIT_ANGLE;
		}
		
		if(fAbsDeltaAngle > 3.0f)
		{
	...
	m_fUpdateAngle = (RwReal)fabs(120*m_fDeltaAngle/SMOOTH_TURN_LIMIT_ANGLE);
	if(m_fUpdateAngle < 20.0f)
		m_fUpdateAngle = 20.0f;
}
...
void CNtlBehaviorCharNormalAttack::SetTransform(const SAttackStuff *pAttackStuff)
{
	...
	RwV3dAssign(&m_vPushDir, &pAttackStuff->vPushDir);
	...
	RwV3d vAttDir;
	RwV3dSub(&vAttDir, &vTarPos, &vAttPos);
	vAttDir.y = 0.0f;
...
void CNtlBehaviorCharNormalAttack::CalcHitStuff(const SAttackStuff *pAttackStuff, RwInt8 byHitGroupIdx)
{
	...
	for(RwInt32 i = 0; i < m_iHitNum; ++i)
	{
		...
		// 공격 결과가 knockdown이 아닐 경우, Target Behavior가 knockdown이구 
		// push flag가 설정되어 있을 경우 behavior를 push로 설정한다.
		...
		//-------------------------------------------------------
		// push direction 설정.
		RwV3dAssign(&pTempHit->vPushDir, &pAttackStuff->vPushDir);
...
void CNtlBehaviorCharPushFollow::Update(RwReal fElapsed)
{
    ...
    // 충돌체크를 한다. Push로 인해서 이동하지 못한 지역으로 이동하는 버그가 있었다.
...
{
	RwV3dAssign(&m_vPushDir, pPushDir);
}
...
void CNtlBehaviorCharSliding::UpdateData(void)
{
	...
	RwV3d vNewPushDir;
	RwV3dSub(&vNewPushDir, &m_vDestPos, &vAttPos);
	vNewPushDir.y = 0.0f;
	...
	RwV3d vPos = m_pActor->GetPosition();
	RwV3dSub(&m_vPushDir, &vNewDestPos, &vPos);
	m_vPushDir.y = 0.0f;
...
{
	RwV3dAssign(&m_vPushDir, pSlidingDir);
}
...
void CNtlBehaviorCharShock::ResetTimeSequence(void)
{
	...
	RwV3dSub(&m_vPushDir, &vDefPos, &vAttPos);
	m_vPushDir.y = 0.0f;
	RwV3dNormalize(&m_vPushDir, &m_vPushDir); 
	...
	RwV3dAssign(&m_vDestPos, &vDefPos);
}
	...
	case KD_KNOCK:
		{
			...
			if(m_pActor->GetFlags() == SLFLAG_SERVER_SENDER)
				API_GetSLPacketGenerator()->SendChangePosition(m_pActor->GetPosition(), m_pActor->GetDirection(), m_pActor->GetServerSyncAvatarType());
		}
		break;
	case KD_KNOCK_EXIT:
		{
...
{
	RwV3dAssign(&m_vKnockDownDir, pKnockDownDir);
...
void CNtlBehaviorCharHurt::SetTrans(SHitStuff *pHitStuff)
{
	...
	RwV3dSub(&vHurtDir, &vAttPos, &vDefPos);
	vHurtDir.y = 0.0f;
...
void CNtlBehaviorCharHomingUp::Update(RwReal fElapsed)
{
	...
	RwV3d vSub;
	RwV3dSub(&vSub, &vTargetPos, &m_vOldTargetPos);
...
void CNtlBehaviorCharStretchAttack::Update( RwReal fElapsed ) 
{
    ...
    if(m_bPulling)              // 당겨오는 상태
    {
        ...
        if(m_pStretchEvent->eType == E_STRETCH_PULLING) // 히트면 타겟은 당겨오지 않는다.
        {
            // 타겟의 위치
            SWorldHeightStuff hStuff;
...
void CNtlBehaviorCharActiveWarpBackAttack::OnWarpBack() 
{
    ...
    API_GetSLPacketGenerator()->SendChangePosition(vPos, pTargetActor->GetDirection(), m_pActor->GetServerSyncAvatarType());
}
...
void CNtlBehaviorCharActiveWarp::OnWarpTarget() 
{
    ...
    RwV3d vDir = pTargetActor->GetPosition() - vPos;
    vDir.y = 0.0f;
    ...
    API_GetSLPacketGenerator()->SendChangePosition(vPos, vDir, m_pActor->GetServerSyncAvatarType());
}
...
CNtlBehaviorCharActiveThroughAttack::CNtlBehaviorCharActiveThroughAttack() 
{
    ...
    m_vDir = ZeroAxis;
    m_vPrevPos = ZeroAxis;
    m_uiAnimAction = m_uiAnimLoop = m_uiAnimEnd = 0;
    m_fRange = 0.0f;
}
...
void CNtlBehaviorCharActiveThroughAttack::Enter( void ) 
{
    ...
    m_vPrevPos = m_pActor->GetPosition();
    m_vTargetPos = pTargetActor->GetPosition();
    m_vPrevPos.y = m_vTargetPos.y = 0.0f;
    
    m_vDir = m_vTargetPos - m_vPrevPos;
    RwV3dNormalize(&m_vDir, &m_vDir);

    m_vTargetPos = m_vPrevPos;
    m_vTargetPos += m_vDir * m_fRange;
...
RwUInt32 CNtlBehaviorCharActiveThroughAttack::HandleEvents( RWS::CMsg &pMsg ) 
{
    if(pMsg.Id == g_EventAnimEnd)
    {
        ...
        else if(pAnimEnd->uiBaseAnimKey == m_uiAnimEnd)
        {
            Finish();
        }
    }
...
void CNtlBehaviorCharActiveThroughAttack::Update( RwReal fElapsed ) 
{
    ...
    RwV3d vPos = m_pActor->GetPosition();
    vPos.y = 0.0f;
    vPos += m_vDir * DBO_STRIKE_RUSH_MOVEMENT_SPEED * fElapsed;

    RwReal fInterVal = (vPos.x - m_vPrevPos.x) / (m_vTargetPos.x - m_vPrevPos.x);
    if(fInterVal >= 1.0f || fInterVal < 0.0f || 
       GetSceneManager()->GetWorldAttribute(vPos) & DBO_WORLD_ATTR_GROUP_FLAG_FORBID_ALL_MOVE)
    {
        vPos = m_vTargetPos;
        m_bStart = FALSE;
        m_pActor->GetSobProxy()->SetBaseAnimation(m_uiAnimEnd, FALSE);
        CalcHitDamage();
    }
    ...
    m_pActor->SetPosition(&vPos);
    API_GetSLPacketGenerator()->SendChangePosition(vPos, m_vDir, m_pActor->GetServerSyncAvatarType());
}

void CNtlBehaviorCharActiveThroughAttack::SetAnimation( RwUInt32 uiAction, RwUInt32 uiActionLoop, RwUInt32 uiActionEnd ) 
{
    m_uiAnimAction = uiAction;
    m_uiAnimLoop = uiActionLoop;
    m_uiAnimEnd = uiActionEnd;
}
