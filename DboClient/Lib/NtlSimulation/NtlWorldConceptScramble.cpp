...
CNtlSobActor* CNtlWorldConceptScramble::GetNearAttackableEnemy( CNtlSobActor* pActor )
{
	...
	for( RwUInt8 i = 0 ; i < NUM_SEARCH_TYPE ; ++i )
	{
		...
		for( it = pSobGroup->GetObjects().begin() ; it != pSobGroup->GetObjects().end() ; ++it )
		{
			...
			CNtlSobActor* pActor = reinterpret_cast<CNtlSobActor*>( pSobPlayer );
			if(!Logic_CanTargetted(pActor) || Logic_IsCondition(pActor, CHARCOND_FLAG_HIDING_KI))
				continue;

			v3ActorPos = pActor->GetPosition();
			RwV3dSub( &v3Sub, &v3AvatarPos, &v3ActorPos );
