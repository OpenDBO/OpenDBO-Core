...
VOID CNtlGuild::HandleEvents(RWS::CMsg &pMsg)
{
		...
		case SLGE_GUILD_INFO:
			{
				...
				memcpy(m_wszGuildName, pDBO_GUILD_INFO->wszName, sizeof(WCHAR)*NTL_MAX_SIZE_GUILD_NAME_IN_UNICODE);
		...
		case SLGE_ADD_MEMBER:
			{
				...
				memcpy(pMember->wszMemberName, pData->wszMemberName, sizeof(WCHAR)*NTL_MAX_SIZE_CHAR_NAME_UNICODE);
		...
		case SLGE_NOTICE:
			{
				...
				memcpy(m_GuildNotice.awcCharName, pEvent->pData, sizeof(WCHAR)*NTL_MAX_SIZE_CHAR_NAME_UNICODE);
	...
	else if( pMsg.Id == g_EventChangeGuildName )
	{
		...
		{
			memset(m_wszGuildName, 0, sizeof(m_wszGuildName));
			memcpy(m_wszGuildName, pEvent->pwcGuildName, sizeof(WCHAR)*NTL_MAX_SIZE_GUILD_NAME_IN_UNICODE);
