	...
	case DBO_SKILL_PASSIVE_EFFECT_SYSTEM_EFFECT_VALUE_UP:
		{
			...
			{
				m_stUpdatedData.fSkill_Effect_Value[i] = pPassiveEffectData->fSkill_Effect_Value[i];
				m_SkillTbl.fSkill_Effect_Value[i] = pPassiveEffectData->fSkill_Effect_Value[i];				
			}
...
void CNtlSobSkillAttr::RefreshPassiveEffect(void)
{
	...
	{
		for( RwInt32 i = 0 ; i < NTL_MAX_EFFECT_IN_SKILL ; ++i )
			m_SkillTbl.fSkill_Effect_Value[i] = m_stUpdatedData.fSkill_Effect_Value[i];	
	}
}