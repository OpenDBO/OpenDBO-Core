...
public:
	...
	struct sWaitInfo
	{
		sTIMEQUEST_ROOM_JOIN_INFO	Room;		///< 대기방 정보
	...
	// TMQ 예약 정보
	sTIMEQUEST_ROOM_JOIN_INFO*	GetRoom();
	eTIMEQUEST_MODE			GetWaitMode();
	sTIMEQUEST_TBLDAT*		GetTMQTable_for_RegRoom();
	eTIMEQUEST_DIFFICULTY	GetDifficult_for_RegRoom();
...
protected:
	RwUInt8					GetNearDifficult(sTIMEQUEST_ROOM_TMQ_INFO* pRoomInfo);
...
inline sTIMEQUEST_ROOM_JOIN_INFO* CNtlTMQ::GetRoom()
{
	return &m_WaitInfo.Room;
}

inline eTIMEQUEST_MODE CNtlTMQ::GetWaitMode()
{
	return (eTIMEQUEST_MODE)m_WaitInfo.Room.byTimeQuestMode;
}
...
inline eTIMEQUEST_DIFFICULTY CNtlTMQ::GetDifficult_for_RegRoom()
{
	return (eTIMEQUEST_DIFFICULTY)m_WaitInfo.Room.byDifficult;
}
