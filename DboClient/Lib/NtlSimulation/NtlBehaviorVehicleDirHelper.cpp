...
RwBool CNtlBHVehicleSpawnVehicle::Begin( void )
{
	m_bFinish = FALSE;
	m_fShowingTime = 0.5f;
	...
	if ( RwV3dNormalize( &vDir, &vDir ) > 0.0001f )
	{
		m_pVehicle->SetDirection( &vDir );
	}

	RwV3d vPos = m_pVehicle->GetPosition();
	m_pDriver->GetSobProxy()->CreatePLEffect( NTL_VID_VEHICLE_SPAWN, &vPos );
...
RwBool CNtlBHVehicleSpawnVehicle::Update( RwReal fElapseTime )
{
	m_fShowingTime -= fElapseTime;

	if ( m_fShowingTime <= 0.f )
	{
		m_bFinish = TRUE;
	}
...
void CNtlBHVehicleSpawnVehicle::End( void )
{
}

void CNtlBHVehicleSpawnVehicle::HandleEvents( RWS::CMsg& pMsg )
{
}
...
RwBool CNtlBHVehicleCharRunToVehicle::Begin( void )
{
	m_bFinish = FALSE;

	m_vCurDirverPos = m_pDriver->GetPosition(); m_vCurDirverPos.y = 0.f;

	m_vRunDir = m_pVehicle->GetPosition() - m_vCurDirverPos; m_vRunDir.y = 0.f;

	m_fTargetDist = RwV3dNormalize( &m_vRunDir, &m_vRunDir );

	RwReal fRadius = m_pDriver->GetSobProxy()->GetPLEntityDepth();
	fRadius = fRadius < m_pDriver->GetSobProxy()->GetPLEntityWidth() ? m_pDriver->GetSobProxy()->GetPLEntityWidth() : fRadius;

	if ( m_fTargetDist < fRadius )
	{
		return FALSE;
	}

	m_fTargetDist -= fRadius;

	m_fRemainDist = 0;

	m_pDriver->GetSobProxy()->SetBaseAnimation( RUN_FRONT );

	return TRUE;
}

RwBool CNtlBHVehicleCharRunToVehicle::Update( RwReal fElapseTime )
{
	m_fRemainDist += Logic_GetFrontRunSpeed( m_pDriver ) * fElapseTime;

	RwV3d vCurPos;

	if ( m_fRemainDist <= m_fTargetDist )
	{
		vCurPos = m_vCurDirverPos + (m_vRunDir * m_fRemainDist);
	}
	else
	{
		vCurPos = m_vCurDirverPos + (m_vRunDir * m_fTargetDist);

		m_bFinish = TRUE;
	}

	vCurPos.y = m_pDriver->GetPosition().y + 1.f;

	SWorldHeightStuff sHStuff;
	Logic_GetWorldHeight( &vCurPos, sHStuff );
	vCurPos.y = sHStuff.fFinialHeight;

	m_pDriver->SetPosition( &vCurPos );

	return !m_bFinish;
}

void CNtlBHVehicleCharRunToVehicle::End( void )
{
}

void CNtlBHVehicleCharRunToVehicle::HandleEvents( RWS::CMsg& pMsg )
{
}
...
RwBool CNtlBHVehicleCharRideOnVehicle::Begin( void )
{
	m_bFinish = TRUE;

	m_pDriver->SetPosition( &m_pVehicle->GetPosition() );
...
RwBool CNtlBHVehicleCharRideOnVehicle::Update( RwReal fElapseTime )
{
	return !m_bFinish;
}

void CNtlBHVehicleCharRideOnVehicle::End( void )
{
}

void CNtlBHVehicleCharRideOnVehicle::HandleEvents( RWS::CMsg& pMsg )
{
}
