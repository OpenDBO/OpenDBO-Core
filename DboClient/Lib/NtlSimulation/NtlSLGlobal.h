...
typedef struct _SAvatarInfo
{
	...
	RwUInt8				byItemCount;									// 유효한 item count	
...
private:
	...
	RwBool					m_bEnableUseNetpyStore;				///< 6번째가방( 넷피스토어 )을 사용할 수 있는가.
...
public:
	...
	void			EnableNetpyStore(void);
	RwBool			IsEnableUseNetpyStore(void);
};
...
{
	DBOTIME clientTime = time( NULL );
	m_ServerTimeOffset = serverTime - clientTime;
}

inline DBOTIME CNtlSLGlobal::GetServerTime(void)
{
	DBOTIME clientTime = time( NULL );
	return clientTime + m_ServerTimeOffset;
}

inline void CNtlSLGlobal::EnableNetpyStore(void)
{
	m_bEnableUseNetpyStore = TRUE;
}

inline RwBool CNtlSLGlobal::IsEnableUseNetpyStore(void)
{
	return m_bEnableUseNetpyStore;
}
