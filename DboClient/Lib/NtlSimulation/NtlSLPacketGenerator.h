...
public:
	...
	bool SendChangePosition(RwV3d vCurr, RwV3d vDir, RwUInt8 byAvatarType);

	bool SendChangeHeading(RwV3d vCurr, RwV3d vDir);

	bool SendCharTargetSelect(SERIAL_HANDLE hSerialId);

	bool SendCharTargetFacing(SERIAL_HANDLE hSerialId);

	bool SendCharFollowMove(SERIAL_HANDLE hTargetSerialId, RwReal fFollowRange, RwUInt8 byMovementReason, RwUInt8 byAvatarType);

	// jump
	bool SendJump(RwV3d vDir);
	...
	bool SendCharSocialAction(RwUInt32 uiTblIdx);
	bool SendCharItemUseReq(SERIAL_HANDLE hTargetSerialId, RwUInt8 byPlace, RwUInt8 byPos);
	...
	bool SendVehicle_EngineFuelRemoveNfy(RwUInt32 uiFuelItemTblidx);
	bool SendVehicle_EngineEndReq();
    ...
    // 스카우터
    bool SendScouterOnOff(RwBool bOn);    
