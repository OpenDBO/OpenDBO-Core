...
void CNtlBehaviorCharMove::UpdateAnimSpeed(void)
{
    ...
    CNtlBeCharData *pBeData = reinterpret_cast<CNtlBeCharData*>(m_pActor->GetBehaviorData());     
    if(!Logic_IsActorWalkMove(m_pActor, pBeData->GetMoveStuff()->byFormFlag))
    {
...
{
    if(byMoveFlag == NTL_MOVE_B || 
       byMoveFlag == NTL_MOVE_B_TURN_L ||
       byMoveFlag == NTL_MOVE_B_TURN_R)
    {
...
void CNtlBehaviorCharFalling::Update(RwReal fElapsed)
{
    ...
    NtlGetDestination_Jump(vDir.x, vDir.z, m_fFallingSpeed, vPos.x, vPos.y, vPos.z, m_vFallingDir.x, m_vFallingDir.z, pMoveStuff->byMoveFlags, (DWORD)(fElapsed*1000.f), 1.0f, &vHeading, &vDest);
...
{
    RwV3d vSub;
    RwV3dSub(&vSub, &vPos, &m_vFallingPos);
