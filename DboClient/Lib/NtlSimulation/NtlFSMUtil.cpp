...
RwBool FSMUtil_IsScouterUse( CNtlFSMStateBase* pStateBase ) 
{
    if(pStateBase == NULL)
        return FALSE;

    if(pStateBase->GetStateId() == NTL_FSMSID_SCOUTER)
        return TRUE;
...
RwBool FSMUtil_StandingAdjustMove(CNtlFSMStateBase* pStateBase, CNtlSobActor *pActor)
{
	...
	if(fLen > 0.6f)
	{
		...
		RwV3dAssign(&pMoveStuff->vCurrLoc, &vPos);  
		RwV3dAssign(&pMoveStuff->vDest, &vServerPos);  
