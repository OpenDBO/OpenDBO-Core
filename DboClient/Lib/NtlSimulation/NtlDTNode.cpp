...
{
	RwV3d vSub;
	RwV3dSub(&vSub, &vLookAtPos, &vStartPos);
...
void CNtlDTSplineNode::SetLookAtSelfScaleAllAxis(RwV3d vStartPos, RwV3d vEndPos, RwV3d vLookAtPos)
{
	...
	RwV3d vSub;
	RwV3dSub(&vSub, &vEndPos, &vStartPos);
	RwReal fDist = RwV3dLength(&vSub);

	RwV3dSub(&vSub, &pCurvePt[iTotCurveCnt-1].vPos, &pCurvePt[0].vPos);
...
void CNtlDTSplineNode::UniformTypeUpdate(RwReal fElapsed)
{
	...
	RwV3d vSub;
	RwV3dSub(&vSub, &pCurvePt[m_iPtIdx+1].vPos, &m_vPtPos);
	...
	RwV3dSub(&vSub, &m_vPtPos, &pCurvePt[m_iPtIdx].vPos);
	RwReal fNextLen = RwV3dLength(&vSub) + fULen;

	RwV3dSub(&vSub, &pCurvePt[m_iPtIdx+1].vPos, &pCurvePt[m_iPtIdx].vPos);
	...
	{
		RwV3dSub(&vDir, &m_vLookAtPos, &vPos);
		RwV3dNormalize(&vDir, &vDir);
	}
