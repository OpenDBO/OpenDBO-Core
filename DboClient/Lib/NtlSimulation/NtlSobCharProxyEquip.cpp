...
CNtlSobCharEquipProxy& CNtlSobCharEquipProxy::operator = (CNtlSobCharEquipProxy& EquipProxy)
{
	...
	for(RwUInt8 i = 0; i < NTL_MAX_EQUIP_ITEM_SLOT; i++)
	{
		...
		if(bCostume)
		{
			if( i != EQUIP_SLOT_TYPE_COSTUME && Logic_IsHideCostumeItem(wCostumeBitFlags, (void*)m_arrSlotItem[i].pPLItem) )
			{
		...
		if(bCostume && i == EQUIP_SLOT_TYPE_COSTUME)
		{
...
void CNtlSobCharEquipProxy::CreateEquipItem(RwUInt32 uiTblId, RwUInt8 bySlotIdx, RwUInt8 byGrade)
{
	...
	if(m_arrSlotItem[EQUIP_SLOT_TYPE_COSTUME].uiItemTblId != INVALID_TBLIDX)
		bCostume = TRUE;
	...
	if(bCostume)
	{
		sITEM_TBLDAT *pCostumeItemTblData = reinterpret_cast<sITEM_TBLDAT*>( pItemTbl->FindData(m_arrSlotItem[EQUIP_SLOT_TYPE_COSTUME].uiItemTblId) );
...
void CNtlSobCharEquipProxy::SobSubWeaponActiveEventHandler(RWS::CMsg &pMsg)
{
	...
	// 기존에 장착하고 있는 sub weapon을 remove 한다.
	...
	// 움직이는 sub weapon 이거나 혹은 active 상태에서 attach 되지 않을 경우.
	...
	// sub weapon trace를 setting 해 준다.
	if(m_arrSlotItem[bySlotIdx].pPLItem)
...
void CNtlSobCharEquipProxy::SobSubWeaponDeActiveEventHandler(RWS::CMsg &pMsg)
{
	...
	// 장착하고 있는 sub weapon을 remove 시켜준다.
