...
public:
	...
	SHitStuff& operator = (const SHitStuff& stuff)
	{
		...
		RwV3dAssign(&vPushDir, &stuff.vPushDir);
...
#define NTL_MOVETARGET_DIR			1
#define NTL_MOVETARGET_LOC			2
#define NTL_MOVETARGET_ACTOR		3
#define NTL_MOVETARGET_DASH			4
#define NTL_MOVETARGET_DASH_ACTOR	5
...
struct SMoveStuff
{
	...
	RwUInt8			byFormFlag;						// 걷기, 뛰기, 수영 등등.
	RwUInt8			byMoveFlags;					// keyboard move direction
	RwV3d			vCurrLoc;						// 이동 현재 위치.
	RwV3d			vDir;							// dash일 경우 방향 vector로 사용된다.
	RwV3d			vDest;							// mouse 및 dash 이동일 경우 최종 목적지.
	RwV3d			vSendDest;						// 두번째 이동 목적지 ( 버스에서 이용 )
	...
	RwUInt8			byMoveResult;					// 이동한 후의 결과.
...
public:
    ...
    SMoveSyncStuff* GetMoveSyncStuff() {return &m_sMoveSyncStuff;}
