...
void CNtlBehaviorCharJump::UpdatePositionMove(SMoveStuff *pMoveStuff, SJumpStuff *pJumpStuff, RwV3d& vPos, RwBool bIncHeight, RwReal fElapsed, RwReal fSpeedScale /*= 1.0f*/)
{
    ...
    if(pMoveStuff->byMoveFlags != NTL_MOVE_NONE)
    {
        ...
        // 다른 플레이어가 연속 점프할때 간혹 상태가 어긋나서 걷는 Move Flag가 오는 경우가 있다. 이것을 점프 플래그로 변경해준다.
        ...
        NtlGetDestination_Jump(vDir.x, vDir.z, fSpeed, vPos.x, vPos.y, vPos.z, pJumpStuff->vJumpDir.x, pJumpStuff->vJumpDir.z, pMoveStuff->byMoveFlags, (DWORD)(fElapsed*1000.f), 1.0f, &vHeading, &vDest);
...
void CNtlBehaviorCharJump::UpdateJumpWaterLanding(SMoveStuff *pMoveStuff, SJumpStuff *pJumpStuff, RwReal fElapsed)
{
    ...
    if(m_fJumpUpSpeed < 0.0f)
    {
        if(vPos.y > m_sHStuff.fWorldHeight)
        {
            ...
            CNtlVector vHeading, vDest;
            NtlGetDestination_Jump(vDir.x, vDir.z, fSpeed, vNextPos.x, vNextPos.y, vNextPos.z, pJumpStuff->vJumpDir.x, pJumpStuff->vJumpDir.z, pMoveStuff->byMoveFlags, (DWORD)(fElapsed*1000.f), 1.0f, &vHeading, &vDest);
...
void CNtlBehaviorCharJump::UpdateSendSyncCheck( RwReal fElapsedTime ) 
{
    ...
    {
        API_GetSLPacketGenerator()->SendChangePosition(m_pActor->GetPosition(), 
                                                       m_pActor->GetDirection(),
                                                       m_pActor->GetServerSyncAvatarType());
