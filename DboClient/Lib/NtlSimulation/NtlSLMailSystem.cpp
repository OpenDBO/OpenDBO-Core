	...
	else if (pMsg.Id == g_EventMailRead)
	{
		...
		if (pMailProfile)
		{
			if (!pMailProfile->bIsRead)
			{
				...
				if (pMailProfile->bySenderType == eMAIL_SENDER_TYPE_SYSTEM || pMailProfile->bySenderType == eMAIL_SENDER_TYPE_GM)
				{
...
RwUInt32 CNtlMailSystem::GetAbility()
{
	...
	if (Logic_ExistScouterPartsType(SCOUTER_PARTS_MAILER))
	{
		return (RwUInt32)(MAILSYSTEM_ABILITY_SEND | MAILSYSTEM_ABILITY_READ);
	}
