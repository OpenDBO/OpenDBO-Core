...
private:
	...
	RwReal m_fUpdateAngle;

	void SetAnim(RwReal fDeltaAngle);
...
public:
    ...
    void    SetRange(RwReal fRange) {m_fRange = fRange;}                                        ///< 이동 거리를 세팅한다.
    void    SetAnimation(RwUInt32 uiAction, RwUInt32 uiActionLoop, RwUInt32 uiActionEnd);       ///< 애니메이션을 세팅한다.

protected:    
    void    CalcHitDamage();                                                                    ///< 맞는 타겟의 데미지 표시

protected:    
    RwBool m_bStart;                                    ///< 발동    
    RwV3d  m_vTargetPos;                                ///< 타겟 위치    
    RwV3d  m_vDir;
    RwV3d  m_vPrevPos;
    RwUInt32 m_uiAnimAction;
    RwUInt32 m_uiAnimLoop;
    RwUInt32 m_uiAnimEnd;
    RwReal m_fRange;                              ///< 타겟 거리
};
