...
{
	RwV3d vDist;
	RwV3dSub( &vDist, &vCamPos, &vLookAt );
...
void CNtlGameCameraManager::HandleEvents(RWS::CMsg &pMsg)
{
    ...
    else if(pMsg.Id == g_EventTMQBoardNotify)
    {
        // 카메라 연출중에 TMQ가 에러가 나서 끝났을때, 연출을 해제한다.
        SNtlEventTMQBoardNotify* pData = reinterpret_cast<SNtlEventTMQBoardNotify*>(pMsg.pData);
        if(pData->uiEventType == TBT_ROOM_SERVER_EXCEPTION)
        {
            RemoveController(CAMERA_CONTROL_TIMEMACHINE);
            CheckNormalControlPause();
        }
    }
    ...
    {
        // 1인칭 카메라(스카우터)에서 키보드로 Yaw를 변경시킨다 (by agebreak)
