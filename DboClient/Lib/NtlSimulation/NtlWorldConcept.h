...
public:
	...
	RwBool						IsEnableAction(EAvatarAction eAction);						///< 현재 액션을 수행할 수 있는 상태인지 검사한다.	
	...
	RwBool						IsEnemyTargetFromAvatarActor(CNtlSobActor *pActor, SERIAL_HANDLE hTargetSerial);	///< 아바타의 타겟이 적인지 판단한다.
	CNtlSobActor*				FindNearEnemyFromAvatarActor(CNtlSobActor *pActor);									///< 가장 가까이 있는 적을 반환한다.
