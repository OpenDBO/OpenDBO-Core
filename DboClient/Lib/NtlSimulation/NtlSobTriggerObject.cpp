...
void CNtlSobTriggerObject::HandleEvents( RWS::CMsg& pMsg )
{
	...
	if(pMsg.Id == g_EventSobCreate)
	{
		...
		// 좌표와 방향 세팅.
		RwV3dAssign(&m_vPos, &pSobCreate->vLoc);
		RwV3d vDir = GetSobProxy()->GetDirection();
		RwV3dAssign(&m_vDirection, &vDir); 
		
		// 스카우터 오브젝트인 경우 스카우터 오브젝트 생성 이벤트 발생
		if ( IsScouterObject() )
		{
			CNtlSLEventGenerator::TrigObjScouterDir_CreateSTObj( GetSerialID() );
		}
	...
	{
		// 스카우터 오브젝트인 경우 스카우터 오브젝트 삭제 이벤트 발생
		if ( IsScouterObject() )
		{
			CNtlSLEventGenerator::TrigObjScouterDir_DeleteSTObj( GetSerialID() );
		}
	}
...
RwBool CNtlSobTriggerObject::CanClicked(void)
{
	...
	if ( (pAttrTObj->m_pTriggerObjTbl->wFunction & eDBO_TRIGGER_OBJECT_FUNC_SELECTION) != eDBO_TRIGGER_OBJECT_FUNC_SELECTION )
	{
		return FALSE;
	}
...
{
	return (((CNtlSobTriggerObjectAttr*)GetSobAttr())->GetTriggerObjectTbl()->wFunction & eDBO_TRIGGER_OBJECT_FUNC_SCOUTER) ? TRUE : FALSE;
}

RwBool CNtlSobTriggerObject::IsTLQObject( void )
{
	return (((CNtlSobTriggerObjectAttr*)GetSobAttr())->GetTriggerObjectTbl()->wFunction & eDBO_TRIGGER_OBJECT_FUNC_TIME_LEAP_QUEST) ? TRUE : FALSE;
}

RwBool CNtlSobTriggerObject::IsPIDObject( void )
{
	return (((CNtlSobTriggerObjectAttr*)GetSobAttr())->GetTriggerObjectTbl()->wFunction & eDBO_TRIGGER_OBJECT_FUNC_PARTY_INSTANCE_DUNGEON) ? TRUE : FALSE;
}

RwBool CNtlSobTriggerObject::IsDojoSealObject( void )
{
	return (((CNtlSobTriggerObjectAttr*)GetSobAttr())->GetTriggerObjectTbl()->wFunction & eDBO_TRIGGER_OBJECT_FUNC_DOJO_SEAL) ? TRUE : FALSE;
}
