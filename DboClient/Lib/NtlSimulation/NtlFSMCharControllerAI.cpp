...
void CNtlFSMAutoFollowCmdAI::UpdateIdle(RwReal fElapsed)
{
	...
	RwV3d vPos = m_pActor->GetPosition();
	CNtlSLEventGenerator::SobFollowMove(m_pActor->GetSerialID(), pCtrlStuff->hTargetSerialId, NTL_MOVE_FLAG_RUN, vPos, pCtrlStuff->fFollowRange, NTL_FOLLOW_CHASE, 0);
...
RwReal CNtlSummonPetAI::GetDistanceOwner(void)
{
	...
	RwV3d vDelta;
	RwV3dSub(&vDelta, &vOwnerPos, &vActorPos);
...
RwBool CNtlSummonPetAI::BattleWander(void)
{
	...
	RwV3dSub(&vOwnerDir, &vOwnerPos, &vPos);
	RwV3dSub(&vTarDir, &vTarPos, &vPos);
...
/**
* \brief Skill을 사용한다.
* \param iSlotIdx	(RwInt32) Skill의 Slot Index
* \return 성공, 실패 여부
*/
RwBool CNtlSummonPetAI::SkillAction(RwInt32 iSlotIdx)
{
	...
	return pSobSkillIcon->Use( hTarget );
}
...
#define PET_AI_PATHFIND_DIST	7.0f
	...
	RwV3dAssign(&vStart1, &vPos);
	RwV3dAssign(&vStart2, &vPos);
	...
	if(rand() % 2)
	{
		RwV3dAssign(&vLineDir, &mat.right);
...
void CNtlSummonPetAI::EventGeneratorFollowState(void)
{
	...
	RwV3d vPos = pActor->GetPosition();
	CNtlSLEventGenerator::SobFollowMove(GetActor()->GetSerialID(), pActor->GetOwnerID(), NTL_MOVE_FLAG_RUN, vPos, fFollowRange, NTL_FOLLOW_CHASE, 0);
}
...
void CNtlSummonPetAI::EventGeneratorAttackFollowState(void)
{
	...
	RwV3d vPos = pActor->GetPosition();
	CNtlSLEventGenerator::SobFollowMove(GetActor()->GetSerialID(), m_hBattleSerialId, NTL_MOVE_FLAG_RUN, vPos, fFollowRange, NTL_FOLLOW_FIGHTING, 0);
}
