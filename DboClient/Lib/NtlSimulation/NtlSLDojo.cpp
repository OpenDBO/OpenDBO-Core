...
void CNtlDojo::HandleEvents( RWS::CMsg &msg)
{
	...
	else if( msg.Id == g_EventScrambleDefaultInfo )
	{
		...
		swprintf_s(m_pScrambleTeam[SCRAMBLE_TEAM_OFFENCE]->awcGuildName, NTL_MAX_SIZE_GUILD_NAME_IN_UNICODE + 1, L"%s", pEvent->pwcOffenceGuildName);
		...
		swprintf_s(m_pScrambleTeam[SCRAMBLE_TEAM_DEFENCE]->awcGuildName, NTL_MAX_SIZE_GUILD_NAME_IN_UNICODE + 1, L"%s", pEvent->pwcDefenceGuildName);
