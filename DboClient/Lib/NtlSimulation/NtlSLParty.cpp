...
RwBool CNtlParty::Create(void)
{
	...
	LinkMsg(g_EventPartyCharmActivity);
...
void CNtlParty::Destroy(void)
{
	...
	UnLinkMsg(g_EventPartyCharmActivity);
...
VOID CNtlParty::Leave()
{
	...
	{
		m_CharmBuff[i].pITEM_TBLDAT = NULL;
		m_CharmBuff[i].pCHARM_TBLDAT = NULL;
		m_CharmBuff[i].fRemainTime = 0.f;
	}
...
VOID CNtlParty::Update(RwReal fElapsed)
{
	for(RwUInt8 i = 0 ; i < NTL_PARTY_CHARM_INVENTORY_COUNT ; ++i)
	{
		m_CharmBuff[i].fRemainTime -= fElapsed;

		if( m_CharmBuff[i].fRemainTime < 0.f )
			m_CharmBuff[i].fRemainTime = 0.f;
	}
...
VOID CNtlParty::SetPartyName(WCHAR* pcName)
{
	...
	if(pcName)
		memcpy(m_pwPartyName, pcName, sizeof(WCHAR)*NTL_MAX_SIZE_PARTY_NAME_IN_UNICODE);
}
...
const sPartyCharm& CNtlParty::GetCharmbyIndex(RwUInt8 byIndex)
{
	NTL_ASSERT(byIndex < dINVALID_CHARM_BUFF_INDEX, "CNtlParty::GetCharm, Wrong charm index");
	return m_CharmBuff[byIndex];
}

const sPartyCharm& CNtlParty::GetCharmbyTableIndex(RwUInt32 uiTableIndex)
{
	RwUInt8 byIndex = 0;
	for( ; byIndex < NTL_PARTY_CHARM_INVENTORY_COUNT ; ++byIndex)
	{
		if( m_CharmBuff[byIndex].pCHARM_TBLDAT &&
			m_CharmBuff[byIndex].pCHARM_TBLDAT->tblidx == uiTableIndex )
			return m_CharmBuff[byIndex];
	}

	return m_CharmBuff[0];
}
...
RwBool CNtlParty::IsOpenCharm()
{
	return m_bLeaderOpenCharm;
}
...
{	
	...
	else if(pMsg.Id == g_EventPartyMemberAdd)
	{
		...
		pNewMember->wCurLP		= pPacket->wCurLP;
		pNewMember->wMaxLP		= pPacket->wMaxLP;
		...
		memcpy(pNewMember->wszMemberName, pPacket->pcName, sizeof(WCHAR)*NTL_MAX_SIZE_CHAR_NAME_UNICODE);
	...
	else if(pMsg.Id == g_EventPartyUpdate)
	{
		...
		case PMT_LEADER_CHARM_OPEN:
			{
				m_bLeaderOpenCharm = TRUE;
				break;
			}
		case PMT_LEADER_CHARM_CLOSE:
			{
				m_bLeaderOpenCharm = FALSE;
				break;
			}
	...
	else if(pMsg.Id == g_EventPartyCharmActivity)
	{
		sPartyCharmActivity* pEvent = reinterpret_cast<sPartyCharmActivity*>( pMsg.pData );

		switch(pEvent->uiMessage)
		{
		case PCAT_ACTIVE:
			{				
				sITEM_TBLDAT* pITEM_TBLDAT = Logic_GetItemDataFromTable(pEvent->uiTableIndex);
				...
				if( !pCHARM_TBLDAT )
				{
					DBO_FAIL("Not exist charm table of index : " << pITEM_TBLDAT->Charm_Tblidx);
					break;
				}

				if( FALSE == Logic_IsUsableIndex(pEvent->byIndex, NTL_PARTY_CHARM_INVENTORY_COUNT) )
				{
					DBO_FAIL("Invalid index : " << pEvent->byIndex);
					break;
				}

				m_CharmBuff[pEvent->byIndex].pITEM_TBLDAT = pITEM_TBLDAT;
				m_CharmBuff[pEvent->byIndex].pCHARM_TBLDAT = pCHARM_TBLDAT;
				m_CharmBuff[pEvent->byIndex].fRemainTime = pEvent->fRemainTime / 1000.f;
		...
		case PCAT_RELEASE:
			{
				...
				m_CharmBuff[pEvent->byIndex].pITEM_TBLDAT = NULL;
				m_CharmBuff[pEvent->byIndex].pCHARM_TBLDAT = NULL;
				m_CharmBuff[pEvent->byIndex].fRemainTime = 0.f;
...
void CNtlParty::PartyShareTargetNfyHandler( RWS::CMsg& pMsg ) 
{
    ...
    // 1. 기존의 타겟중의 포함되어 있지 않은것은 마크를 없앤다. (NOTE: 이펙트를 계속 생성/해제하는것을 막기 위해서 체크한다)    
    for(int i = 0; i < NTL_MAX_SHARETARGET_COUNT; ++i)
    {
        if(m_aShareTargetInfo[i].hTarget == INVALID_SERIAL_ID)
            continue;
    ...
    for(RwUInt8 i = 0; i < NTL_MAX_SHARETARGET_COUNT; ++i)
    {
        // 2. 받은 타겟에 전부 마크를 생성한다. (이미 같은 마크가 존재하면 무시한다)
