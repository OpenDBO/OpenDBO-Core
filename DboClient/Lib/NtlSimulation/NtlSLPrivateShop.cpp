...
RwBool CNtlPrivateShop::Create()
{
	...
	m_mapEventFunc[g_EventPrivateShopBusinessImmediateItemBuyingRes.Get_pEventId()]	= &CNtlPrivateShop::HandleEvents_PrivateShopBusinessBuyingRes;
	m_mapEventFunc[g_EventPrivateShopBusinessImmediateItemBuyingNfy.Get_pEventId()] = &CNtlPrivateShop::HandleEvents_PrivateShopBusinessBuyingNfy;
		...
		{				
			if (pShopItem->sPrivateShopItemData.bIsSaveItem)
			{
				...
				CNtlSLEventGenerator::SobItemAdd(GetNtlSLGlobal()->GetSobAvatar()->GetSerialID(),
					...
					sPrivateShopItemData.sItem.aOptionTblidx[0],
					sPrivateShopItemData.sItem.aOptionTblidx[1],
					sPrivateShopItemData.sItem.byPlace,
					sPrivateShopItemData.sItem.byPosition,
					...
					sPrivateShopItemData.sItem.byRestrictType,
...
void CNtlPrivateShop::HandleEvents_PrivateShopBusinessBuyingRes( RWS::CMsg &pMsg ) 
{
	SNtlEventPrivateShopBusinessImmediateItemBuyingRes* pShopItemBuying =
		reinterpret_cast<SNtlEventPrivateShopBusinessImmediateItemBuyingRes*>( pMsg.pData );

	sPRIVATESHOP_ITEM_DATA& sPrivateShopItemData = pShopItemBuying->sPrivateShopItemData;
	if (sPrivateShopItemData.hItem != INVALID_SERIAL_ID &&
		sPrivateShopItemData.byPrivateShopInventorySlotPos != PRIVATESHOP_ITEMPOS_INVALID)
	{
		sITEM_TBLDAT* pTblData = Logic_GetItemDataFromTable(sPrivateShopItemData.sItem.itemNo);
...
void CNtlPrivateShop::HandleEvents_PrivateShopBusinessBuyingNfy( RWS::CMsg &pMsg ) 
{
	SNtlEventPrivateShopBusinessImmediateItemBuyingNfy* pShopItemBuying =
		reinterpret_cast<SNtlEventPrivateShopBusinessImmediateItemBuyingNfy*>( pMsg.pData );

	if (pShopItemBuying->sPrivateShopItemData.byPrivateShopInventorySlotPos != PRIVATESHOP_ITEMPOS_INVALID)
	{
		sPRIVATESHOP_ITEM_DATA& sPrivateShopItemData = m_aPrivateShopItemData[pShopItemBuying->sPrivateShopItemData.byPrivateShopInventorySlotPos];
		...
		SetPrivateShopItemData(pShopItemBuying->sPrivateShopItemData.byPrivateShopInventorySlotPos, NULL);
	}
}
