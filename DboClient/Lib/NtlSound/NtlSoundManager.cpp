							...
							float fDistacneFactor /* = 1.0 */, float fRollOffScale /* = 1.0 */)
{
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////
	//
	//	유저가 설정한 사운드 모드를 사운드 카드에서 지원하지 않는다면 음원이 플레이되지 않는다
	//	이런 경우에는 스테레오 모드로 플레이하도록 한다
	//
	////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////
	//
	// avooo's comment : 일부 사운드 디바이스에서의 예외처리( FMOD 추천 코드 )
...
/*
	////////////////////////////////////////////////////////////////////////////////////////////////////////
	//
	//	5.1채널 설정에서 2채널 플레이시 볼륨 이상문제 해결전까지 막자
	//	FMOD 추천 코드 : 어떠한 이유로 스피커 모드가 제대로 설정되지 않을 경우를 대비하여
	//
	////////////////////////////////////////////////////////////////////////////////////////////////////////
	result = CNtlSoundGlobal::m_pFMODSystem->init(MAX_DBO_CHANNELS, FMOD_INIT_NORMAL, NULL);
...
*/	
	////////////////////////////////////////////////////////////////////////////////////////////////////////
	//
	//	사운드 카드가 제공하는 채널 이상은 쓸 수 없지만 채널 수를 줄여서 사용할 수는 있다
	//	충분한 채널을 확보하기 위해 사운드카드에서 지원하는 채널을 제외하고 소프트웨어 채널을 선언한다
	//
	////////////////////////////////////////////////////////////////////////////////////////////////////////
	int iSoundcardMax3DChannels;	///< 사운드 카드의 최대 3D 채널
	int	iSoundcardMax2DCHannels;	///< 사운드 카드의 최대 2D 채널
	int	iSoundcardTotalChannels;	///< 사운드 카드의 최대 채널수
	...
	if( MAX_DBO_CHANNELS > iSoundcardTotalChannels )
	{
		...
		/* avooo's commnet : 함수가 제대로 동작하지 않는 사운드 카드도 있었다. -_-;; 젠장할...
		if( result != FMOD_OK )
		{
...
void CNtlSoundManager::CreateChannelGroups()
{
	...
	// NtlSoundDefines.h 의 eChannelGroupType 순서를 바꾸어도 영향을 끼치지 않도록 
	// 매번 인덱스와 비교하여 클래스를 만든다
...
int CNtlSoundManager::SetSoundPosition(SOUND_HANDLE hHandle, float fPosX, float fPosY, float fPosZ)
{
	...
	if(pSound)
	{
		// 플레이가 중지된 오브젝트 뮤직의 경우 다시 플레이를 시켜준다
...
void CNtlSoundManager::Update(float fElapsed)
{
	...
	// 각 채널의 update후에 release된 채널이 있다면 강제종료됬던 채널을 플레이한다.
...
bool CNtlSoundManager::IsValidGroupRange(int iChannelGroup, float fPosX, float fPosY, float fPosZ)
{
	...
	case CHANNEL_GROUP_WEATHER_EFFECT_SOUND:
		{
			// PLSound에서 Play를 요청할 때는 위치를 0,0,0으로 세팅하여 실제로 소리를 들을 수 없다.
			// Play요청후에 위치를 재조정하는 구조이미로 아래와 같이 검사해서는 안된다...TT
