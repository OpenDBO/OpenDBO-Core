...
// 채널 갯수 //////////////////////////////////////////////////////////////////////////
#define MAX_FMOD_CHANNELS				4093	///< FMOD에서 지원하는 최대 channel(hardware + software)
#define MAX_DBO_CHANNELS				100		///< Dbo 에서 쓰이는 채널수
#define MAX_EFFECT_CHANNELS				40		///< 최대로 플레이 될 수 있는 이펙트의 모든 채널 갯수
...
#define MAX_SOUND_DUPLICATION			3		///< 같은 이름의 사운드가 동시에 연주될 수 있는 최대 갯수

#define d2D_SOUND_MODE		(FMOD_2D | FMOD_SOFTWARE)
#define d3D_SOUND_MODE		(FMOD_3D | FMOD_SOFTWARE | FMOD_3D_LINEARROLLOFF | FMOD_3D_WORLDRELATIVE)
...
enum eBGMType
{
	BGM_TYPE_SHARE_THEME,						///< DBO 공통 테마곡
	BGM_TYPE_LOCAL_THEME,						///< 지역 테마곡
...
{
	// for 2D
	SOUND_HANDLE		hHandle;			///< 생성된 Channel의 핸들을 반환한다.
	int					iChannelGroup;		
	char*				pcFileName;
	...
	float				fYPos;
	float				fZPos;	
	float				fMinDistance;		///< 볼륨이 최대가 되는 최소 거리
	float				fMaxDistance;		///< 볼륨이 최소가 되는 최대 거리	
