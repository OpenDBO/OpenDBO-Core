	...
	if( hSelectedItem )
	{
		...
		if( dlg.DoModal() == IDOK )
		{
			if( dlg.m_strRename.Compare( GetItemText( hSelectedItem ).GetBuffer() ) != 0 )
			{
				...
				{
					AfxMessageBox( _T("현재 깊이에 같은 이름이 존재합니다."), MB_OK );
					*pResult = 0;
					return;
				}
				...
				{
					AfxMessageBox( _T("이름에 공백이 포함되어서는 안됩니다."), MB_OK );
...
{
	if( AfxMessageBox( "정말 삭제하시겠습니까?", MB_YESNO | MB_DEFBUTTON2 | MB_ICONQUESTION ) == IDNO )
		return;
