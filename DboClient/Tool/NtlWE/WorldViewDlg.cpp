...
void CWorldViewDlg::CreateWorldViewResource()
{
	...
	if (m_pRpWorld)
	{
		...
		m_pRwTexWorldTile = CNtlPLResourceManager::GetInstance()->LoadTexture("Grid1x1", "tool/saber/texture/ui/");
		DBO_ASSERT(m_pRwTexWorldTile, "Texture load failed.");

		RwTextureSetFilterMode(m_pRwTexWorldTile, rwFILTERLINEARMIPLINEAR);

		m_pRwTexWorldTileSelect = CNtlPLResourceManager::GetInstance()->LoadTexture("Grid1x1_Select", "tool/saber/texture/ui/");
		...
		m_pRwTexWorldLoading = CNtlPLResourceManager::GetInstance()->LoadTexture("Grid_OutLine", "tool/saber/texture/ui/");
...
{	
	...
	if (dGET_MOUSE()->GetLBOnce())
	{
		...
		RwV3dSub(&vRayDeltha, &rayCamera.end, &rayCamera.start);
