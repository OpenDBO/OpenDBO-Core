...
{    
	...
	{
		CString strmessage;
		strmessage.Format( _T("ERROR : Load bitmap resource [%s]"), Filename );
...
CDIB::LoadPPM ( LPSTR Filename )
{
    ...
    if ((fp = fopen ( Filename , "rb"))==NULL)
    {
	    ...
	    AfxMessageBox( _T("File open Faild !") );
        goto cleanup;
    }
...
BOOL CDIB::GaussianBlur( void )
{
	...
	if( !tAlphaImage )	
	{
        AfxMessageBox( _T("[ CDIB::GaussianBlur() ] 메모리 할당에 실패 했습니다. [ %ld Bytes]"), m_Size.cx * m_Size.cy  * sizeof( COLORREF ) );
		return FALSE;
    }
