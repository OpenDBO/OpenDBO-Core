...
protected:  
    ...
    void ApplyTMQEventProperties(const SEventTMQ* pEventTMQ);                           ///< TMQ Event에 설정된 내용으로 컨트롤들의 값을 업데이트한다.
    void SetTMQEventProperties(SEventTMQ* pEventTMQ, UINT nID);                         ///< TMQ Event의 프로퍼티를 설정한다.
