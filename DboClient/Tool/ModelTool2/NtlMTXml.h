...
public:
    ...
    BOOL CreateXML(char* szRootNodeName);                                           ///< 새로운 XML 객체를 만든다.
    bool Save(char* szFileName);                                  ///< XML을 파일로 저장한다.    
    bool LoadTreeXML(char* szFileName, SItemNode* itemNode);               ///< 트리구조의 XML파일에서 아이템 벡터를 반환한다.
    void AddFolderNode(char* szParentFolder,  char* szFolderName); ///< 새로운 폴더노드를 추가한다.
    void AddScriptNode(char* szParentFolder,  char* szScriptName); ///< 새로운 스크립트 노드를 추가한다
