...
protected:
    ...
    // Tree 내용 저장 관련
    RwBool LoadTreeXML(const WCHAR* szXmlFileName);  ///< 트리구조를 저장하고 있는 XML 파일을 로딩한다.    
    void   GetAllChildNode(HTREEITEM hItem, TREEITEM_VECTOR& vTreeItem); ///< 재귀를 돌면서 모든 자식 아이템을 찾는다.
    RwBool SaveTreeXML(const WCHAR* szXmlFileName);  ///< 트리구조를 XML 파일에 저장한다.    

    RwBool OnSetClump();                             ///< 모델의 Clump를 변경한다.
    RwBool OnSaveScript(HTREEITEM hItem, const WCHAR* szScriptName, BOOL bVisible);  ///< 스크립트를 저장한다.
