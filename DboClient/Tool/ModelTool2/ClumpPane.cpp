...
void CClumpPane::OnInitialUpdate()
{
    ...
    m_editClumpName.SetDlgFilter(L"Clump File (*.dff)|*.dff||");

    m_edEnvMap.Initialize(this, BES_XT_CHOOSEFILE);
    m_edEnvMap.SetDlgFilter(L"Texture File (*.dds) | *.dds||");
	...
	m_cbShadowMapResolution.AddString(L"64");
	m_cbShadowMapResolution.AddString(L"128");
	m_cbShadowMapResolution.AddString(L"256");
	m_cbShadowMapResolution.AddString(L"512");	
...
void CClumpPane::SetClump(CMTClump* pMTClump)
{
    ...
    if(!pMTClump)
    {
        ...
        m_editClumpName.SetWindowText(L"");
        SetEnable(FALSE);                
    }
    else
    {
        m_bFlagSetModel = TRUE;        
        m_editClumpName.SetWindowText(A2W(m_pMTClump->GetClumpName()));
        DisplayAtomics();
        SetEnable(TRUE);
    }
}
...
void CClumpPane::DisplayAtomics()
{
    ...
    {
        m_listAtomic.AddString(A2W(vAtomicNames[i]));
        m_cbEnv.AddString(A2W(vAtomicNames[i]));
    }    
...
void CClumpPane::OnEnChangeEditClumpName()
{
    ...
    else
    {
        ...
        if(strClumpFileName != "")
        {
            OnLoadClump(W2A(strClumpFileName));    
        }        
    }
}
...
void CClumpPane::OnLbnSelchangeListAtomic()
{
    ...
    if(nCount > 0)
    {
        ...
        for(int i = 0; i < nCount; i++)
        {
            ...
            m_pMTClump->CreateBB((RwChar*)(W2A(szAtomicName)));            
...
void CClumpPane::OnSetCheckButtons(WCHAR* szAtomicName)
{
    USES_CONVERSION;
    ...
    RpAtomic* pAtomic = m_pMTClump->GetAtomicByName(W2A(szAtomicName));
    if(!pAtomic)
        return;
	...
	else
	{
		...
		m_cbShadowMapResolution.EnableWindow(FALSE);
		m_cbShadowMapResolution.SelectString(0, L"0");
	}

	// Frustum Scale
	RwReal fFrustumScale = RpNtlAtomicGetUserDatReal(pAtomic);
	CString strFrustumScale;
	strFrustumScale.Format(L"%.3f", fFrustumScale);
    ...
    if(nCount > 0)
    {
        for(int i = 0; i < nTotalCount; ++i)
        {
            if(m_listAtomic.GetSel(i) <= 0)
            {
                ...
                RwBool retBool = m_pMTClump->RemoveAtomic(W2A(szAtomicName));
                if(retBool == FALSE)
                {
...
void CClumpPane::OnBnClickedBtClumpSave()
{
    ...
    if(fileDlg.DoModal() == IDOK)
    {
        ...
        if(CModelToolApplication::GetInstance()->GetAppMode() == MT_MODE_CLUMP)
        {
            ...
            m_pMTClump->Load(W2A(strClumpName));   
            DisplayAtomics();
        }        
    }  
}
...
void CClumpPane::OnBnClickedBtClumpSaveAll()
{
    ...
    CString strFileName = A2W(m_pMTClump->GetClumpName());
    strFileName = strFileName.MakeUpper();
    strFileName.Replace(L".DFF", L"_");
    ...
    for(int i = 0; i < m_listAtomic.GetCount(); ++i)
    {
        ...
        strSaveFileName += L".DFF";
        
        OnBnClickedBtDelete();
        m_pMTClump->Save(W2A(strSaveFileName));
    ...
    MessageBox(L"All Split Parts Save Success");
}
...
void CClumpPane::SetChangeAtomicFlag( ENtlAtomicFlag eFlag, RwBool bEnable ) 
{
	...
	if(nCount > 0)
	{
		...
		for(int i = 0; i < nCount; i++)
		{
			WCHAR szAtomicName[128] = {0,};			
			m_listAtomic.GetText(arrIndex[i], szAtomicName); 			
			m_pMTClump->SetAtomicFlag(eFlag, W2A(szAtomicName), bEnable);                

			// Flag에 따른 추가 작업
			switch(eFlag)
			{
			case NTL_COLLISION:			// Collision Tree를 빌드한다.
				m_pMTClump->CollisionDataBuild(W2A(szAtomicName));
				break;
			case NTL_SHADOW_MAP:		// 설정된 멀티텍스쳐의 크기를 설정한다.
				...
				{
					m_cbShadowMapResolution.SelectString(-1, L"0");
				}
			}
		}	
...
void CClumpPane::OnBnClickedCk2side()
{
    ...
    if(nCount > 0)
    {
        ...
        for(int i = 0; i < nCount; ++i)
        {
            ...
            {
                m_pMTClump->SetAtomicFlag(NTL_TOON_NOT_EDGE, W2A(strAtomicName), TRUE);
                m_ck2SideEdge.SetCheck(BST_UNCHECKED);                
                m_pMTClump->SetAtomicFlag(NTL_TOON_DETAIL_EDGE, W2A(strAtomicName), FALSE);
            }
            else
            {
                m_pMTClump->SetAtomicFlag(NTL_TOON_NOT_EDGE, W2A(strAtomicName), FALSE);
            }

			RpAtomic* pAtomic = m_pMTClump->GetAtomicByName(W2A(strAtomicName));
			if(pAtomic)
			{
...
void CClumpPane::OnBnClickedCk2sideEdge()
{
    ...
    if(nCount > 0)
    {
        ...
        for(int i = 0; i < nCount; ++i)
        {
            ...
            {
                m_pMTClump->SetAtomicFlag(NTL_TOON_DETAIL_EDGE, W2A(strAtomicName), TRUE);                
                m_ck2Side.SetCheck(BST_UNCHECKED);
                m_pMTClump->SetAtomicFlag(NTL_TOON_NOT_EDGE, W2A(strAtomicName), FALSE);                
            }
            else
...
void CClumpPane::OnCbnSelchangeCombo1()
{
	...
	if(nCount > 0)
	{
		...
		for(int i = 0; i < nCount; i++)
		{
			WCHAR szAtomicName[128] = {0,};			
			m_listAtomic.GetText(arrIndex[i], szAtomicName); 						
			RpNtlAtomicSetUserDatU16(m_pMTClump->GetAtomicByName(W2A(szAtomicName)), uiSize);
		}	
...
void CClumpPane::OnBnClickedBtSaveclump()
{
    ...
    {
        strMsg.Format(L"[%s]\n Clump Save Complete", A2W(m_pMTClump->GetClumpName()));
        MessageBox(strMsg, NULL, MB_ICONINFORMATION);
    }
    else
    {
        strMsg.Format(L"[%s]\n Clump Save Fail", A2W(m_pMTClump->GetClumpName()));
        MessageBox(strMsg, NULL, MB_ICONERROR);
    }    
}

void CClumpPane::SetShadowMapSize( WCHAR* szAtomicName ) 
{
	USES_CONVERSION;

	RwUInt32 uiSize = RpNtlAtomicGetUserDatU16(m_pMTClump->GetAtomicByName(W2A(szAtomicName)));
	if(uiSize <= 0)
	{
		// 값이 설정되어 있지 않으면 디폴트로 256을 설정한다.
		uiSize = 256;
		RpNtlAtomicSetUserDatU16(m_pMTClump->GetAtomicByName(W2A(szAtomicName)), (RwUInt16)uiSize);
	}

	CString strSize;
	strSize.Format(L"%d", uiSize);					
...
void CClumpPane::OnEnChangeEdFrustum()
{
	...
	RwReal fFrustumScale = (RwReal)_wtof(strFrustumScale);	
	int nCount = m_listAtomic.GetSelCount();
	if(nCount > 0)
	{
		int* arrIndex = new int[nCount];
		...
		for(int i = 0; i < nCount; i++)
		{
			WCHAR szAtomicName[128] = {0,};			
			m_listAtomic.GetText(arrIndex[i], szAtomicName); 						
			RpNtlAtomicSetUserDatReal(m_pMTClump->GetAtomicByName(W2A(szAtomicName)), fFrustumScale);			
		}	
		delete[] arrIndex;
	}  
...
void CClumpPane::SetEnvMap()
{
    ...
    m_cbEnv.SelectString(-1, A2W(chBuffer));
    m_cbEnv.EnableWindow(TRUE);

    RwUInt16 uiTexName = RpNtlAtomicGetEnvTexName(pAtomic);
    CString strTexName;
    strTexName.Format(L"%d", uiTexName);
    ...
    if(pCharacter)
    {
        ...
        RwChar* szEnvMap = uiTexName > 0 ? W2A(strTexName) : NULL;
...
void CClumpPane::SaveEnvMap() 
{
    ...
    RwUInt16 uiTexture = (RwUInt16)_wtoi(strEnvTex);
    if(uiTexture > 0)
    {
        // 기존에 설정된게 있으면 지워야 한다.
        RpAtomic* pAtomic = Helper_GetEnvMapAtomic(m_pMTClump->GetClump());
        ...
        RpAtomic* pAtomicNew = m_pMTClump->GetAtomicByName(W2A(strAtomicName));        
