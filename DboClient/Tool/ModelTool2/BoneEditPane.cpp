...
void CBoneEditPane::SetModel(CMTCharacter* pCharacter)
{
    ...
    else
    {
        ...
        {
            CString strBoneName = A2W(m_pCharacter->GetBoneName(i));
            if(strBoneName != "")
            {
...
void CBoneEditPane::OnCbnSelchangeCbBone()
{
    ...
    WCHAR buf[12] = {0,};
    swprintf(buf, L"%.2f", m_fBoneLength);    
    m_edBoneLength.SetWindowText(buf);

    swprintf(buf, L"%.2f", m_fBoneWidth);
    m_edBoneWidth.SetWindowText(buf);

    swprintf(buf, L"%.2f", pBoneData->vPosOffset.y);
...
void CBoneEditPane::OnChangeBoneLength(BOOL bPlus)
{
    ...
    swprintf(buf, L"%.2f", pBoneData->BoneScale[m_nSelectBoneIndex].fLength);
    m_edBoneLength.SetWindowText(buf);          
}
...
void CBoneEditPane::OnChangeBoneWidth(BOOL bPlus)
{
    ...
    swprintf(buf, L"%.2f", pBoneData->BoneScale[m_nSelectBoneIndex].fWidth);
    m_edBoneWidth.SetWindowText(buf);            
}
...
void CBoneEditPane::OnChangeSensitive(BOOL bPlus)
{
    ...
    swprintf(buf, L"%.2f", m_fSensitive);
...
void CBoneEditPane::OnEnChangeEdBoneLength2()
{
    ...
    float fBaseScale = (float)_wtof(strValue);
    m_pCharacter->SetBaseScale(fBaseScale);    
}
