...
void CItemViewPane::UpdateTree() 
{
    ...
    CString strXmlFile = A2W(CModelToolApplication::GetInstance()->GetWorkDir());
...
void CItemViewPane::OnTvnSelchangedTreeItem(NMHDR *pNMHDR, LRESULT *pResult)
{
    ...
    else
    {
        ...
        //{
            ...
            if(pItem->GetProperty()->LoadScript(W2A(strXMLFilePath)))
            {
                pItem->SetProperty(pItem->GetProperty());

                ::SetCurrentDirectoryA(CModelToolApplication::GetInstance()->GetWorkDir());
                CString strClumpFilePath = A2W(pItem->GetProperty()->GetMeshPath().c_str());
                strClumpFilePath += A2W(pItem->GetProperty()->GetMeshFileName().c_str());
                if(pItem->Create(W2A(strClumpFilePath), TRUE))
                {
                    m_treeItem.SetItemData(hItem, (DWORD_PTR)pItem);                    
        ...
        MessageBox(L"Item Script Load Fail!", L"Item View", MB_ICONERROR);
        NTL_DELETE(pItem);
    }
...
void CItemViewPane::EquipItem(HTREEITEM hItem)
{
    ...
    if(pRemoveItem && pRemoveItem != pItem)
    {
        ...
        CString strRemoveItemName = A2W(pRemoveItem->GetProperty()->GetName());
