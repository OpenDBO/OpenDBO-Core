...
void CNaviMeshPane::SetObject(CMTObject* pObject)
{
    ...
    CString strName = A2W(pObject->GetClumpInfo()->GetClumpName());
    strName = GetFileNameFromPath(strName);
    strName.Replace(L".DFF", L"");
    strName.Replace(L".dff", L"");

    CString strNaviMesh = A2W(CModelToolApplication::GetInstance()->GetWorkDir());
    strNaviMesh += MT_PATH_NAVI_MESH;
    strNaviMesh += strName + L"_PE_Nav_";

    CString strObsMesh = A2W(CModelToolApplication::GetInstance()->GetWorkDir());
    strObsMesh += MT_PATH_NAVI_MESH;
    strObsMesh += strName + L"_PE_Obs_";
    ...
    while(TRUE)
    {
        ...
        tempNavi.Format(L"%s%02d.dff", strNaviMesh, nCount);
        tempObs.Format(L"%s%02d.dff", strObsMesh, nCount);
...
void CNaviMeshPane::Render() 
{
    ...
    for(int i = 0; i < nCnt; ++i)
    {
        ...
        for each(CMTClump* pClump in m_vecClump)
        {
            if(strstr(pClump->GetClumpName(), W2A(strMeshName)))
            {
...
void CNaviMeshPane::CreateClump( CString& strFileName ) 
{
    ...
    if(!pMTClump->Load(W2A(strFileName)))
    {
        delete pMTClump;
        return;
    }
