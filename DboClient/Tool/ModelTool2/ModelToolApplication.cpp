...
RwBool CModelToolApplication::CreateWorld()
{
	...
	{
		RwChar WorldVer[10];
		fread(WorldVer,	10, 1, pFile);
		if(strcmp(dGET_WORLD_PARAM()->WorldVer, WorldVer))
		{/*
		 RwChar Msg[_MAX_PATH] = {0,};
		 sprintf_s(Msg, _MAX_PATH, "Current world[%s] is not the latest version[%s].", WorldVer, dGET_WORLD_PARAM()->WorldVer);
		 ::MessageBox(NULL, Msg, "DragonBall", MB_OK);
		 return;*/
		}
