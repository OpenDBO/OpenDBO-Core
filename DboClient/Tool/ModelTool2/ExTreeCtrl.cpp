...
void CExTreeCtrl::SaveTree(const WCHAR* szFileName)
{
    if(!szFileName)
        return;
    
    CNtlMTXml xmlSave;
    xmlSave.CreateXML((LPWSTR)(LPCWSTR)GetItemText(GetRootItem()));
    ...
    xmlSave.Save((WCHAR*)szFileName);  

}

void CExTreeCtrl::LoadTree(const WCHAR* szFileName)
{
    if(!szFileName)
        return;
    ...
    xmlLoad.LoadTreeXML((WCHAR*)szFileName, &m_itemNode);
