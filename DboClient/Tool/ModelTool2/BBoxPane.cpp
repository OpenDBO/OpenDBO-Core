...
void CBBoxPane::OnInitialUpdate()
{
    ...
    if(m_property.Create(rc, this, IDC_PROPERTY_ANIM_BBOX))
    {
        ...
        m_pCatAnimBBox = m_property.AddCategory(L"Animation BBox");        

        m_pItemWidth = (CCustomItemSpin*)m_pCatAnimBBox->AddChildItem(new CCustomItemSpin(L"Width"));
        m_pItemHeight = (CCustomItemSpin*)m_pCatAnimBBox->AddChildItem(new CCustomItemSpin(L"Height"));
        m_pItemDepth = (CCustomItemSpin*)m_pCatAnimBBox->AddChildItem(new CCustomItemSpin(L"Depth"));
