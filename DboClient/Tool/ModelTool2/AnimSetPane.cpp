...
void CAnimSetPane::SetModel(CNtlPLAttach* pModel)
{
    ...
    else
    {
        ...
        case PLENTITY_CHARACTER:
            {
                ...
                {
                    // 탈것용 Anim Set                    
                    m_cbAnimSet.AddString(L"Vehicle Animation Set");
                ...
                else
                {
                    ...
                    m_cbAnimSet.InsertString(HTB_ANIM_SET, L"HTB Animation Set");
                    m_cbAnimSet.InsertString(SOCIAL_ANIM_SET, L"Social Animation Set");
                    m_cbAnimSet.InsertString(TRIGGER_ANIM_SET, L"Trigger Animation Set");
                    m_cbAnimSet.InsertString(TRANSFORM_ANIM_SET, L"Transform Animation Set");
                    m_cbAnimSet.InsertString(VEHICLE_SRP1_ANIM_SET, L"Vehicle SRP 1");
                    m_cbAnimSet.InsertString(VEHICLE_SRP2_ANIM_SET, L"Vehicle SRP 2");
                    m_cbAnimSet.InsertString(ITEM_USE_ANIM_SET, L"Item Use Animation Set");
        ...
        case PLENTITY_ITEM:
            ...
            m_cbAnimSet.ResetContent();
            m_cbAnimSet.AddString(L"Item Animation Set");
            m_cbAnimSet.SetCurSel(0);
            break;                
		case PLENTITY_OBJECT:
			...
			m_cbAnimSet.ResetContent();
			m_cbAnimSet.AddString(L"Trigger Object Animation Set");
			m_cbAnimSet.SetCurSel(0);			
			break;
		}
    ...
    for(int i = nAnimKeyStart; i < nAnimKeyEnd; ++i)
    {
        ...
        if(sAnimName)
        {
            ...
            if(m_pAnimTable)
            {
                ...
                {
                    m_ltAnimItem.SetItem(nCount, 2, LVIF_TEXT, A2W(pAnimData->strAnimName.c_str()), 0, 0, 0, 0);
                }
            }            
            ++nCount;
        }        
...
void CAnimSetPane::OnMenuAnimSet()
{
    ...
    if(fileDlg.DoModal() == IDOK)
    {
        ...
        sLoadFilePath.Replace(strWorkPath, L".");
        sLoadFilePath.Replace(sLoadFileName, L"");
        std::string sAnimFilePath = W2A(sLoadFilePath);

        // property에 애니메이션 파일 정보를 추가한다.
        CString sAnimID = m_ltAnimItem.GetItemText(nIndex, 0);
        RwUInt32 uiAnimKey = (RwUInt32)_wtoi(sAnimID);

        std::string sAnimFileName = W2A(sLoadFileName);
...
void CAnimSetPane::SetAnimData(RwUInt32 uiKey)
{
    ...
    if(m_eAnimSetMode == ANIMSET_CHARACTER || m_eAnimSetMode == ANIMSET_VEHICLE)
    {
        ...
        {
            MessageBox(L"Animation File is Not Exist\n Check Script File (*.xml)", L"Anim Error", MB_ICONERROR);
            return;
        }
