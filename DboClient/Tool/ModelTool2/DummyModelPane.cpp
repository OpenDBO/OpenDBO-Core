...
void CDummyModelPane::OnEnChangeEditFilename()
{
    ...
    if(m_pCharacter)
    {
        m_pCharacter->LoadClump(W2A(strFileName));
    }
    else
    {
        m_pCharacter = new CMTCharacter();
        m_pCharacter->Create();
        if(m_pCharacter->LoadClump(W2A(strFileName)))
        {
            RwV3d vPos;
            vPos.x = 1.5f;
            vPos.y = 0.0f;
            ...
            strPosX.Format(L"%.2f", vPos.x);
            strPosZ.Format(L"%.2f", vPos.z);
...
void CDummyModelPane::OnEnChangeEditPosx()
{
    ...
    RwReal fPosX = (RwReal)_wtof(strValue);
...
void CDummyModelPane::OnEnChangeEditPosy()
{
    ...
    RwReal fPosZ = (RwReal)_wtof(strValue);
