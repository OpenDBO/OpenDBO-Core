...
void CTreeObjectPane::OnInitialUpdate()
{
    ...
    m_strObjectPath = A2W(CModelToolApplication::GetInstance()->GetWorkDir());
    m_strObjectPath += L"\\Object\\Mesh";    
    ...
    tvStruct.item.pszText = L"Object Resource";
    tvStruct.item.iImage  = 0;    
    m_itemResRoot = m_treeRes.InsertItem(&tvStruct);
    WCHAR* szResPath = new WCHAR[256];
    swprintf(szResPath, L"%s", m_strObjectPath);
    ...
    tvStruct.hInsertAfter = TVI_LAST;
    tvStruct.item.mask = TVIF_TEXT | TVIF_IMAGE;
    tvStruct.item.pszText = L"Object Script";
    tvStruct.item.iImage  = 0;    
...
void CTreeObjectPane::OnTvnSelchangedTreeScript(NMHDR *pNMHDR, LRESULT *pResult)
{
    ...
    else
    {
        ...
        if(pObject->GetProperty()->LoadScript(W2A(strFilePath)))
        {
           ...
           CString strFileName = A2W(pObject->GetProperty()->GetDffName());
           CString strFilePath = FindFileFromPath(strFileName, m_strObjectPath);
           if(strFilePath != "")
           {
               if(pObject->LoadClump(W2A(strFilePath)))                                             
               {
                   if(m_pObject)                   
                       m_pObject->SetVisible(FALSE);
...
void CTreeObjectPane::OnTvnSelchangedTreeResource(NMHDR *pNMHDR, LRESULT *pResult)
{
    ...
    else
    {
       ...
       if(hItemFolder && m_treeRes.GetItemData(hItemFolder))
       {
           ...
           CMTObject* pObject = new CMTObject();
           if(pObject->LoadClump(W2A(strObjectPath)))
           {
...
void CTreeObjectPane::OnObjectresourceExporttoscript()
{
    ...
    while(hItem != NULL)
    {
        ...
        strKeyName.Replace(L".DFF", L"");               ///< 확장자를 뺀이름이 keyName이 된다.
        ...
        // property를 설정한다.
        pObject->GetProperty()->SetName(W2A(strKeyName));
        pObject->GetProperty()->SetDffName(W2A(strResName));
        pObject->GetProperty()->m_strMeshPath = W2A(OBJECT_MESH_PATH);
        pObject->GetProperty()->m_strTexPath = W2A(OBJECT_TEXTURE_PATH);

        // 스크립트를 저장하고, 스크립트 트리의 루트폴더에 추가한다.
        if(pObject->GetProperty()->SaveScript(W2A(strScriptPath)))
        {
            TV_INSERTSTRUCT tvStruct;
    ...
    if(strText != "")
    {
        ...
        if(nImage == 2) // 스크립트라면 파일명도 변경하여 준다.
        {
            ...
            if(pObject)
            {
                ...
                pObject->GetProperty()->SetName(W2A(strText));                                
                CModelToolApplication::GetInstance()->SetEditObject(pObject);
...
{
    WCHAR szFilter[] = L"Object Script File (*.XML)|*.XML||";
...
void CTreeObjectPane::OnObjectscriptSaveas()
{
    ...
    if(dlg.DoModal() == IDOK)
    {
        ...
        CString strTempKeyName = A2W(pObject->GetProperty()->GetName());
        pObject->GetProperty()->SetName(W2A(strSaveFileName));
        if(pObject->GetProperty()->SaveScript(W2A(strSaveFilePath)))
        {
            // 트리에 항목을 추가한다. (파일명만 추가하면 된다)
            TV_INSERTSTRUCT sTvStruct;
        ...
        pObject->GetProperty()->SetName(W2A(strTempKeyName));
    }
}
...
RwBool CTreeObjectPane::OnSaveScript(HTREEITEM hItem)
{
    ...
    if(pObject)
    {
        ...
        pObject->GetProperty()->SetName(W2A(scriptName));
        
        if(pObject->GetProperty()->SaveScript(W2A(strFileName)))
        {
            // * 표시가 있으면 없애준다.
...
void CTreeObjectPane::OnShowSaveForChanges() 
{
    ...
    if(vChangeItem.size())
    {
        ...
        strTitle.Format(L"< Object > Save Changes to following Items?\n\n");
