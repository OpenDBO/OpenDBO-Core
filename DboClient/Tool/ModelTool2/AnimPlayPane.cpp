...
void CAnimPlayPane::SetAnimData(CMTCharacter* pCharacter, STypeAnimData* pAnimData)
{
    if(!pCharacter || !pAnimData)
    {
        ...
        m_edAnimSpeed.SetWindowText(L"");
        m_edAnimTime.SetWindowText(L"");
...
void CAnimPlayPane::SetTriggerObjectAnimData( CMTObject* pObject, STypeAnimData* pAnimData ) 
{
	...
	m_edAnimSpeed.SetWindowText(L"1.0");
...
{
    CString strAnimTime;
    strAnimTime.Format(L"%.2f/%.2f", fCurrentTime, fTotalTime);
...
void CAnimPlayPane::OnSlidePosChanged()
{
	...
	case ANIMPLAYPANE_CHARACTER:
	    ...
	    sAnimTime.Format(L"%.2f/%.2f", fTime, m_pCharacter->GetBaseDurationAnimTime());
		break;
	case ANIMPLAYPANE_OBJECT:
		m_pObject->SetBaseCurrentAnimTime(fTime);
		sAnimTime.Format(L"%.2f/%.2f", fTime, m_pObject->GetBaseDurationAnimTime());
		break;
	}
...
void CAnimPlayPane::OnSlideMarkerAdd(SSliderMarker* pSliderMarker)
{
    ...
    if(pSliderMarker->m_pAnimEvent == NULL)
    {
        ...
        case EVENT_ANIM_SKILL_CANCEL:
            ...
            {
                MessageBox(L"Skill Cancel Event Must Add After All Hit Event!");                
            }
            break;
        }
...
void CAnimPlayPane::UpdateEventComboBox() 
{
	...
	{
	case ANIMPLAYPANE_CHARACTER:
		m_cbEvent.AddString(L"Hit");
		m_cbEvent.AddString(L"Effect");
		m_cbEvent.AddString(L"Sound");
		m_cbEvent.AddString(L"FootStep");    
		m_cbEvent.AddString(L"WeightTime");
		m_cbEvent.AddString(L"TraceEvent");
		m_cbEvent.AddString(L"SubWeapon");
		m_cbEvent.AddString(L"PostEffect");
		m_cbEvent.AddString(L"SummonPet");        
        m_cbEvent.AddString(L"Alpha Fade");
        m_cbEvent.AddString(L"Direct Event");
        m_cbEvent.AddString(L"Color Change");
        m_cbEvent.AddString(L"Stretch Bone");
        m_cbEvent.AddString(L"Trigger");
        m_cbEvent.AddString(L"Skill Cancel");
	...
	case ANIMPLAYPANE_OBJECT:
		m_cbEvent.AddString(L"Effect");
		m_cbEvent.AddString(L"Sound");
        m_cbEvent.AddString(L"TMQ Event");
        m_cbEvent.AddString(L"Alpha Fade");
        m_cbEvent.AddString(L"Explosion Camera");
