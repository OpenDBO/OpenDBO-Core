...
BEGIN_MESSAGE_MAP(CPackEditorDlg, CDialog)
    ...
    ON_BN_CLICKED(IDC_BTN_TS_EXPORT, &CPackEditorDlg::OnBnClickedBtnTsExport)
...
BOOL CPackEditorDlg::OnInitDialog()
{
	...
	SetDlgItemText(IDC_STATIC_PACK_NOTIFY, L"Build not start !!!");
	m_ctrlPackNotify.EnableWindow(FALSE);

	GetNtlResourcePackManager()->LinkMakePackNotify(this, &CPackEditorDlg::MakePackNotify);

	if(m_byPackUserLevel == PACK_USER_MASTER_ALL || m_byPackUserLevel == PACK_USER_MASTER_PACK)
	{
	...
	CFileFind find;
	BOOL bFind = find.FindFile( L"devmaster.xml" );
	if(!bFind)
...
unsigned int CPackEditorDlg::PackExport(void* pParam)
{
	if(m_bUnPack)
    {
        ...
        {
            if(CUnPackSelectDlg::m_LoadPackType[i] == TRUE)
            {
	...
	if(!m_bThreadExport)
	{
		...
		if(m_nResult != NTL_FILEPACK_SUCCESS)
		{
			...
			case NTL_FILEPACK_FILEREADONLY:
				AfxMessageBox(L"File is readonly !!!");
				break;
			case NTL_FILEPACK_CREATEFAIL:
				AfxMessageBox(L"File is create fail !!!");
				break;
			case NTL_FILEPACK_NOT_USEFILE:
				AfxMessageBox(L"It can not use file format !!!");
				break;
			case NTL_FILEPACK_ERROR:
				AfxMessageBox(L"File pack is error !!!");
				break;
			}
		}
		else
...
void CPackEditorDlg::OnBnClickedBtnTsExport()
{
    ExportPack(NTL_PACK_TYPE_TS);    
}
