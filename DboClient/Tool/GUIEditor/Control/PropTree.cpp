...
void CPropTree::InitGlobalResources()
{
	...
	strcpy_s(lf.lfFaceName, LF_FACESIZE, info.lfMenuFont.lfFaceName);
#else
...
#if defined(_MSC_VER) && (_MSC_VER >= 1400)
			strcpy_s(lf.lfFaceName, LF_FACESIZE, strOfficeFontName);
#else
...
#if defined(_MSC_VER) && (_MSC_VER >= 1400)
			strcpy_s(lf.lfFaceName, LF_FACESIZE, strDefaultFontName);
#else
