...
VOID CCharSelectGui::LocateComponent(RwInt32 iWidth, RwInt32 iHeight)
{
	...
	m_pExitButton		->SetPosition(iWidth - 185, iHeight - 75);
		...
		if( pEvent->byState == CHAR_STATE_SELECT_ENTER )
		{
			...
			if( pLobby->GetCharacterCount() > 0)
			{
				...
				else if( !pLobby->IsLoadedLastCharacter() )
				{
					...
					WCHAR awcBuffer[NTL_MAX_SIZE_CHAR_NAME_UNICODE+1];
					::MultiByteToWideChar(GetACP(), 0, strFinalCharName.c_str(), -1, awcBuffer, NTL_MAX_SIZE_CHAR_NAME_UNICODE+1);
