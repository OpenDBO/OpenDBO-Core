...
#define dPRIVATESHOP_ITEM_PRICE_MAX				999999

// 개인상점 흥정 대기 시간 5초
#define dPRIVATESHOP_BUSINESS_WAIT				5.0f

// 개인상점 흥정 취소 타입
#define dPRIVATESHOP_BUSINESS_CANCEL_WAITING	1	// 흥정 대기 때 취소
#define dPRIVATESHOP_BUSINESS_CANCEL_BUSINESS	2	// 흥정 때 취소
...
public:
	...
	void			OnClickedBtnBusiness(gui::CComponent* pComponent);
	...
	void			ClickEnableBusiness(bool bClickEnable = true)	{ m_pShopItemBusiness->ClickEnable(bClickEnable); }
	void			ShowBusiness(bool bShow = true)					{ m_pShopItemBusiness->Show(bShow); }
...
protected:
	...
	gui::CButton*		m_pShopItemBusiness;		///< Item 흥정의 버튼
	...
	gui::CSlot			m_slotClickedBtnBusiness;	

	// Effects
	CSurfaceGui			m_EffectPick;				///< 픽된 상태의 그림
	CSurfaceGui			m_EffectBusiness;			///< 흥정 상태의 그림
...
public:
	...
	// HandleEvents : 개인상점 흥정 관련										( 받는 사람 )
	void	HandlePrivateShopBusinessRequestRes(void* pData);				///< 흥정 요청 손님
	void	HandlePrivateShopBusinessRequestNfy(void* pData);				///< 흥정 요청 손님 외
	void	HandlePrivateShopBusinessConsentReq(void* pData);				///< 주인
	void	HandlePrivateShopBusinessConsentNfy(void* pData);				///< 주인에게 흥정 신청한 손님
	void	HandlePrivateShopBusinessCancleRes(void* pData);				///< 취소요청자
	void	HandlePrivateShopBusinessCancleNfy(void* pData);				///< 취소요청자외의 대상
	void	HandlePrivateShopBusinessImmediateItemBuyingRes(void* pData);	///< 흥정 손님
	void	HandlePrivateShopBusinessImmediateItemBuyingNfy(void* pData);	///< 상점주인, 흥정 손님 외
	void	HandlePrivateShopBusinessWaittimeOutNfy(void* pData);			///< 흥정손님 포함한 모든 손님

	// 흥정 대기 상태
	void	BusinessWait( RwBool bWait );
	void	BusinessWaitTimeOut();			///< 흥정 대기 시간 초과
	...
	void	OnShopItemBusinessClicked(CPrivateShopItemGui* pShopItem, gui::CComponent* pComponent);
	
	// Gui Setting
	void	SetSale(bool bSale);		// 판매개시
	void	SetAway(bool bAway);		// 자리비움
	void	SetBusiness(bool bBusiness);
	...
	RwBool	GetShopBusiness() { return m_bOwnerBusiness; }
	RwUInt32 GetShopBusinessTarget() { return m_uiBusinessTarget; }

	void	Clear();
	void	ClearShopItem();
	void	CloseBusinessWindow();
	void	CloseBusinessMsgBox();
...
protected:
	...
	bool						m_bAway;			///< 자리비움
	bool						m_bSale;			///< 판매중
	bool						m_bOwnerBusiness;	///< 상점 주인이 흥정중인가?
	...
	// 흥정 대기 상태
	RwBool						m_bBusinessWait;
	RwReal						m_fBusinessWait;
	RwUInt8						m_uiBusinessSlotPos;
	RwUInt32					m_uiBusinessTarget;
