...
VOID CItemDiceGui::SetItemData(void* pItemData)
{
	...
	if( m_byWorkID == DICE_WORKD_ID_PARTY_CHARM_DICE )
	{
		sITEM_TBLDAT* pITEM_TBLDAT = (sITEM_TBLDAT*)pItemData;

		m_slot.SetSerialType(REGULAR_SLOT_ITEM_TABLE);
		m_slot.SetIcon(pITEM_TBLDAT->tblidx);
	}
	else
	{
		...
		if( m_slot.PtInRect(nX, nY) )
		{
			...
			if( m_byWorkID == DICE_WORKD_ID_PARTY_CHARM_DICE )
			{
				GetInfoWndManager()->ShowInfoWindow(TRUE, CInfoWndManager::INFOWND_TABLE_ITEM,
													...
													m_slot.GetItemTable(), DIALOG_DICE_MANAGER );
			}
			else if( m_slot.GetSerialType() == REGULAR_SLOT_ITEM_NOT_IDENTIFICATION )
			{
