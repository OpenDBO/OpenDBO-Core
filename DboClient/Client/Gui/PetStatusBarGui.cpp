...
RwBool CPetStatusBarGui::Create()
{
	...
	DBO_ASSERT( m_pBuff, "CHpGui::Create : Buff is not allocated. Not enough memory" );
...
void CPetStatusBarGui::UpdatePetInfo()
{
    ...
    sprintf_s(buf, "%u", pPetAttr->m_wLastAttackRate);
    m_pSttInfoHR->SetText(buf);

    sprintf_s(buf, "%u", pPetAttr->m_wLastDodgeRate);
    ...
    {
        sprintf_s(buf, "%u", pPetAttr->m_wLastPhyshicalOffence);    
    }
    else // 기공 공격 펫
    {
        sprintf_s(buf, "%u", pPetAttr->m_wLastEnergyOffence);    
    }
    
    m_pSttInfoPO->SetText(buf);

    sprintf_s(buf, "%u", pPetAttr->m_wLastPhyshicalDefence);
    m_pSttInfoPD->SetText(buf);

    sprintf_s(buf, "%u", pPetAttr->m_wLastEnergyDefence);
...
VOID CPetStatusBarGui::UpdatePetPowerValue()
{
	...
	RwUInt32 uiPower = Dbo_CalculatePowerLevel(
		pAttr->m_byLastStr, pAttr->m_byLastCon, pAttr->m_byLastFoc, pAttr->m_byLastDex, pAttr->m_byLastSol, pAttr->m_byLastEng,
		pAttr->m_wLastPhyshicalOffence, pAttr->m_wLastPhyshicalDefence, pAttr->m_wLastEnergyOffence, pAttr->m_wLastEnergyDefence,
		pAttr->m_wLastAttackRate, pAttr->m_wLastDodgeRate, pAttr->m_wLastCurseSuccessRate, pAttr->m_wLastCurseToleranceRate,
		pAttr->m_wLastPhysicalCriticalRate, pAttr->m_wLastEnergyCriticalRate, pAttr->m_wLastAttackSpeedRate,
...
VOID CPetStatusBarGui::OnCaptureMouseUp( const CKey& key )
{
	if( m_pThis->GetScreenRect().PtInRect( (RwInt32)key.m_fX, (RwInt32)key.m_fY ) )
	{
		if( key.m_nID == UD_LEFT_BUTTON )
		{
			...
			{
				if(m_uiPetID != GetDboGlobal()->GetTargetSerial())
				{
					...
					GetDboGlobal()->SetTargetSerial(m_uiPetID);
					CNtlSLEventGenerator::SobTargetSelect(m_uiPetID);

					API_GetSLPacketGenerator()->SendCharTargetSelect(m_uiPetID);
				}
