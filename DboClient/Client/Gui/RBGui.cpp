...
VOID CRBGui::HandleEvents( RWS::CMsg& msg )
{
	if( msg.Id == g_EventRBBattleStateUpdate )
	{
		...
		if( pData->byBattleState == RANKBATTLE_BATTLESTATE_STAGE_RUN )
		{
	...
	else if( msg.Id == g_EventRBBattlePlayerState )
	{
		...
		else
		{
			...
			DBO_WARNING( NULL, "g_EventRBBattlePlayerStaet : Invalid MemberData in State [" << eRBState << "]" );
			DBO_WARNING( NULL, "AvatarServerState = " << byServerState << " , AvatarClientState = " << byClientState );
		}
	}
