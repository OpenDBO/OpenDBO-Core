...
RwBool CPetitionGui::Create()
{
	...
	for( RwUInt8 i = 0 ; i < PETITION_CATEGORY_1_COUNT ; ++i )
		m_pCategory->AddItem( GetDisplayStringManager()->GetString(DST_PETITION_CATEGORY1_ACCUSE + i) );
	...
	m_pInput->SetMaxLength(NTL_MAX_SIZE_QUESTION_CONTENT_UNICODE);
...
VOID CPetitionGui::SetTextCategory(RwInt32 iSelect /* = INVALID_INDEX */)
{
	...
	{
		m_pCategory2->AddItem( GetDisplayStringManager()->GetString(DST_PETITION_CATEGORY2_ACCUSE_AUTOPROGRAM + vecTemp[i]) );
	}


	const WCHAR* pwcText = GetDisplayStringManager()->GetString(DST_PETITION_GREAT_CATEGORY);
	const WCHAR* pwcText2 = GetDisplayStringManager()->GetString(DST_PETITION_CATEGORY1_ACCUSE + iSelectedIndex);
...
VOID CPetitionGui::OnClicked_SendButton(gui::CComponent* pComponent)
{
	...
	if( contentSize > NTL_MAX_SIZE_QUESTION_CONTENT_UNICODE )
	{
		// 내용이 너무 깁니다
