...
RwBool CPartyMenu::Create()
{
	...
	// 파티 부적 버튼
	m_pPartyCharmButton = (gui::CButton*)GetComponent("btnPartyCharm");
	m_pPartyCharmButton->Show(false);
	//m_pPartyCharmButton->SetToolTip(GetDisplayStringManager()->GetString(DST_PARTYGUI_CHARM));
	//m_slotPartyCharmButton = m_pPartyCharmButton->SigClicked().Connect(this, &CPartyMenu::OnClick_PartyCharmButton);
...
VOID CPartyMenu::Clear()
{
	...
	for each( CPartyBuffGui* pPartyBuff in m_listPartyBuff )
	{
		pPartyBuff->Destroy();
		NTL_DELETE(pPartyBuff);
	}
	m_listPartyBuff.clear();
}
...
VOID CPartyMenu::DisplayBuff(RwUInt8 byBuffIndex, RwUInt32 uiItemIndex)
{
	CPartyBuffGui* pPartyBuff = NTL_NEW CPartyBuffGui("PartyBuff");
	if( !pPartyBuff->Create(byBuffIndex, uiItemIndex) )
	{
		pPartyBuff->Destroy();
		NTL_DELETE(pPartyBuff);
		return;
	...
	if( !IsShow() )
		pPartyBuff->Switch(false);

	m_listPartyBuff.push_back(pPartyBuff);
	RelocateBuff();
}

VOID CPartyMenu::UnDisplayBuff(RwUInt8 byBuffIndex)
{
	PARTYBUFF_ITER it_PartyBuff = m_listPartyBuff.begin();
	for( ; it_PartyBuff != m_listPartyBuff.end() ; ++it_PartyBuff )
	{
		CPartyBuffGui* pPartyBuff = *it_PartyBuff;

		if( byBuffIndex == pPartyBuff->GetBuffIndex() )
		{
			pPartyBuff->Destroy();
			NTL_DELETE(pPartyBuff);
			m_listPartyBuff.erase(it_PartyBuff);
			RelocateBuff();
			break;
		}
	}
...
VOID CPartyMenu::RelocateBuff()
{	
	CRectangle rtScreen = m_pThis->GetScreenRect();
	RwInt32 iX = rtScreen.right + dBUFF_GAP;

	for each( CPartyBuffGui* pPartyBuff in m_listPartyBuff )
	{
		pPartyBuff->SetPosition(iX, rtScreen.top);
		iX += pPartyBuff->GetWidth() + dBUFF_GAP;
	}
}
...
VOID CPartyMenu::SwitchCharmBuff(bool bOpen)
{
	for each( CPartyBuffGui* pPartyBuff in m_listPartyBuff )
		pPartyBuff->Switch(bOpen);
}
...
VOID CPartyMenu::MemberButton_UpdateTooltip()
{
	...
	{
		if( WORLD_PLAY_DOJO_SCRAMBLE == pWorldConcept->GetConceptType() )
		{
			bEnableButton = TRUE;
		}
		else if( WORLD_STATE_EXIT == pWorldConcept->GetState() ||
				 WORLD_STATE_NONE == pWorldConcept->GetState() )
		{
			bEnableButton = TRUE;
		}
	}
	...
	else
	{
		...
		RwUInt32 uiStringIndex = Logic_GetWarnningWorldConceptActionMessageIndex(WORLD_CONCEPT_FIRST_GRADE);
		if( uiStringIndex != 0xffffffff )
			m_pMemberMenuButton->SetToolTip( GetDisplayStringManager()->GetString(uiStringIndex) );
	}	
}
...
VOID CPartyMenu::OnClick_PartyCharmButton(gui::CComponent* pComponent)
{
	CNtlParty* pParty = GetNtlSLGlobal()->GetSobAvatar()->GetParty();
	...
	for(RwUInt8 i = 0 ; i < NTL_PARTY_CHARM_INVENTORY_COUNT ; ++i )
	{
		...
		if( !rPartyCharm.pCHARM_TBLDAT )
		{
			GetDboGlobal()->GetGamePacketGenerator()->SendPartyOpenCharmSlot(i);
			break;
		}
	}
}
...
RwInt32 CPartyMenu::SwitchDialog(bool bOpen)
{
	if( bOpen )
	{
		...
		SwitchCharmBuff(true);
	}	
	else
	{
		Show(false);
		SwitchMemberGui(false);
		SwitchCharmBuff(false);
		CheckPopup();
	}
...
VOID CPartyMenu::HandleEvents( RWS::CMsg &msg )
{
	...
	else if(msg.Id == g_EventCharm)
	{
		...
		case PCET_DISPLAY_CHARM_BUFF:
			{
				DisplayBuff((RwUInt8)pPacket->iValue, pPacket->uiValue2);
				break;
			}
		case PCET_UNDISPLAY_CHARM_BUFF:
			{
				UnDisplayBuff((RwUInt8)pPacket->iValue);
				break;
			}
		}
	}
