...
* \brief Constructions
*/
CCautionNotifyContinuance::CCautionNotifyContinuance( CCautionNotifyGui* pNotify, RwUInt32 uiStringID, RwReal fLimitTime, RwReal fIntervalTime ) 
{
...
int CCautionNotifyContinuance::OnIntervalTimer( RwReal fIntervalTime ) 
{
	...
	WCHAR awcBuffer[256];
	swprintf_s( awcBuffer, 256, GetDisplayStringManager()->GetString( m_uiStringID ), (RwInt32)m_fRemainTime );
	m_pNotifyGui->ShowCautionNotify( TRUE, m_uiStringID, awcBuffer );
...
VOID CCautionNotifyGui::HandleEvents( RWS::CMsg& msg ) 
{
	if( msg.Id == g_EventTimeFixedNotify )
	{
		...
		HANDLEMAP::iterator it = m_mapNotifyHandler.find( pNotify->uiStringID );
		if( it != m_mapNotifyHandler.end() )
		{
			(this->*((*it).second))(pNotify->bActive, pNotify->uiStringID);
			return;
		}

		ShowCautionNotify( pNotify->bActive, pNotify->uiStringID, pNotify->pString );
...
*/
VOID CCautionNotifyGui::SetCurrentCautionNotify( RwUInt32 uiStringID, const WCHAR* pwcString ) 
{
	m_fCurrentTime = 0.0f;
	m_fFadeOutTime = 0.0f;
...
*/
VOID CCautionNotifyGui::ShowCautionNotify( RwBool bActive, RwUInt32 uiStringID, const WCHAR* pwcString ) 
{
	if( bActive )
	{
		// 이미 출력하고 있는 CautionNotify가 없다면 바로 출력한다.
		if( m_bFinish )
			SetCurrentCautionNotify( uiStringID, pwcString );
		else
		{
			// 출력하고 있는 CautionNotify가 있다면 SideIcon으로 현재의 출력되고 있는 CautionNotify를 보내버리고
			// 새로운 CautionNotify를 등록한다.
			CDboEventGenerator::CautionSideNotify( TRUE, uiStringID, m_pStbOut->GetText().c_str(), m_fLifeTime-m_fCurrentTime);
...
*/
VOID CCautionNotifyGui::FreePvpOutSideHandler( RwBool bActive, RwUInt32 uiStringID) 
{
	// bActive 가 FALSE면 제거한다.
	if( bActive )
	{
		if( !m_listContinuance.empty() )
			DeleteContinuance( uiStringID );
	
		// 등록
		AddContinuance( uiStringID, dFREEPVP_OUTSIDE_STAGE_LIMIT_TIME, dDEFAULT_NOTIFY_TIMER_INVERVAL_TIME );
		...
		ShowCautionNotify( TRUE, uiStringID, awcBuffer );
...
*/
VOID CCautionNotifyGui::AddContinuance( RwUInt32 uiStringID, RwReal fLimitTime, RwReal fIntervalTime ) 
{
	CCautionNotifyContinuance* pContinuance = NTL_NEW CCautionNotifyContinuance( this, uiStringID, fLimitTime, fIntervalTime );
...
* \param uiStringID		(RwUInt32) 문자열 인덱스
*/
VOID CCautionNotifyGui::DeleteContinuance( RwUInt32 uiStringID ) 
{	
	CONTINUANCELIST::iterator it = m_listContinuance.begin();
	while( it != m_listContinuance.end() )
	{
