...
// NPCBalloon 
// 1. PC 반경 30m
// 2. NPC 혹은 Monster 머리위 표시.
// 3. NPC 클릭, 혹은 서버에의해 생성됨.
// 4. 감정에 따라 이미지 바뀜(미적용)
...
: CBalloonGui( pOwnerObject, fLifeTime, posOffsetDelta, eType ), m_bTimeProc( bTimeProc ), m_byBalloonType( byBalloonType ), m_wstrString( pMessage )
{
	...
	RwInt32 nLineCount = GetBalloonManager()->GetTestText()->GetLineCount();
...
VOID CBalloonManager::AddAvatarBalloon( const WCHAR* pMessage )
{
	...
	if( !GetNtlSLGlobal()->GetSobAvatar()->IsVisible() )
		return;

	if( AlreadyExist( GetNtlSLGlobal()->GetSobAvatar() ) )
	{
		RwBool	bNeedToCreate = TRUE;
		CBalloonGui* pDeleteBalloon = NULL;

		MMAP_OBJ_IT_PAIR objitpair = m_mmapObjBalloon.equal_range( GetNtlSLGlobal()->GetSobAvatar() );
		...
		if( bNeedToCreate )
		{
			...
			pBalloon = NTL_NEW CChatBalloonGui( GetNtlSLGlobal()->GetSobAvatar(), posOffsetDelta, pMessage, CBalloonGui::TYPE_AVATAR_CHAT );
...
VOID CBalloonManager::AddPlayerBalloon( CNtlSob* pSobObject, const WCHAR* pMessage )
{
	...
	if( AlreadyExist( pSobObject ) )
	{
		...
		if( bNeedToCreate )
		{
			...
			{
				pBalloon = NTL_NEW CChatBalloonGui( pSobObject, posOffsetDelta, pMessage, CBalloonGui::TYPE_PLAYER_CHAT );
				AddBalloonData( pBalloon );
			}
		}
	}
			...
			else if( pObject->GetClassID() == SLCLASS_MONSTER )
			{
				if( pDialog->bIsRefSpeechTable )
				{
					...
					{		
						...
						AddNPCBalloon( pObject, wBuf, pSpeechData->dwDisplayTime / 1000.0f, TRUE, TRUE, 0 );
					}
				}
				else
				{
