...
RwBool CGuildEmblemMakerGui::Create()
{
	...
	m_mapStuffTypeC = (MAP_STUFF*)GetEmblemMaker()->GetStuffList(EMBLEMTYPE_C);
	...
	if( m_mapStuffTypeC->size() > 0 )
		SetTextureStuff(EMBLEMTYPE_C, m_mapStuffTypeC->begin()->second);
	...
	m_slotLeftCButton	= m_pLeftCButton->SigClicked().Connect(this, &CGuildEmblemMakerGui::OnClicked_LeftCButton);
	...
	m_slotRightCButton	= m_pRightCButton->SigClicked().Connect(this, &CGuildEmblemMakerGui::OnClicked_RightCButton);
...
VOID CGuildEmblemMakerGui::UpdateBeforeCamera(RwReal fElapsed)
{
	...
	if( pEmblemTexture )
	{
		m_strEmblemTextureName = RwTextureGetName(pEmblemTexture);
...
VOID CGuildEmblemMakerGui::SetTextureStuff(RwInt8 byStuffType, RwUInt32 uiStuffTextureIndex)
{
	...
	case EMBLEMTYPE_C:		pmapStuffType = m_mapStuffTypeC;		break;
	default:
		{
...
VOID CGuildEmblemMakerGui::OnClicked_LeftCButton(gui::CComponent* pComponent)
{
	MAP_STUFF_ITER it = m_mapStuffTypeC->find( m_StuffSlot[EMBLEMTYPE_C][0].byStuffTextureIndex );

	if( it == m_mapStuffTypeC->end() )
		return;

	if( it != m_mapStuffTypeC->begin() )
	{
		--it;
	}
	else if( it == m_mapStuffTypeC->begin() )
	{
		it = m_mapStuffTypeC->end();
		--it;
	}

	SetTextureStuff(EMBLEMTYPE_C, it->second);
}
...
VOID CGuildEmblemMakerGui::OnClicked_RightCButton(gui::CComponent* pComponent)
{
	MAP_STUFF_ITER it = m_mapStuffTypeC->find( m_StuffSlot[EMBLEMTYPE_C][1].byStuffTextureIndex );
...
VOID CGuildEmblemMakerGui::OnMouseUp(const CKey& key)
{
	...
	{
		// 재료 텍스처를 선택했다
		m_byCurStuffRow	= m_MouseInfo.bySlotType;
		m_bUpdateEmblem	= TRUE;		
		...
		default: DBO_FAIL("Ivalid emblem stuff type : " << m_MouseInfo.bySlotType);
		}
	}
	else if( m_MouseInfo.bySlotType == dSLOTSTUFF_TYPE_PALETTE )
	{
		if( m_MouseInfo.bySlotIndex == PtInRectPalette((RwInt32)key.m_fX, (RwInt32)key.m_fY) )
		{
			if( m_byCurStuffRow != INVALID_BYTE )
			{
				...
				default: DBO_FAIL("dSLOTSTUFF_TYPE_PALETTE == m_MouseInfo.bySlotType, Invalid stuff row : " << m_byCurStuffRow);
				}
