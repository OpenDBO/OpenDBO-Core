...
VOID CRBBoardItem::SetItemData( RwUInt32 uiFieldID, const WCHAR* pName, RwUInt32 uiAppliedCount )
{
...
RwBool CRBBoardGui::Create(VOID)
{
	...
	if( !CNtlPLGui::Create( "", "gui\\RBBoard.srf", "gui\\RBBoard.frm" ) )
		NTL_RETURN( FALSE );
	...
	m_pstbTitle = reinterpret_cast<gui::CStaticBox*>( GetComponent( "stbBattleApplyTitle" ) );
	m_pstbField	= reinterpret_cast<gui::CStaticBox*>( GetComponent( "stbBattleFieldTitle" ) );
	...
	m_pbtnChallenge = reinterpret_cast<gui::CButton*>( GetComponent( "btnApply" ) );
	...
	m_slotClickChallenge = m_pbtnChallenge->SigClicked().Connect( this, &CRBBoardGui::OnClickChallenge );
...
{
	m_pstbTitle->SetText( GetDisplayStringManager()->GetString( DST_RB_TITLE ) );
	m_pstbField->SetText( GetDisplayStringManager()->GetString( DST_RB_FIELD ) );
	...
	m_pbtnChallenge->SetText( GetDisplayStringManager()->GetString( DST_RB_BTN_APPLY ) );
...
VOID CRBBoardGui::SetBoardItem( RwInt32 nOffset )
{
	...
	for( i = 0 ; i < MAX_BOARD_ITEM && iter != m_vecBoardData.end() ; ++iter, ++i )
	{
		...
		m_BoardItem[i].SetItemData( data->_uiFieldID, data->_wstrFieldName.c_str(), data->_uiAppliedCount );
		m_BoardItem[i].Show( true );
...
VOID CRBBoardGui::StateApplySetting(VOID)
{
	...
	if( uiState == CNtlRankBattle::RB_REQUEST_STATE_NONE )
	{
		m_pbtnChallenge->Show( true );
		m_pbtnCancel->Show( false );				
	}
	else
	{
		m_pbtnChallenge->Show( false );
...
VOID CRBBoardGui::EnableButtons( bool bEnable )
{
	...
	m_pbtnChallenge->Enable( bEnable );
...
{
	GetDboGlobal()->GetGamePacketGenerator()->SendRBInfoReq( RANKBATTLE_MODE_INDIVIDUAL );
	StateBattleTypeSetting( RANKBATTLE_MODE_INDIVIDUAL );	
	EnableButtons( false );		
...
VOID CRBBoardGui::OnClickParty( gui::CComponent* pComponent )
{
	...
	StateBattleTypeSetting( RANKBATTLE_MODE_PARTY );
	EnableButtons( false );		
...
{
	if( CNtlGameRuleConcept::GetInstance()->CanTransition( GAME_RULE_TYPE_MAIN_RANKBATTLE_READY ) == FALSE )
		return;
...
VOID CRBBoardGui::OnClickCancel( gui::CComponent* pComponent )
{
	...
	{
		// 이미 버튼 활성화로 State를 잡아놓았기때문에 굳이 검사할 필요 없음.
