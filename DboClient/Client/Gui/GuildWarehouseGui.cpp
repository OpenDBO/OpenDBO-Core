...
#define dHEIGHT_GAP			30		// 제니를 저장할 수 있는 창고와 일반 창고의 높이 차이
};
...
RwBool CGuildWarehouseGui::Create()
{
	...
	if(!CNtlPLGui::Create("", "", "gui\\GuildWarehouse.frm"))
		NTL_RETURN(FALSE);
		...
		if( m_Slot[iPtinSlot].GetSerial() != INVALID_SERIAL_ID )
		{
			...
			else
			{
				if( m_byInfoWindowIndex != iPtinSlot )
				{
					...
					for(RwUInt8 i = 0 ; i < NTL_MAX_OPTION_IN_ITEM ; ++i)
						item_Profile.aOptionTblidx[i] = pSlotData->aOptionTblidx[i];
...
RwBool CGuildWarehouseAddonGui::Create()
{
	...
	if(!CNtlPLGui::Create("", "", "gui\\GuildWarehouseAddon.frm"))
		NTL_RETURN(FALSE);
	...
	m_srfZennySlotDestination.SetSurface( GetNtlGuiManager()->GetSurfaceManager()->GetSurface( "Inventory.srf", "srfMoneyBtnWayFocus" ) );
