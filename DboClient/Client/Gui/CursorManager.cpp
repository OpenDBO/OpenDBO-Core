...
VOID CCursorManager::Update(RwReal fElapsed, sWorldPickInfo_for_Cursor* pWorldPickInfo_for_Cursor)
{
	...
	else
	{
		...
		if( GetDialogManager() )
		{
			...
			if( DIALOGMODE_ITEM_REPAIR			== eMode		||
				DIALOGMODE_ITEM_IDENTIFICATION	== eMode		)
				return;
		}
...
void CCursorManager::WorldCursorProc(CNtlSob* pSobObj)
{
	...
	{
		if( GetDboGlobal()->GetTargetSerial() == pSobObj->GetSerialID() )
			SetMouseCursor( CS_BATTLE_ATTACK );			// 타겟 어택시 Sound는  TargetHpGui 에서
		else
		{
			SetMouseCursor( CS_BATTLE_TARGET );
		}
	}
