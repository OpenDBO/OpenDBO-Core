...
RwBool CRBResultGui::SetResultData( VOID* pData )
{
	...
	// Result : SetFlashResult에서는 대전자인 경우만 연출함.
	...
	// Battle Record : peessi : 파티시는 전적표시안함. 현재는 파티,개인전 모두 전적 표시 안함. 서버 작업 완료후 진행.
	//if( pRankBattle->IsPartyBattle() )
	//{
	//	if( pRankBattle->IsRedTeamWin() )
	//	{
	//		m_pstbInfo1[RED]->Format( GetDisplayStringManager()->GetString( DST_RB_RESULT_RECORD_INFO_2 ),
	//								  pFinishInfo->sWinerRecord.wStraightWinCount, pFinishInfo->sWinerRecord.wStraightKOWinCount );
	//	}
	//	else if( pRankBattle->IsBlueTeamWin() )
	//	{
	//		m_pstbInfo1[BLUE]->Format( GetDisplayStringManager()->GetString( DST_RB_RESULT_RECORD_INFO_2 ),
	//								   pFinishInfo->sWinerRecord.wStraightWinCount, pFinishInfo->sWinerRecord.wStraightKOWinCount );
	//	}
	//}
	//else
	//{
	//	m_pstbInfo1[BLUE]->Format( GetDisplayStringManager()->GetString( DST_RB_RESULT_RECORD_INFO_1 ), 
	//							   pFinishInfo->sOwnerInfo.wWinCount + pFinishInfo->sOwnerInfo.wLoseCount + pFinishInfo->sOwnerInfo.wDrawCount,
	//							   pFinishInfo->sOwnerInfo.wWinCount, pFinishInfo->sOwnerInfo.wLoseCount );
	//	m_pstbInfo1[RED]->Format( GetDisplayStringManager()->GetString( DST_RB_RESULT_RECORD_INFO_1 ), 
	//							  pFinishInfo->sChallengerInfo.wWinCount + pFinishInfo->sChallengerInfo.wLoseCount + pFinishInfo->sChallengerInfo.wDrawCount,
	//							  pFinishInfo->sChallengerInfo.wWinCount, pFinishInfo->sChallengerInfo.wLoseCount );

	//	if( pRankBattle->IsRedTeamWin() )
	//	{
	//		m_pstbInfo2[RED]->Format( GetDisplayStringManager()->GetString( DST_RB_RESULT_RECORD_INFO_2 ),
	//								  pFinishInfo->sWinerRecord.wStraightWinCount, pFinishInfo->sWinerRecord.wStraightKOWinCount );
	//	}
	//	else if( pRankBattle->IsBlueTeamWin() )
	//	{
	//		m_pstbInfo2[BLUE]->Format( GetDisplayStringManager()->GetString( DST_RB_RESULT_RECORD_INFO_2 ),
	//								   pFinishInfo->sWinerRecord.wStraightWinCount, pFinishInfo->sWinerRecord.wStraightKOWinCount );
	//	}
	//}

	// Score
	m_numResult[RED].SetNumber( pFinishInfo->sChallengerInfo.byScore );
	m_numResult[BLUE].SetNumber( pFinishInfo->sOwnerInfo.byScore );
	m_numScore[RED].SetNumber( (RwInt32)pFinishInfo->sChallengerInfo.fPoint );
	m_numScore[BLUE].SetNumber( (RwInt32)pFinishInfo->sOwnerInfo.fPoint );
	m_numBonus[RED].SetNumber( (RwInt32)pFinishInfo->sChallengerInfo.fBonusPoint );
	m_numBonus[BLUE].SetNumber( (RwInt32)pFinishInfo->sOwnerInfo.fBonusPoint );
