...
VOID CRegularSlotGui::SetCount(RwUInt32 uiCount)
{
	...
	if( NTL_MAX_ITEM_STACK_COUNT < uiCount )
		return;	
	...
	if( m_uiStackCount >= 1 )
	{
		...
		if( ( pITEM_TBLDAT && pITEM_TBLDAT->byMax_Stack > 1 ) ||
			IsOnlyUsableIcon() )
		{
			m_pCount->SetText(m_uiStackCount);
			m_pCount->Show(true);
...
VOID CRegularSlotGui::SetPrice(unsigned int uiPrice)
{
	m_uiPrice = uiPrice;
}
