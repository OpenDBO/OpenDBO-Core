...
CDropItemInfoGui::CDropItemInfoGui(RwUInt32 init_m_uiDropItemSerial, const RwChar* pName)
	: CNtlPLGui(pName)

{
	Init(init_m_uiDropItemSerial, TRUE);
}

CDropItemInfoGui::CDropItemInfoGui(const RwChar* pName)
	: CNtlPLGui(pName)
{
	Init(INVALID_SERIAL_ID, FALSE);
}
...
VOID CDropItemInfoGui::Init(RwUInt32 init_m_uiDropItemSerial, RwBool init_m_pDropItemShow)
{
	m_fTime = 0.0f;
	m_eState = HIDE;
...
RwBool CDropItemInfoGui::Create()
{
	...
	if (m_pDropItemShow) {
		CNtlSob* pObj = GetNtlSobManager()->GetSobObject(m_uiDropItemSerial);
		m_pDropItem = reinterpret_cast<CNtlSobWorldItem*>(pObj);
		ShowItemInCapsule();
	}
	else {
		SetState(HIDE);
...
VOID CDropItemInfoGui::ShowItemInCapsule() {
	...
	{
	case ITEM_RANK_NORMAL:		m_pThis->SetColor(255, 255, 255); break;
	case ITEM_RANK_SUPERIOR:	m_pThis->SetColor(106, 136, 167); break;
	case ITEM_RANK_EXCELLENT:	m_pThis->SetColor(180, 154, 53); break;
	case ITEM_RANK_RARE:		m_pThis->SetColor(173, 76, 89); break;
	case ITEM_RANK_LEGENDARY:   m_pThis->SetColor(163, 85, 148); break;
	}
...
VOID CDropItemInfoGui::FadeEffect(VOID)
{
	...
	{
		if (m_fTime > DROPITEM_SHOW_TIME && !m_pDropItemShow)
			SetState(FADE_OUT);
	}
