...
VOID CDropItemInfoGui::PositionUpdate(VOID)
{
	...
	if( !GetGuiFuntor()->Calc3DPosTo2D(vDropItemPos, 1.2f, &posOffset, &posRes ) )
	{
		Show( false );
		return;
	}
...
VOID CDropItemInfoGui::OnMouseEnter( gui::CComponent* pComponent )
{
	...
	if( pAttr->IsItem() )
	{
		if( pAttr->IsIdentified() )
		{
			...
			sItemProfile.byRestrictType = ITEM_RESTRICT_TYPE_NONE;
			memset( sItemProfile.awchMaker, 0, NTL_MAX_SIZE_CHAR_NAME_UNICODE + 1 );

			sItemProfile.aOptionTblidx[0] = pAttr->GetOptionTblIdx1();
			sItemProfile.aOptionTblidx[1] = pAttr->GetOptionTblIdx2();
	...
	{
		WCHAR wBuf[256];
		swprintf_s( wBuf, 256, GetDisplayStringManager()->GetString( DST_DROPITEM_ZENNY ), pAttr->GetMoney() );
