...
void CTBNewsGui::HandleEvents( RWS::CMsg& msg )
{
	...
	else if( msg.Id == g_EventBudokaiPrizeWinnerName )
	{
		...
		WCHAR awcWinnerName[NTL_MAX_SIZE_CHAR_NAME_UNICODE+1];
		WCHAR awcSecondWinnerName[NTL_MAX_SIZE_CHAR_NAME_UNICODE+1];

		::ZeroMemory( awcWinnerName, sizeof(WCHAR) * NTL_MAX_SIZE_CHAR_NAME_UNICODE+1 );
		::ZeroMemory( awcSecondWinnerName, sizeof(WCHAR) * NTL_MAX_SIZE_CHAR_NAME_UNICODE+1 );
...
void CTBNewsGui::SetPrizeText( eNoticeType eNitType, RwUInt8 byMatchType, gui::CStaticBox* pComponent, const WCHAR* awcWinner, const WCHAR* awcSecondWinner )
{
	...
	{
		RwUInt32 uiStrPrizeWinner = DST_BUDOKAI_NEWS_NOTICE_PRIZE_WINNER;
		RwUInt32 uiStrPrizeSecondWinner = DST_BUDOKAI_NEWS_NOTICE_PRIZE_SECOND_WINNER;
		RwUInt32 uiStrNotPrize = DST_BUDOKAI_NEWS_NOTICE_NOT_PRIZE;
	...
	else if( eNitType == NIT_STATE_END )
	{
		...
		RwUInt32 uiStrPrizeSecondWinner = DST_BUDOKAI_NEWS_NOTICE_PRIZE_SECOND_WINNER;
		RwUInt32 uiStrNotPrize = DST_BUDOKAI_NEWS_NOTICE_NOT_PRIZE;
		RwUInt32 uiMatchType = DST_BUDOKAI_NOTICE_SOLO;
