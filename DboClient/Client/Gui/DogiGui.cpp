...
VOID CDogiGui::RegisterDogiItem( TBLIDX tblIdx, RwUInt8 byGrade )
{
	...
	if ( pCharProxy )
	{
		SEquipItem* pEquipItem = pCharProxy->GetUIDogiCharacter()->pUIEquipProxy->GetEquipItem( EQUIP_SLOT_TYPE_COSTUME );
		...
		if ( pEquipItem )
		{
			...
			pCharProxy->GetUIDogiCharacter()->pUIEquipProxy->DeleteEquipItem( EQUIP_SLOT_TYPE_COSTUME );
			pCharProxy->GetUIDogiCharacter()->pUIEquipProxy->CreateEquipItem( tblIdx, EQUIP_SLOT_TYPE_COSTUME, byGrade );
...
VOID CDogiGui::UnregisterDogiIem( void )
{
	...
	if ( pCharProxy )
	{
		...
		if ( pEquipItem && m_tblDogiItem != pEquipItem->uiItemTblId )
		{
			...
			pCharProxy->GetUIDogiCharacter()->pUIEquipProxy->DeleteEquipItem( EQUIP_SLOT_TYPE_COSTUME );
			pCharProxy->GetUIDogiCharacter()->pUIEquipProxy->CreateEquipItem( m_tblDogiItem, EQUIP_SLOT_TYPE_COSTUME, m_byDogiGrade );
