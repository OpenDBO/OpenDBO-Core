...
RwBool CDiceManager::CreateInstance()
{
	...
	// 파티 참 주사위
	CDiceGui* pPartyCharmDiceGui = NTL_NEW CItemDiceGui("PartyCharmDiceGui", DICE_WORKD_ID_PARTY_CHARM_DICE);
	if( !pPartyCharmDiceGui->Create(dDICE_FUNCTION_NONE) )
	{
		pPartyCharmDiceGui->Destroy();
		NTL_DELETE(pPartyCharmDiceGui);

		CDiceManager::DeleteInstance();
		NTL_RETURN(FALSE);
	}	
	((CItemDiceGui*)pPartyCharmDiceGui)->SetExplain( GetDisplayStringManager()->GetString(DST_PARTY_CHARM_START_DICE) );
	m_pInstance->m_mapDiceGui[DICE_WORKD_ID_PARTY_CHARM_DICE] = pPartyCharmDiceGui;
...
VOID CDiceManager::HandleEvents( RWS::CMsg &msg )
{
	...
	else if( msg.Id == g_EventDiceRollReqtoServer )
	{
		...
		case DICE_WORKD_ID_PARTY_INVEN_DICE:
			{
				GetDboGlobal()->GetGamePacketGenerator()->SendPartyInven_DiceRollReq();
				break;
			}
		case DICE_WORKD_ID_PARTY_CHARM_DICE:
			{
				GetDboGlobal()->GetGamePacketGenerator()->SendPartyCharmDiceRollReq();
				break;
			}
		default:
			{
