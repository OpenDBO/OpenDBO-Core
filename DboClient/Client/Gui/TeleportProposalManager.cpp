...
void CTeleportProposalManager::HandleMsgBoxResult( RWS::CMsg& msg )
{
	...
	{
	case MBW_TELEPORT_CONFIRM_TIMEOUT: // Teleport Confirm messagebox이 TimeOut 되었을 때
		{
		...
		break;
	case MBW_TELEPORT_FINAL_CONFIRM_TIMEOUT: // Teleport Confirm messagebox가 TimeOut 되었을 때
		{
			if( pResult->eResult == MBR_OK )
			{
				GetDboGlobal()->GetGamePacketGenerator()->SendTeleportConfirmReq( false, pResult->pData->byIndex );
		...
		break;
	case MBW_TELEPORT_COMMON_CONFIRM_YES: // Teleport Confirm messagebox에서 이동하기 가 들어왔을 때
		{
			if( pResult->eResult == MBR_OK )
			{
		...
		break;
	case MBW_TELEPORT_COMMON_CONFIRM_NO: // Teleport Confirm messagebox에서 이동하지 않기가 들어왔을 때
		{
			if( pResult->eResult == MBR_OK )
			{
