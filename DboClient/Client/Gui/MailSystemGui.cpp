...
void CMailSystemMsg::ModReplyMsg(WCHAR* pwcBuffer, RwUInt32 uiSize, const sMAIL_PROFILE* pMailProfile)
{
	...
	case eMAIL_TYPE_ITEM:
		...
		{
			swprintf_s(pwcBuffer, uiSize, GetDisplayStringManager()->GetString(DST_MAILSYSTEM_MSG_REPLY_MAIL_ITEM), pMailProfile->wszFromName, Logic_GetItemDataFromTable(uiItem)->wszNameText);
		}		
		break;
	case eMAIL_TYPE_ZENNY:
		swprintf_s(pwcBuffer, uiSize, GetDisplayStringManager()->GetString(DST_MAILSYSTEM_MSG_REPLY_MAIL_ZENNY), pMailProfile->wszFromName, uiZenny);
		break;
	case eMAIL_TYPE_ITEM_ZENNY:
		if (uiItem == INVALID_TBLIDX)
		{
			swprintf_s(pwcBuffer, uiSize, GetDisplayStringManager()->GetString(DST_MAILSYSTEM_MSG_REPLY_MAIL_ITEM_ZENNY), pMailProfile->wszFromName, GetDisplayStringManager()->GetString(DST_MAILSYSTEM_NOT_IDENTIFICATION), uiZenny);
		}
		else
		{
			swprintf_s(pwcBuffer, uiSize, GetDisplayStringManager()->GetString(DST_MAILSYSTEM_MSG_REPLY_MAIL_ITEM_ZENNY), pMailProfile->wszFromName, Logic_GetItemDataFromTable(uiItem)->wszNameText, uiZenny);
		}		
		break;
	case eMAIL_TYPE_ITEM_ZENNY_REQ:
		if (uiItem == INVALID_TBLIDX)
		{
			swprintf_s(pwcBuffer, uiSize, GetDisplayStringManager()->GetString(DST_MAILSYSTEM_MSG_REPLY_MAIL_ITEM_ZENNY_REQ), pMailProfile->wszFromName, GetDisplayStringManager()->GetString(DST_MAILSYSTEM_NOT_IDENTIFICATION), uiZenny);
		}
		else
		{
			swprintf_s(pwcBuffer, uiSize, GetDisplayStringManager()->GetString(DST_MAILSYSTEM_MSG_REPLY_MAIL_ITEM_ZENNY_REQ), pMailProfile->wszFromName, Logic_GetItemDataFromTable(uiItem)->wszNameText, uiZenny);
		}
		break;
	case eMAIL_TYPE_ZENNY_REQ:
		swprintf_s(pwcBuffer, uiSize, GetDisplayStringManager()->GetString(DST_MAILSYSTEM_MSG_REPLY_MAIL_ZENNY_REQ), pMailProfile->wszFromName, uiZenny);
		break;
	}
}
...
const WCHAR* CMailSystemMsg::GetItemName(const sMAIL_PROFILE* pMailProfile, RwBool bItemText)
{
	...
	{
		return Logic_GetItemDataFromTable(pMailProfile->sItemProfile.tblidx)->wszNameText;
	}
}
...
void CMailSlotGui::UpdateMailSlot()
{
	...
	WCHAR	aMailContent[NTL_MAX_LENGTH_OF_MAIL_MESSAGE_IN_UNICODE + 1];
	switch (m_sMailProfile.bySenderType)
	{
	case eMAIL_SENDER_TYPE_REPLY:
		CMailSystemMsg::ModReplyMsg(aMailContent, NTL_MAX_LENGTH_OF_MAIL_MESSAGE_IN_UNICODE + 1, &m_sMailProfile);
		pMailContent = aMailContent;
		break;
	case eMAIL_SENDER_TYPE_RETURN:
		CMailSystemMsg::ModReturnMsg(aMailContent, NTL_MAX_LENGTH_OF_MAIL_MESSAGE_IN_UNICODE + 1, &m_sMailProfile);
		pMailContent = aMailContent;
		break;
	}
...
RwBool CMailSystemGui::CreateSendMail(gui::CComponent* pParent)
{
	...
	m_pIpbTargetName->SetMaxLength(NTL_MAX_SIZE_CHAR_NAME_UNICODE);
	m_pIpbMailContent->SetMultilineMode(TRUE);
	m_pIpbMailContent->SetMaxLength(NTL_MAX_LENGTH_OF_MAIL_MESSAGE_IN_UNICODE);
...
void CMailSystemGui::HandleEvents(RWS::CMsg& msg)
{
	...
	if (msg.Id == g_EventSobInfoUpdate)
	{
		...
		if( GetDialogManager()->IsOpenDialog( DIALOG_MAILSYSTEM ) )
		{
			...
			else if( Logic_IsUpdateType( EVENT_AIUT_ZENNY, msg.pData ) )
			{
				RwInt32 nMailZenny = GetNtlSLGlobal()->GetAdjustZennyInfo()->GetAdjustZenny(SAdjustZennyInfo::MAIL_BOX );
				nMailZenny = abs(nMailZenny);
				SetSendMailZenny( m_pBtnAttachSend->IsDown(), m_pBtnAttachReq->IsDown(), nMailZenny );
			}
		}
	}
	...
	else if (msg.Id == g_EventCalcPopupResult)
	{
		...
		if (pEvent->nSrcPlace == PLACE_SUB_MAILSYSTEM_ZENNY)
		{
			...
			{
			case MAILSYSTEM_ATTACH_SEND:	// 송금하기
				{
					if( pEvent->uiValue == 0 )
						return;

					GetIconMoveManager()->IconMovePickUp( INVALID_SERIAL_ID, PLACE_SUB_MAILSYSTEM_ZENNY, INVALID_INDEX, pEvent->uiValue, m_pMoneyIconTexture);
				}
				break;
			case MAILSYSTEM_ATTACH_REQ:		// 청구하기
				{
	...
	else if (msg.Id == g_EventMsgBoxResult)
	{
		...
		if( pEvent->eResult == MBR_OK )
		{
			...
			{
			case MBW_MAILSYSTEM_SEND_ASK:
				SendMail();
				break;
			case MBW_MAILSYSTEM_AWAY_ON:
				GetDboGlobal()->GetGamePacketGenerator()->SendCharAwayReq(true);
				break;
			case MBW_MAILSYSTEM_AWAY_OFF:
...
void CMailSystemGui::SetSendMailTargetName(WCHAR* pwcTargetName)
{
	if (pwcTargetName)
	{
		wcscpy_s(m_awcTargetName, NTL_MAX_SIZE_CHAR_NAME_UNICODE + 1, pwcTargetName);
	}
	else
	{
		memset(m_awcTargetName, NULL, (NTL_MAX_SIZE_CHAR_NAME_UNICODE + 1) * sizeof(WCHAR));
	}
}
...
void CMailSystemGui::OnClickedBtnAttachZenny(gui::CComponent* pComponent)
{
	...
	if (m_pBtnAttachReq->IsDown())
	{
		...
		CMailSystemMsg::MsgSay(CMailSystemMsg::MAIL_MSG_SAY_ATTACH_ZENNY, NTL_MAX_MAIL_SEND_ZENNY_AMOUNTS);
	...
	else if (m_pBtnAttachSend->IsDown())
	{
		...
		{
			RwInt32 nZenny = GetNtlSLGlobal()->GetAdjustZennyInfo()->GetAdjustZenny(SAdjustZennyInfo::MAIL_BOX);
			if( nZenny == 0 )
				return;

			RwInt32 nMaxZenny = NTL_MAX_MAIL_SEND_ZENNY_AMOUNTS;
			if( abs(nZenny) < NTL_MAX_MAIL_SEND_ZENNY_AMOUNTS )
				nMaxZenny = abs(nZenny);
		
			CRectangle rect = pComponent->GetScreenRect();
			CMailSystemMsg::MsgSay(CMailSystemMsg::MAIL_MSG_SAY_ATTACH_ZENNY, NTL_MAX_MAIL_SEND_ZENNY_AMOUNTS);
			CDboEventGenerator::CalcPopupShow(TRUE, uiSerial, PLACE_SUB_MAILSYSTEM_ZENNY, rect.left - dMAILSYSTEM_UI_CALC_OFFSET, rect.top, nMaxZenny);
		}
	}
	else
	{
...
void CMailSystemGui::OnClickedBtnSend(gui::CComponent* pComponent)
{
	...
	{
	case eMAIL_TYPE_BASIC:
		CMailSystemMsg::MsgBox(CMailSystemMsg::MAIL_MSG_BOX_SEND_BASIC, m_eSendMailStamp);
		break;
	case eMAIL_TYPE_ITEM:
		if (m_AttachItemSlot.GetSerialType() == REGULAR_SLOT_ITEM_SOB)
		{
			CMailSystemMsg::MsgBox(CMailSystemMsg::MAIL_MSG_BOX_SEND_ITEM, m_eSendMailStamp, m_AttachItemSlot.GetItemTable()->wszNameText);
		}
		else
		{
			CMailSystemMsg::MsgBox(CMailSystemMsg::MAIL_MSG_BOX_SEND_ITEM, m_eSendMailStamp, GetDisplayStringManager()->GetString(DST_MAILSYSTEM_NOT_IDENTIFICATION_ITEM));
		}
		break;
	case eMAIL_TYPE_ZENNY:
		CMailSystemMsg::MsgBox(CMailSystemMsg::MAIL_MSG_BOX_SEND_ZENNY, m_eSendMailStamp, m_uiSendMailZenny );
		break;
	case eMAIL_TYPE_ITEM_ZENNY:
		if (m_AttachItemSlot.GetSerialType() == REGULAR_SLOT_ITEM_SOB)
		{
			CMailSystemMsg::MsgBox(CMailSystemMsg::MAIL_MSG_BOX_SEND_ITEM_ZENNY, m_eSendMailStamp, m_AttachItemSlot.GetItemTable()->wszNameText, m_uiSendMailZenny);
		}
		else
		{
			CMailSystemMsg::MsgBox(CMailSystemMsg::MAIL_MSG_BOX_SEND_ITEM_ZENNY, m_eSendMailStamp, GetDisplayStringManager()->GetString(DST_MAILSYSTEM_NOT_IDENTIFICATION_ITEM), m_uiSendMailZenny);
		}
		break;
	case eMAIL_TYPE_ITEM_ZENNY_REQ:
		if (m_AttachItemSlot.GetSerialType() == REGULAR_SLOT_ITEM_SOB)
		{
			CMailSystemMsg::MsgBox(CMailSystemMsg::MAIL_MSG_BOX_SEND_ITEM_ZENNY_REQ,m_eSendMailStamp,  m_AttachItemSlot.GetItemTable()->wszNameText, m_uiSendMailZenny);
		}
		else
		{
			CMailSystemMsg::MsgBox(CMailSystemMsg::MAIL_MSG_BOX_SEND_ITEM_ZENNY_REQ, m_eSendMailStamp, GetDisplayStringManager()->GetString(DST_MAILSYSTEM_NOT_IDENTIFICATION_ITEM), m_uiSendMailZenny);
		}
		break;
	case eMAIL_TYPE_ZENNY_REQ:
		CMailSystemMsg::MsgBox(CMailSystemMsg::MAIL_MSG_BOX_SEND_ZENNY_REQ, m_eSendMailStamp, m_uiSendMailZenny);
		break;
	}
}
...
void CMailSystemGui::SetSendMailZenny(bool bIsAttachSend, bool bIsAttachReq, RwUInt32 uiZenny)
{
	...
	else
	{
		...
		m_pStbAttachZenny->SetText(uiZenny);
	}
	...
	else if (msg.Id == g_EventMailItemReceive)
	{
		...
		if (m_sMailProfile.mailID == pEvent->mailID)
		{
			...
			case eMAIL_TYPE_ITEM:
				...
				break;
			case eMAIL_TYPE_ZENNY:
				CMailSystemMsg::MsgSay(CMailSystemMsg::MAIL_MSG_SAY_RECEIVE_ZENNY, m_sMailProfile.dwZenny);
				break;
			case eMAIL_TYPE_ITEM_ZENNY:
				CMailSystemMsg::MsgSay(CMailSystemMsg::MAIL_MSG_SAY_RECEIVE_ITEM, CMailSystemMsg::GetItemName(&m_sMailProfile, TRUE));
				CMailSystemMsg::MsgSay(CMailSystemMsg::MAIL_MSG_SAY_RECEIVE_ZENNY, m_sMailProfile.dwZenny);
			...
			case eMAIL_TYPE_ITEM_ZENNY_REQ:
				CMailSystemMsg::MsgSay(CMailSystemMsg::MAIL_MSG_SAY_RECEIVE_ITEM, CMailSystemMsg::GetItemName(&m_sMailProfile, TRUE));
				CMailSystemMsg::MsgSay(CMailSystemMsg::MAIL_MSG_SAY_RECEIVE_ZENNY_REQ, m_sMailProfile.dwZenny, m_sMailProfile.wszFromName);
				break;
			case eMAIL_TYPE_ZENNY_REQ:
				CMailSystemMsg::MsgSay(CMailSystemMsg::MAIL_MSG_SAY_RECEIVE_ZENNY_REQ, m_sMailProfile.dwZenny, m_sMailProfile.wszFromName);
				break;
...
void CMailSystemReadGui::OnClickedBtnAttachAction(gui::CComponent* pComponent)
{
	...
	case eMAIL_TYPE_ITEM:
		...
		break;
	case eMAIL_TYPE_ZENNY:
		CMailSystemMsg::MsgBox(CMailSystemMsg::MAIL_MSG_BOX_RECEIVE_ZENNY, m_sMailProfile.dwZenny);
		break;
	case eMAIL_TYPE_ITEM_ZENNY:
		CMailSystemMsg::MsgBox(CMailSystemMsg::MAIL_MSG_BOX_RECEIVE_ITEM_ZENNY, m_sMailProfile.dwZenny, CMailSystemMsg::GetItemName(&m_sMailProfile, FALSE));
		break;
	case eMAIL_TYPE_ITEM_ZENNY_REQ:
		CMailSystemMsg::MsgBox(CMailSystemMsg::MAIL_MSG_BOX_RECEIVE_ITEM_ZENNY_REQ, CMailSystemMsg::GetItemName(&m_sMailProfile, FALSE), m_sMailProfile.dwZenny);
		break;
	case eMAIL_TYPE_ZENNY_REQ:
		CMailSystemMsg::MsgBox(CMailSystemMsg::MAIL_MSG_BOX_RECEIVE_ZENNY_REQ, m_sMailProfile.dwZenny);
		break;
	}
	m_mailIDTemp = m_sMailProfile.mailID;	
	//GetDboGlobal()->GetGamePacketGenerator()->SendMailItemReceiveReq(pMailSystem->GetUseObjectSerial(), m_sMailProfile.mailID);
}
...
void CMailSystemReadGui::DisplayReadMailZenny()
{
	...
	m_pStbAttachZenny->Format(L"%u", m_sMailProfile.dwZenny);
