...
VOID CSkillAbilitySetGui::HandleEvents( RWS::CMsg& msg )
{
	if( msg.Id == g_EventRpBonusSetup )
	{
		...
		for(int i=0; i < ICONPOS_NUMS; ++i )
			SetAbility( (eICONPOS)i, pSkillTbl->abyRpEffect[i], pSkillTbl->adwRpEffectValue[i], pSkillTbl );
...
VOID CSkillAbilitySetGui::SetAbility( eICONPOS eIconPos, RwUInt8 byAbilityType, RwUInt32 uiValue, sSKILL_TBLDAT* pSkillTbl, RwBool bEnable /*= TRUE */ )
{
	if( byAbilityType == DBO_RP_BONUS_TYPE_INVALID )
	{
		...
		SetAbility( eIconPos, pNextSkillTbl->abyRpEffect[ eIconPos ],
			pNextSkillTbl->adwRpEffectValue[ eIconPos ], pNextSkillTbl, FALSE );
	}
	else
	{
		if( byAbilityType >= DBO_RP_BONUS_TYPE_INVALID )
		...
		m_asTooltipType[eIconPos].uiValue = uiValue;
...
VOID CSkillAbilitySetGui::SetToolTip( RwInt32 nX, RwInt32 nY, RwUInt8 byAbilityType, RwUInt32 uiValue, RwUInt8 byEffectType )
{
	...
	case DBO_RP_BONUS_TYPE_RESULT_PLUS:
		{
			...
			if( byEffectType == 0 )
			{
				swprintf_s( awcBuffer, 512, GetDisplayStringManager()->GetString( DST_SKILL_RPBONUS_TOOLTIP_RESULT_ABSOLUTE ), uiValue );
				GetInfoWndManager()->ShowInfoWindow( TRUE, CInfoWndManager::INFOWND_JUST_WTEXT,
					nX, nY, 
					(VOID*)awcBuffer ,
					DIALOG_SKILLABILITYSET );
			...
			{
				swprintf_s( awcBuffer, 512, GetDisplayStringManager()->GetString( DST_SKILL_RPBONUS_TOOLTIP_RESULT_PERCENT ), uiValue );
				GetInfoWndManager()->ShowInfoWindow( TRUE, CInfoWndManager::INFOWND_JUST_WTEXT,
					nX, nY, 
					(VOID*)awcBuffer ,
					DIALOG_SKILLABILITYSET );
	...
	case DBO_RP_BONUS_TYPE_EP_MINUS:
		{
			WCHAR awcBuffer[512];
			swprintf_s( awcBuffer, 512, GetDisplayStringManager()->GetString( DST_SKILL_RPBONUS_TOOLTIP_EP_MINUS ), uiValue );
			GetInfoWndManager()->ShowInfoWindow( TRUE, CInfoWndManager::INFOWND_JUST_WTEXT,
				nX, nY, 
				(VOID*)awcBuffer,
				DIALOG_SKILLABILITYSET );
	...
	case DBO_RP_BONUS_TYPE_KEEP_TIME_PLUS:
		{
			WCHAR awcBuffer[512];
			swprintf_s( awcBuffer, 512, GetDisplayStringManager()->GetString( DST_SKILL_RPBONUS_TOOLTIP_KEEPTIME_PLUS ), uiValue );
			GetInfoWndManager()->ShowInfoWindow( TRUE, CInfoWndManager::INFOWND_JUST_WTEXT,
				nX, nY, 
				(VOID*)awcBuffer,
				DIALOG_SKILLABILITYSET );
	...
	case DBO_RP_BONUS_TYPE_CASTING_TIME_MINUS:
		{
			WCHAR awcBuffer[512];
			swprintf_s( awcBuffer, 512, GetDisplayStringManager()->GetString( DST_SKILL_RPBONUS_TOOLTIP_CASTING_TIME_MINUS ), uiValue );
			GetInfoWndManager()->ShowInfoWindow( TRUE, CInfoWndManager::INFOWND_JUST_WTEXT,
				nX, nY, 
				(VOID*)awcBuffer,
				DIALOG_SKILLABILITYSET );
	...
	case DBO_RP_BONUS_TYPE_COOL_TIME_MINUS:
		{
			WCHAR awcBuffer[512];
			swprintf_s( awcBuffer, 512, GetDisplayStringManager()->GetString( DST_SKILL_RPBONUS_TOOLTIP_COOLTIME_MINUS ), uiValue );
			GetInfoWndManager()->ShowInfoWindow( TRUE, CInfoWndManager::INFOWND_JUST_WTEXT,
				nX, nY, 
				(VOID*)awcBuffer,
				DIALOG_SKILLABILITYSET );
	...
	case DBO_RP_BONUS_TYPE_GUARD_CRASH:
		GetInfoWndManager()->ShowInfoWindow( TRUE, CInfoWndManager::INFOWND_JUST_WTEXT,
			nX, nY, 
			(VOID*)GetDisplayStringManager()->GetString( DST_SKILL_RPBONUS_TOOLTIP_GUARD_CRASH ),
			DIALOG_SKILLABILITYSET );
		break;
	default:
		break;
