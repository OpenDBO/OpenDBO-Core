...
{
	COUNTER_DISPLAY_1,		// 2D 배경에 시간대별 이미지 크기를 다르게 할 수 있다
	COUNTER_DISPLAY_2,		// 플래쉬 배경
};

enum eCounterQuadrant		// SetPosition 함수로 지정된 지정으로 부터 정렬되는 공간
{							//						||
...
#define dCOUNTER_DISPLAY_FLAG_BIGGER	0x01		///< 일정시간마다 GUI가 커진다(COUNTER_DISPLAY_2은 적용되지 않는다)
