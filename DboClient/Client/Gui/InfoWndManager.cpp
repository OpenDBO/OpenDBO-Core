...
VOID CInfoWndManager::ShowInfoWindow( RwBool bShow, RwInt32 eState /* = INFOWND_NOTSHOW */, RwInt32 nXPos /* = 0 */, RwInt32 nYPos /* = 0 */, VOID* pInstance /* = NULL */, RwInt32 eRequestGui /* = DIALOG_UNKNOWN  */)
{
	...
	if( bShow )
	{
		...
		case INFOWND_ITEM_DATA:
			{
				...
				for( RwUInt32 i = 0 ; i < NTL_MAX_OPTION_IN_ITEM ; ++i )
					sItemProfile.aOptionTblidx[i] = pItemData->aOptionTblidx[i];
		...
		case INFOWND_REWARD:
			...
			break;
		case INFOWND_CHARM_BUFF:
			m_pInfoWnd->SetCharmBuffInfo( reinterpret_cast<sITEM_TBLDAT*>( pInstance ) );
			break;
        ...
        case INFOWND_PORTAL:
            ...
            break;
		case INFOWND_SCOUTER_SLOT:
			m_pInfoWnd->SetScouterInfo( *reinterpret_cast<SERIAL_HANDLE*>( pInstance ) );
			break;
