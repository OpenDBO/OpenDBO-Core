...
RwBool CTBGambleResultGui::Create()
{
	...
	if( !CNtlPLGui::Create("", dGAMBLERESULT_FILE_SURFACE, dGAMBLERESULT_FILE_FRAME ) )
		NTL_RETURN(FALSE);
	...
	LinkMsg( g_EventShopGambleBuyRes, 0 );
...
VOID CTBGambleResultGui::OnMove( RwInt32 iOldX, RwInt32 iOldY )
{
	...
	m_sgGambleItem.SetParentPosition( rect.left + dGAMBLERESULT_SLOT_OFF_X_FROM_SLOT, rect.top + dGAMBLERESULT_SLOT_OFF_Y_FROM_SLOT );
}
