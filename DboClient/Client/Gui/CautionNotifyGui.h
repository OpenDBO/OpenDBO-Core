...
public:
	...
	CCautionNotifyContinuance(CCautionNotifyGui* pNotify, RwUInt32 uiStringID, RwReal fLimitTime, RwReal fIntervalTime);
	~CCautionNotifyContinuance();

	void		Update(RwReal fElapsed);
	RwReal		IsRemain();
	RwUInt32	GetStringID()				{ return m_uiStringID; }

	int			OnIntervalTimer(RwReal fIntervalTime);

protected:
	RwUInt32			m_uiStringID;
...
{
public:
	typedef VOID (CCautionNotifyGui::*HandleFunc)(RwBool, RwUInt32);
	typedef std::map<RwUInt32, HandleFunc>			HANDLEMAP;
...
public:
	...
	VOID	ShowCautionNotify( RwBool bActive, RwUInt32 uiStringID, const WCHAR* pwcString );

protected:
	VOID	PositionAlign( RwInt32 nScreenWidth, RwInt32 nScreenHeight );
	VOID	SetCurrentCautionNotify( RwUInt32 uiStringID, const WCHAR* pwcString );
	VOID	ClearCurrentCautionNotify();

	// Handler for each other string id
	VOID	FreePvpOutSideHandler(RwBool bActive, RwUInt32 uiStringID);
	
protected:
	VOID	RegisterHandler();
	VOID	AddContinuance( RwUInt32 uiStringID, RwReal fLimitTime, RwReal fIntervalTime );
	VOID	DeleteContinuance( RwUInt32 uiStringID );
