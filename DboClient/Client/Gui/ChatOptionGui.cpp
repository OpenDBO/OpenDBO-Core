...
RwBool CChatOptionGui::Create(CChatGui* pChatGui)
{
	...
	m_pCommandButton->SetText( GetDisplayStringManager()->GetString(DST_CHAT_SHORT_CUT_COMMAND) );
	...
	EnableCatting(CHAT_TYPE_TRADE,		false);
...
VOID CChatOptionGui::EnableCatting(eChatType eType, BOOL bEnable)
{
	...
	{
		if( false == m_pChatGui->GetLastRecieveWhisperName().empty() )
			return;
