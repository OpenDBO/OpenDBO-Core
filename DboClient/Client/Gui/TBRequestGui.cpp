...
RwBool CTBRequestGui::Create()
{
	...
	m_pIpbTeamName->SetMaxLength(NTL_MAX_SIZE_PARTY_NAME_IN_UNICODE);
	...
	else if( m_byGuiType == TYPE_TICKET )
	{
		if( m_byRequestType == REQUEST_INDIVIDUAL )
		{
			...
			if( pJoinInfo->sIndividualInfo.bDojoRecommender )
			{
				...
				if( pTexture )
				{
					m_strEmblemTextureName = RwTextureGetName( pTexture );
...
void CTBRequestGui::OnClickedBtnAccept( gui::CComponent* pComponent )
{
	...
	{		
		if( m_byRequestType == REQUEST_INDIVIDUAL )
		{
			//CDboEventGenerator::MsgBoxShow( GetDisplayStringManager()->GetString( DST_BUDOKAI_INDI_REQ_CANCEL ),
			//	MBW_TB_REQUEST_INDI_CANCEL, MBTF_OK | MBTF_CANCEL );
			GetAlarmManager()->AlarmMessage( DST_BUDOKAI_INDI_REQ_CANCEL );
		}
		else if( m_byRequestType == REQUEST_TEAM )
		{
			//CDboEventGenerator::MsgBoxShow( GetDisplayStringManager()->GetString( DST_BUDOKAI_TEAM_REQ_CANCEL ),
			//	MBW_TB_REQUEST_TEAM_CANCEL, MBTF_OK | MBTF_CANCEL );
