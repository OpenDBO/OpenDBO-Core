...
, m_nRankBattleItemCount(0)
{
	...
	ZeroMemory( m_awcSearchName, sizeof(WCHAR) * (NTL_MAX_SIZE_CHAR_NAME_UNICODE+1) );
...
void CRankBattleBoard::MessageProc( RWS::CMsg& msg ) 
{
	...
	if( msg.Id == g_EventRankBattleRankListRes )
	{
		...
		for( int i = 0; i< pRes->byRankInfoCount; ++i )
		{
			...
			::ZeroMemory( m_sRankBattleItem[i].awcName, sizeof(WCHAR) * NTL_MAX_SIZE_CHAR_NAME_UNICODE+1 );
			...
			::ZeroMemory( m_sRankBattleItem[i].awcGuildName, sizeof(WCHAR) * NTL_MAX_SIZE_GUILD_NAME_IN_UNICODE+1 );
