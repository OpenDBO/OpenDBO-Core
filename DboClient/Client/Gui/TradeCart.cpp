...
RwBool CTradeCart::Create()
{
	...
	m_pTotalSellMoney->SetText("1234567890");
...
VOID CTradeCart::OnCaptureWheelMove(RwInt32 iFlag, RwInt16 sDelta, CPos& pos)
{
	...
	{
		if( sDelta > 0 )
			AddItem(SELL_SLOT, iSlot, 1);
		else
			SubItem(SELL_SLOT, iSlot, 1);
	}
}
...
VOID CTradeCart::AddItemCount(RwInt32 iSlotKind, RwInt32 iSlot, RwInt32 iCount)
{
	if( iSlotKind == BUY_SLOT )
	{
		RwInt32 iResult = m_BuySlotInfo[iSlot].NPCShopBuyInfo.byStack + iCount;

		if( iResult > NTL_MAX_ITEM_STACK_COUNT )
			iResult = NTL_MAX_ITEM_STACK_COUNT;
...
VOID CTradeCart::UpdateCount(RwUInt8 bySlotKind, RwUInt8 bySlot, RwUInt8 byCount)
{
	...
	{
		if( byCount > NTL_MAX_ITEM_STACK_COUNT )
			byCount = NTL_MAX_ITEM_STACK_COUNT;
...
VOID CTradeCart::CalcTotalBuyPrice()
{
	...
	m_pTotalBuyMoney->SetText((RwInt32)iTotalPrice);
...
VOID CTradeCart::CalcTotalSellPrice()
{
	...
	for( RwInt32 i = 0 ; i < MAX_SLOT ; ++i )
	{
		...
		{
			iSellPrice = Logic_CalcPrice_for_Sell(m_SellSlotInfo[i].slot.GetItemTable(),
												  m_SellSlotInfo[i].slot.GetItemOptionTable(),
												  m_SellSlotInfo[i].NPCShopSellInfo.byStack);

			if( iSellPrice <= 0 )
				iSellPrice = 1;

			m_uiTotalSellPrice += iSellPrice;
		}		
	}

	m_pTotalSellMoney->SetText((RwInt32)m_uiTotalSellPrice);
...
VOID CTradeCart::AddItem(RwInt32 iSlotKind, RwInt32 iSlotY, RwInt32 iCount)
{
	...
	else if( iSlotKind == SELL_SLOT )
	{
		...
		{			
			if( m_eDialogType == DIALOG_SHOPING_CART )
			{
				...
				AddItemCount(SELL_SLOT, iSlotY, iCount);
				CalcTotalSellPrice();
			}
			else
			{
...
VOID CTradeCart::SubItem(RwInt32 iSlotKind, RwInt32 iSlotY, RwInt32 iCount)
{
	if( iSlotY < 0 || iSlotY >= MAX_SLOT )
	{
...
VOID CTradeCart::RegSellItemByDrag(RwInt32 iSlot)
{
	...
	if( m_SellSlotInfo[iSlot].slot.GetCount() <= 0 )
	{
		...
		CRectangle rect = m_pThis->GetScreenRect();				
		RwInt32 iCount = GetIconMoveManager()->GetStackCount();
		RwUInt32 uiPrice = pITEM_TBLDAT->dwCost;
		...
		// 갯수
		AddItemCount(SELL_SLOT, iSlot, iCount);
...
VOID CTradeCart::RegBuyItemByEvent(RwInt32 iSlot, SDboEventShop& TradeInfo)
{
	...
	AddItemCount(BUY_SLOT, iSlot, TradeInfo.iOverlapCount);
	CalcTotalBuyPrice();
}
...
{		
	...
	m_SellSlotInfo[iSlot].slot.SetPrice(pITEM_TBLDAT->dwCost);
	...
	if( TradeInfo.iOverlapCount > NTL_MAX_ITEM_STACK_COUNT)
		TradeInfo.iOverlapCount = NTL_MAX_ITEM_STACK_COUNT;
...
VOID CTradeCart::RegTakeItemByPacket(RwInt32 iSlot, SERIAL_HANDLE hGiveHandle, sITEM_DATA* pITEM_DATA, RwUInt8 byCount)
{
	...
	AddItemCount(BUY_SLOT, iSlot, byCount);	
}
		...
		case USERTRADE_UPDATE_ZENNY:
			{
				...
				m_pTotalSellMoney->SetText(pPacket->uiHandle);
		...
		case USERTRADE_UPDATE_ZENNY_NOTIFY:
			{
				...
				m_pTotalBuyMoney->SetText(pPacket->uiHandle);
