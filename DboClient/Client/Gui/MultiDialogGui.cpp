...
VOID CMultiDialogGui::HandleEvents( RWS::CMsg& msg )
{
	if( msg.Id == g_EventSobNpcCommunity )
	{
		...
		{
			// 월드컨셉 체크 2차 월드컨셉이 진행중이면 
			if( GetNtlWorldConcept()->IsActivePlayGrade( WORLD_CONCEPT_SECOND_GRADE ) )
			{
				// 세팅해주었던 TargetSerialID를 해제하고 WorldConcept 에 관련된 메시지를 출력한다.
				m_hTargetSerialID = INVALID_SERIAL_ID;
			...
			{
				// 천하제일 무도회 상인은 NPC_FUNC_FLAG_MERCHANT와 NPC_FUNC_FLAG_BUDOHSI_MERCHANT 플래그를 가지고 있다
				// NPC_FUNC_FLAG_BUDOHSI_MERCHANT 플래그 검사점에서 RegistCommand 함수를 호출하자
			...
			if( pTableData->dwFunc_Bit_Flag & NPC_FUNC_FLAG_BANKER )
			{
				...
				if( pAvatar )
				{
					...
					// 처음 창고NPC와 대화를 하면 자신의 창고 정보를 서버로부터 받는다.
					// 창고 정보를 알고 있어야 자신이 더 살 수 있는 창고를 알아볼 수 있다
	...
	else if( msg.Id == g_EventSobTriggerObjCommunity )
	{
		...
		else
		{
			if( pGetTriggerList->dwType & eDBO_TRIGGER_OBJECT_FUNC_BIND )
			{
				...
				DBO_ASSERT( pTriggerObj, "CMultiDialogGui::HandleEvents : TriggerObj Serial( " << pData->hSerialId << " )가 존재하지 않습니다." );
...
VOID CCommandList::ClearState_ClearAndMakeBuyBankMenu(VOID)
{
	...
	// avooo's command : 기획의 의도에 따라 창고 하나는 기본적으로 가지고 있다.
	//					 따라서, 구입할 수 있는 창고의 인덱스는 1부터 시작한다
	for( RwUInt8 i = 1 ; i < NTL_MAX_BANKSLOT_COUNT ; ++i )
	{
		...
		if( hWarehouseSlotHandle == INVALID_SERIAL_ID )
		{
			...
			{
				// 공유 창고는 유료화하기로 하였다. 유료화 작업전까지 막아두자
