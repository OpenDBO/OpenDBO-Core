...
protected:
	...
	gui::CStaticBox*	m_pDialogName;			///< 다이얼로그 이름
	gui::CStaticBox*	m_pRegionName;			///< 지역 이름
	gui::CStaticBox*	m_pLandMarkName[MAX_LANDMARK_NAME];	///< 랜드 마크 이름
	...
	ZONEID				m_FocusZoneID;			///< 포커싱 되고 있는 존 인덱스
	...
	ZONEID				m_uiPressedZoneIndex;	///< 마우스 다운된 존의 인덱스
	...
	WORLDID				m_uiRenderingWorldID;	///< 화면에 그려지고 있는 지역의 월드 아이디
	ZONEID				m_uiRenderingZoneID;	///< 화면에 그려지고 있는 지역의 존 아이디

	WORLDID				m_uiActiveWorldID;		///< 아바타가 위치한 월드 ID
