...
RwBool CWarehouseGui::Create()
{
	...
	if(!CNtlPLGui::Create("", "gui\\Inventory.srf", "gui\\Warehouse.frm"))
		NTL_RETURN(FALSE);
...
RwBool CWarehouseCommonGui::Create()
{
	...
	if(!CNtlPLGui::Create("", "gui\\Inventory.srf", "gui\\WarehouseCommon.frm"))
		NTL_RETURN(FALSE);
...
RwInt32 CWarehouseBarGui::SwitchDialog(bool bOpen)
{
	...
	else
	{
		...
		else
		{
			...
			{
				SERIAL_HANDLE hItem = pMsgBox->GetMsgBoxData()->sItemDeleteInfo.hSerial;
