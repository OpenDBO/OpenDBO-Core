...
RwUInt8 CQuickSlotItem::GetStackCount(VOID)
{
	if( !IsItem() )
		return 0;

	LIST_QUICKSLOTOBJ_ITER iter;
	RwUInt8 byStackCount = 0;
	...
	for( iter = m_listObj.begin() ; iter != m_listObj.end() ; ++iter )
	{
		...
		byStackCount = byStackCount + pItemAttr->GetStackNum();
	}
...
RwBool CQuickSlotGui::Create(VOID)
{
	...
	/*m_ppnlAlt = (gui::CPanel*)GetComponent( "pnlAlt" );
	m_ppnlCtrl = (gui::CPanel*)GetComponent( "pnlCtrl" );*/
...
VOID CQuickSlotGui::Update( RwReal fElapsed )
{
	for( RwInt32 i = 0 ; i < m_nVisibleRowCount ; ++i )
	{
		...
		{
			RwInt32 nDataRow = GetDataRowIndex( i );				
...
VOID CQuickSlotGui::HandleEvents( RWS::CMsg& msg )
{
	...
	else if( msg.Id == g_EventQuickSlotDelNfy )
	{
		...
		if( !m_bLock )
		{
			UnsetIconFromQuickSlot( SLOTID_TO_ROW( pNotify->bySlotIdx ), SLOTID_TO_COL( pNotify->bySlotIdx ), INVALID_SERIAL_ID, FALSE );
			UpdateAllSlot();		
		}
	}
...
VOID CQuickSlotGui::LoadQuickSlot(VOID)
{
	...
	for( RwInt32 i = 0 ; i < pAvatarInfo->byQuickSlotInfoCount ; ++i )
	{
		...
		// peessi : 튜토리얼때, 서버에서 스킬을 지우기 위해 날리는 패킷이 아바타 생성전에 날라올 때가 있다.
		//			그 때, 아바타인포의 퀵슬롯정보의 슬롯정보를 INVALID_BYTE로 수정하여 처리한다. 
		//			패킷이 날라올때 이미 DB정보도 삭제되므로 굳이 서버로 삭제요청을 하지 않아도 된다. 
...
VOID CQuickSlotGui::ShowIconDestination(VOID)
{
	if( GetIconMoveManager()->IsActive() )
	{
		...
		if( pSob->GetIcon()->IsUsePossible() )
		{
			for( RwInt32 j = 0 ; j < QUICKSLOT_MAX_VISIBLE_ROW ; ++j )
			{
				for( RwInt32 i = 0 ; i < QUICKSLOT_MAXCOL ; ++i )
					m_anFocusEffect[j][i] = m_anFocusEffect[j][i] | SLOT_FOCUS_CAN_MOVE;
			}			
		}
	}
	else
...
VOID CQuickSlotGui::IconPutDownProc( SERIAL_HANDLE hSerial, RwInt32 ePlace, RwInt32 nSlotIdx )
{
	if( CheckIconMoveLock() )
		return;	
	...
	else if( ePlace == PLACE_QUICKSLOT )
	{
		...
		if( IsValidRowIndex( nSrcDataRow ) )
		{
			...
			{
				RwInt32 nSlotID = ROWCOL_TO_SLOTID( nDataRow, nCol );
				NTL_ASSERT( nSlotID >= 0 && nSlotID < 48, "CQuickSlotItem::SetItem by PLACE_QUICKSLOT New : SlotID Error!(" << nSlotID << ")" );	
			...
			if( m_aQuickSlotItem[nSrcDataRow][nSrcCol].IsValid() )
			{
				...
				NTL_ASSERT( nSlotID >= 0 && nSlotID < 48, "CQuickSlotItem::SetItem by PLACE_QUICKSLOT Old : SlotID Error!(" << nSlotID << ")" );	
...
VOID CQuickSlotGui::DeleteItemIcon( SERIAL_HANDLE hSerial )
{
	for( RwInt32 i = 0 ; i < QUICKSLOT_MAXROW ; ++i )
	{
		for( RwInt32 j = 0 ; j < QUICKSLOT_MAXCOL ; ++j )
			...
			if( nDataRowIndex >= 0 )
			{
				if( m_aQuickSlotItem[nDataRowIndex][i].IsValid() )
				{
					...
					else if( pCurrentIcon->GetClassID() == SLCLASS_ACTIONSKILL_ICON )
					{
						...
						{
							if( pActionTblDat->byETC_Action_Type == ACTION_FUNC_TYPE_BASIC_ATTACK &&
								m_bAttackMode )
							{
...
VOID CQuickSlotGui::CoolingEffectProc( SERIAL_HANDLE hSerial, BOOL bStart )
{
	for( RwInt32 i = 0 ; i < m_nVisibleRowCount ; ++i )
	{
		for( RwInt32 j = 0 ; j < QUICKSLOT_MAXCOL ; ++j )
...
RwBool CQuickSlotGui::UseableCheck_LPEPRP( CNtlSobAttr* pAttr )
{
	...
	if( pAttr->GetClassID() == SLCLASS_SKILL )
	{
		...
		if( pSkillAttr->GetSkillTbl()->wRequire_LP > pAvatarAttr->GetLp() )
			return FALSE;

		if( pSkillAttr->GetSkillTbl()->wRequire_EP > pAvatarAttr->GetEp() )
			return FALSE;

		if( pSkillAttr->GetSkillTbl()->wFunction_Bit_Flag & 0x01 )		// Enum 정의 필요
		{
...
{
	// 로직은 CNtlSobSkillIcon::IsUsePossible()에서 차용.
	// 변신중일때는 일반 스킬은 사용하지 못한다
	if(Logic_IsTransform(GetNtlSLGlobal()->GetSobAvatar()))
	{
		DWORD flag = pData->dwTransform_Use_Info_Bit_Flag;        
...
{
	// OnMouseDown이 호출되면 현재 마우스가 있는 위치를 가지고 퀵슬롯의 인덱스를 가지고 오고
	// 마우스가 퀵슬롯의 각각의 인덱스에 포함되면 그 인덱스에 눌린 효과를 준다.
