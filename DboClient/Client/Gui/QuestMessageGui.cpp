...
RwBool CQuestMessageGui::Create( CQuestGui* pQuestGui )
{
	...
	//surface = GetNtlGuiManager()->GetSurfaceManager()->GetSurface( "QuestMessage.srf", "srfButtonEffect" );
	//m_MessageButton[NEXT].m_Effect.SetSurface( surface );
	//m_MessageButton[NEXT].m_Effect.SetTime( 0.0f, QUEST_MESSAGE_BTN_PUSLE );
	//m_MessageButton[NEXT].m_Effect.SetAlpha( 0, 255 );

	//surface = GetNtlGuiManager()->GetSurfaceManager()->GetSurface( "QuestMessage.srf", "srfButtonEffect" );
	//m_MessageButton[PREV].m_Effect.SetSurface( surface );
	//m_MessageButton[PREV].m_Effect.SetTime( 0.0f, QUEST_MESSAGE_BTN_PUSLE );
	//m_MessageButton[PREV].m_Effect.SetAlpha( 0, 255 );
...
VOID CQuestMessageGui::Update( RwReal fElapsed )
{
	...
	//for( RwInt32 i = 0 ; i < BTNCOUNT ; ++i )
	//	m_MessageButton[i].Update( fElapsed );
}
...
VOID CQuestMessageGui::HandleEvents( RWS::CMsg& msg )
{
	...
	else if( msg.Id == g_EventQuestMessage )
	{
		...
		{
			NTL_ASSERT(false, "CQuestMessageGui::HandleEvents, g_EventQuestMessage, Not exist Quest text data table of index : " << pTIMEQUEST_DATASET->nameTblidx);
		}		

		// TMQ quest text index 규칙
		// 14 + TMQ table index(3자리수 유지) + stage number(2자리수 유지)
		RwUInt32 uiQuestTextIndex = 1400000 + (pData->uiValue * 100) + pData->uiValue2;
...
VOID CQuestMessageGui::OnPaint(VOID)
{
	//for( RwInt32 i = 0 ; i < BTNCOUNT ; ++i )
	//{
	//	m_MessageButton[i].m_Effect.Render();
	//}
}
...
{
	// 다음페이지로 넘어가야할 때
	//if( m_MessageButton[NEXT].IsEnable() )
	{
		// 마지막 페이지
		...
		{
			//m_MessageButton[NEXT].Enable( FALSE );
			//m_MessageButton[PREV].Enable( TRUE );	
