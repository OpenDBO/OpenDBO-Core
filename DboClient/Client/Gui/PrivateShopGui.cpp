								   ...
								   CPrivateShopGui* pPrivateShopGui)
{
	...
	// 흥정 버튼
	rect.SetRectWH(52, 24, 23, 20);
	m_pShopItemBusiness = NTL_NEW gui::CButton( rect, L"", m_pShopItem, GetNtlGuiManager()->GetSurfaceManager() );
	m_pShopItemBusiness->AddSurfaceUp(GetNtlGuiManager()->GetSurfaceManager()->GetSurface("PrivateShop.srf", "srfTradeUp"));
	m_pShopItemBusiness->AddSurfaceFocus(GetNtlGuiManager()->GetSurfaceManager()->GetSurface("PrivateShop.srf", "srfTradeFocus"));
	m_pShopItemBusiness->AddSurfaceDown(GetNtlGuiManager()->GetSurfaceManager()->GetSurface("PrivateShop.srf", "srfTradeDown"));
	m_pShopItemBusiness->AddSurfaceDisabled(GetNtlGuiManager()->GetSurfaceManager()->GetSurface("PrivateShop.srf", "srfTradeDisable"));
	m_pShopItemBusiness->ClickEnable(true);
	...
	// Business 중의 Surface 세팅
	m_EffectBusiness.SetSurface(GetNtlGuiManager()->GetSurfaceManager()->GetSurface("PrivateShop.srf", "srfShopItemBusiness"));
	m_EffectBusiness.SetPositionfromParent(7, 6);
	...
	m_slotClickedBtnBusiness = m_pShopItemBusiness->SigClicked().Connect(this, &CPrivateShopItemGui::OnClickedBtnBusiness );
...
void CPrivateShopItemGui::Destroy()
{
	...
	NTL_DELETE( m_pShopItemBusiness );
...
void CPrivateShopItemGui::Clear()
{
	...
	m_pShopItemPrice->ClickEnable(false);
	m_pShopItemBusiness->Show(false);
	m_uiItemState	= sPRIVATESHOP_ITEM_DATA::PRIVATESHOP_GENERAL;
	m_uiPos			= PRIVATESHOP_ITEMPOS_INVALID;
}
...
void CPrivateShopItemGui::OnBgPaint()
{
	...
	case sPRIVATESHOP_ITEM_DATA::PRIVATESHOP_GENERAL:
		break;
	case sPRIVATESHOP_ITEM_DATA::PRIVATESHOP_BUSINESS_ITEM:
		m_EffectBusiness.Render();
		break;
...
void CPrivateShopItemGui::OnMove(RwInt32 iOldX, RwInt32 iOldY)
{
	...
	m_EffectPick.SetPositionbyParent(rect.left, rect.top);
 	m_EffectBusiness.SetPositionbyParent(rect.left, rect.top);
}
...
/**
* \brief 손님이 흥정 버튼을 클릭 하였을 경우
* \param pComponent		클릭된 Component
*/
void CPrivateShopItemGui::OnClickedBtnBusiness( gui::CComponent* pComponent ) 
{
	m_pPrivateShopGui->OnShopItemBusinessClicked(this, pComponent);
}
...
void CPrivateShopItemGui::SetIcon(RwUInt8 uiPos, RwUInt32 uiSerial, RwUInt32 uiCount, RwUInt32 uiPrice, RwUInt8 uiItemState, eREGULAR_SLOT_TYPE eSerialType)
{
	...
	if (uiSerial != INVALID_SERIAL_ID)
	{
		...
		WCHAR temp[32];
		swprintf_s(temp, 32, L"%u", uiPrice);
		m_pShopItemPrice->SetText(temp);
...
,m_pStbSoldOut(NULL)
,m_bSale(FALSE)
,m_bBusinessWait(FALSE)
,m_fBusinessWait(0.0f)
,m_bAway(FALSE)
,m_bOwnerBusiness(FALSE)
,m_uiBusinessSlotPos(PRIVATESHOP_ITEMPOS_INVALID)
{
}
...
RwBool CPrivateShopGui::Create()
{
	...
	// 흥정
	LinkMsg(g_EventPrivateShopBusinessRequestRes, 0 );
	LinkMsg(g_EventPrivateShopBusinessRequestNfy, 0 );
	LinkMsg(g_EventPrivateShopBusinessImmediateItemBuyingRes, 0 );
	LinkMsg(g_EventPrivateShopBusinessImmediateItemBuyingNfy, 0 );
	LinkMsg(g_EventPrivateShopBusinessConsentReq, 0 );
	LinkMsg(g_EventPrivateShopBusinessConsentNfy, 0 );
	LinkMsg(g_EventPrivateShopBusinessCancleRes, 0 );
	LinkMsg(g_EventPrivateShopBusinessCancleNfy, 0 );
	LinkMsg(g_EventPrivateShopBusinessWaittimeOutNfy, 0 );
...
void CPrivateShopGui::Destroy()
{
	...
	UnLinkMsg(g_EventPrivateShopBusinessRequestRes);
	UnLinkMsg(g_EventPrivateShopBusinessRequestNfy);
	UnLinkMsg(g_EventPrivateShopBusinessImmediateItemBuyingRes);
	UnLinkMsg(g_EventPrivateShopBusinessImmediateItemBuyingNfy);
	UnLinkMsg(g_EventPrivateShopBusinessConsentReq);
	UnLinkMsg(g_EventPrivateShopBusinessConsentNfy);
	UnLinkMsg(g_EventPrivateShopBusinessCancleRes);
	UnLinkMsg(g_EventPrivateShopBusinessCancleNfy);
	UnLinkMsg(g_EventPrivateShopBusinessWaittimeOutNfy);
...
RwInt32 CPrivateShopGui::SwitchDialog(bool bOpen)
{
	...
	{  
		...
		case PRIVATESHOP_SELL:	// 주인
			{
				// 게임 컨셉에 물어보고 일반 상태로 전환이 불가능하다면 리턴시킨다.
				if( CNtlGameRuleConcept::GetInstance()->CanTransition( GAME_RULE_TYPE_MAIN_NORMAL ) == FALSE )
					NTL_RETURN(FALSE);
...
void CPrivateShopGui::HandleEvents(RWS::CMsg& msg)
{
	...
	else if( msg.Id == g_EventPrivateShopBusinessRequestRes)	
		HandlePrivateShopBusinessRequestRes( msg.pData );
	else if( msg.Id == g_EventPrivateShopBusinessRequestNfy)	
		HandlePrivateShopBusinessRequestNfy( msg.pData );
	else if( msg.Id == g_EventPrivateShopBusinessConsentReq)	
		HandlePrivateShopBusinessConsentReq( msg.pData );
	else if( msg.Id == g_EventPrivateShopBusinessConsentNfy )	
		HandlePrivateShopBusinessConsentNfy( msg.pData );
	else if( msg.Id == g_EventPrivateShopBusinessCancleRes )	
		HandlePrivateShopBusinessCancleRes( msg.pData );
	else if( msg.Id == g_EventPrivateShopBusinessCancleNfy )	
		HandlePrivateShopBusinessCancleNfy( msg.pData );
	else if( msg.Id == g_EventPrivateShopBusinessImmediateItemBuyingRes )
		HandlePrivateShopBusinessImmediateItemBuyingRes( msg.pData );
	else if( msg.Id == g_EventPrivateShopBusinessImmediateItemBuyingNfy )
		HandlePrivateShopBusinessImmediateItemBuyingNfy( msg.pData );
	else if( msg.Id == g_EventPrivateShopBusinessWaittimeOutNfy )
		HandlePrivateShopBusinessWaittimeOutNfy( msg.pData );
...
void CPrivateShopGui::HandleMsgboxResult( void* pData ) 
{
	...
	{
		// 개인상점 시작
	case MBW_PRIVATESHOP_SALE_START:
		{
			if( pMsgBoxResult->eResult == MBR_OK )
			{
				// 게임 룰 컨셉에게 게임상점 판매 상태로 전환이 가능한지 물어보고 불가능하다면 리턴시킨다.
				if( CNtlGameRuleConcept::GetInstance()->CanTransition( GAME_RULE_TYPE_MAIN_PRIVATESHOP_RUN ) == FALSE )
					NTL_RETURNVOID();
		...
		break;
		// 개인상점 중지
	case MBW_PRIVATESHOP_SALE_STOP:
		{
			// 게임 룰 컨셉에게 게임상점 준비 가능 여부 확인
		...
		break;
		// 개인상점 흥정 요청의 상태
	case MBW_PRIVATESHOP_BUSINESS_CONSENT:
		{
			// 수락
			if( pMsgBoxResult->eResult == MBR_OK )
			{
				// 흥정 중인 아이템이 있을 경우
				if( m_uiBusinessSlotPos != PRIVATESHOP_ITEMPOS_INVALID )
				{
					// 아이템의 ID
					sITEM_TBLDAT* pTblDat = Logic_GetItemDataFromTable(
						m_aShopItemData[m_uiBusinessSlotPos].sItem.itemNo );

					// 상대방의 이름
					CNtlSobPlayer* pSobPlayer = reinterpret_cast<CNtlSobPlayer*>(GetNtlSobManager()->GetSobObject(m_uiBusinessTarget));
					
					// TRUE, Table_Index
					GetDboGlobal()->GetGamePacketGenerator()->SendPrivateShopBusinessConsentRes( TRUE,
						pTblDat->tblidx );

					CDboEventGenerator::PrivateShopBusinessStart( PRIVATESHOP_SELL, 
						m_ShopData.hOwner, 
						const_cast<WCHAR*>(Logic_GetName( pSobPlayer )),
						&m_aShopItemData[m_uiBusinessSlotPos],
						this );
				}
			}
			// 거절 또는 중복 취소시
			else if( pMsgBoxResult->eResult == MBR_CANCEL ||
					pMsgBoxResult->eResult == MBR_OPENCANCELED )
			{
				if( m_uiBusinessSlotPos != PRIVATESHOP_ITEMPOS_INVALID )
				{
					// 아이템의 ID
					...
					// FALSE, Table_Index
					GetDboGlobal()->GetGamePacketGenerator()->SendPrivateShopBusinessConsentRes( FALSE,
						pTblDat->tblidx );

					// 아이템 UnLock
					m_aShopItemData[m_uiBusinessSlotPos].byItemState = sPRIVATESHOP_ITEM_DATA::PRIVATESHOP_GENERAL;
					UpdateShopItem();
				}
			}
		...
		break;
		// 주인의 응답을 기다리고 있는 중입니다.
	case MBW_PRIVATESHOP_BUSINESS_WAIT:
		{
			// 취소
			if( !pMsgBoxResult->eResult == MBR_OK )
				GetDboGlobal()->GetGamePacketGenerator()->SendPrivateShopBusinessCancelReq( m_ShopData.hOwner );
...
void CPrivateShopGui::HandleSobPrivateShopCommunity( void *pData ) 
{
	...
	case PRIVATESHOP_STATE_BUSINESS:			// 흥정
		SetBusiness( TRUE );
...
void CPrivateShopGui::HandleSobInfoUpdate( void* pData ) 
{
	...
	if (pSobInfoUpdate->hSerialId == GetNtlSLGlobal()->GetSobAvatar()->GetSerialID())
	{
		...
		{
			m_pZenny->SetText(Logic_GetZenny());
		}
	}
}
...
void CPrivateShopGui::HandlePrivateShopStateVisitor( void *pData ) 
{
	...
	case PRIVATESHOP_EVENT_ENTER:
...

		{
			...
			// 상점.이 흥정 중이라면 TRUE를 해준다.
			if( pShopData->sSummaryPrivateShopData.byShopState == PRIVATESHOP_STATE_BUSINESS_WAIT 
				|| pShopData->sSummaryPrivateShopData.byShopState == PRIVATESHOP_STATE_BUSINESS )
			{
				SetBusiness( TRUE );
			}
		}
...
void CPrivateShopGui::HandlePrivateShopNameTag( void* pData ) 
{
	...
	{		
		...
		case PRIVATESHOP_STATE_DATALOADING:
			GetBalloonManager()->AddPrivateShopBalloon(pSobObject, GetDisplayStringManager()->GetString(DST_PRIVATESHOP_NAMETAG_READY), TRUE);
			break;
		case PRIVATESHOP_STATE_BUSINESS_WAIT:
		case PRIVATESHOP_STATE_BUSINESS:
...
/**
* \brief 손님이 주인에게 흥정을 요청한 경우
*/
void CPrivateShopGui::HandlePrivateShopBusinessRequestRes( void* pData ) 
{
	SNtlEventPrivateShopBusinessRequestRes* pResult = reinterpret_cast<SNtlEventPrivateShopBusinessRequestRes*>(pData);

	SetBusiness( TRUE );

	m_uiBusinessTarget = GetNtlSLGlobal()->GetSobAvatar()->GetSerialID();

	// 아이템을 흥정 상태로 변경
	m_aShopItemData[pResult->byPrivateShopInventorySlotPos].byItemState = pResult->byItemState;
	UpdateShopItem();
	
	// 대기 대화상자 띄움
	//CDboEventGenerator::MsgBoxShow( GetDisplayStringManager()->GetString( DST_PRIVATESHOP_BUSINESS_WAITREQUEST ),
	//	MBW_PRIVATESHOP_BUSINESS_WAIT, MBTF_CANCEL, TRUE);
	GetAlarmManager()->AlarmMessage( DST_PRIVATESHOP_BUSINESS_WAITREQUEST );
}

/**
* \brief 손님이 주인에게 흥정을 요청한 경우( 그 외의 손님들에게 전해지는 Notify )
*/
void CPrivateShopGui::HandlePrivateShopBusinessRequestNfy( void* pData ) 
{
	SNtlEventPrivateShopBusinessRequestNfy* pResult = reinterpret_cast<SNtlEventPrivateShopBusinessRequestNfy*>(pData);
...
/**
* \brief 주인에게 흥정 신청이 들어온 경우
*/
void CPrivateShopGui::HandlePrivateShopBusinessConsentReq( void* pData ) 
{
	SNtlEventPrivateShopBusinessConsentReq* pReq = reinterpret_cast<SNtlEventPrivateShopBusinessConsentReq*>(pData);

	SetBusiness( TRUE );

	// 방문자의 ID
	CNtlSobPlayer* pSobPlayer = reinterpret_cast<CNtlSobPlayer*>(GetNtlSobManager()->GetSobObject(pReq->hVisitorID));

	// 아이템의 테이블
	//m_aShopItemData[pReq->byPrivateShopInventorySlotPos]

	// 미확인 일 경우 미확인 아이템의 이름을 표시해준다.
	if( m_aShopItemData[pReq->byPrivateShopInventorySlotPos].sItem.bNeedToIdentify )	
		GetAlarmManager()->FormattedAlarmMessage( DST_PRIVATESHOP_BUSINESS_CONSENT, FALSE, NULL, Logic_GetName( pSobPlayer ), GetDisplayStringManager()->GetString(DST_PRIVATESHOP_ITEM_NOT_IDENTIFICATION) );
	else
		GetAlarmManager()->FormattedAlarmMessage( DST_PRIVATESHOP_BUSINESS_CONSENT, FALSE, NULL, Logic_GetName( pSobPlayer ), Logic_GetItemName( m_aShopItemData[pReq->byPrivateShopInventorySlotPos].sItem.itemNo ) ); 
					
	// 주인 : 흥정 요청 대화 상자 띄우고 흥정 대기 모드로 들어감
	m_aShopItemData[pReq->byPrivateShopInventorySlotPos].byItemState = sPRIVATESHOP_ITEM_DATA::PRIVATESHOP_BUSINESS_ITEM;
	m_uiBusinessSlotPos = pReq->byPrivateShopInventorySlotPos;
	m_uiBusinessTarget = pReq->hVisitorID;
	UpdateShopItem();
	BusinessWait( TRUE );
	
	//CDboEventGenerator::MsgBoxShow(awcBuffer, MBW_PRIVATESHOP_BUSINESS_CONSENT, MBTF_OK | MBTF_CANCEL, TRUE );

}

/**
* \brief 주인에게 흥정을 요청한 손님에게 띄우는 알리는 흥정 시작
*/
void CPrivateShopGui::HandlePrivateShopBusinessConsentNfy( void* pData ) 
{
	SNtlEventPrivateShopBusinessConsentNfy* pNotify = 		
		reinterpret_cast<SNtlEventPrivateShopBusinessConsentNfy*>(pData);

	CNtlSobPlayer* pSobPlayer = reinterpret_cast<CNtlSobPlayer*>(GetNtlSobManager()->GetSobObject( m_ShopData.hOwner ));

	// 흥정 창 띄우고 대기하던 MsgBox를 끈다. 포커스 회복 해줌.
	CloseBusinessMsgBox();
	CDboEventGenerator::PrivateShopBusinessStart( PRIVATESHOP_BUY, 
		m_ShopData.hOwner, 
		const_cast<WCHAR*>( Logic_GetName( pSobPlayer ) ),
		&m_aShopItemData[pNotify->byPrivateShopInventorySlotPos],
		this );
}

/**
* \brief 취소요청자가 받는 결과
* 흥정 대화상자 Off, 흥정 아이템 UnLock
* 상점 주인일 경우( 흥정 상태를 일반 상태로 바꿔줌 )
*/
void CPrivateShopGui::HandlePrivateShopBusinessCancleRes( void* pData ) 
{
	SNtlEventPrivateShopBusinessCancleRes* pResult =
		reinterpret_cast<SNtlEventPrivateShopBusinessCancleRes*>(pData);

	SetBusiness( FALSE );

	if( pResult->byCancelType == dPRIVATESHOP_BUSINESS_CANCEL_WAITING /* 흥정 대기중 취소 */)
	{
		CloseBusinessMsgBox();
		BusinessWait( FALSE );
	}
	else if( pResult->byCancelType == dPRIVATESHOP_BUSINESS_CANCEL_BUSINESS /* 흥정 때 취소 */)
	{
		CloseBusinessMsgBox();
		CloseBusinessWindow(); 
	}
	else if( pResult->byCancelType == INVALID_BYTE )
		return;

	// 아이템 UnLock
	m_aShopItemData[pResult->byPrivateShopInventorySlotPos].byItemState = sPRIVATESHOP_ITEM_DATA::PRIVATESHOP_GENERAL;
	UpdateShopItem();
}

/**
* \brief 취소를 한 경우
* 취소요청자에 따라서 Notify를 받는 대상이 달라진다.  
* (취소요청자)주인		: 흥정 손님, 손님들
* (취소요청자)흥정 손님 : 주인, 손님들
*/
void CPrivateShopGui::HandlePrivateShopBusinessCancleNfy( void* pData ) 
{
	SNtlEventPrivateShopBusinessCancleNfy* pNotify =
		reinterpret_cast<SNtlEventPrivateShopBusinessCancleNfy*>(pData);

	SetBusiness( FALSE );

	if( pNotify->byCancelType == dPRIVATESHOP_BUSINESS_CANCEL_WAITING /* 흥정이 아닐때 취소 */)
	{
		CloseBusinessMsgBox();
		BusinessWait( FALSE );
	}
	else if( pNotify->byCancelType == dPRIVATESHOP_BUSINESS_CANCEL_BUSINESS /* 흥정 때 취소 */ )	
	{	
		CloseBusinessMsgBox();
		CloseBusinessWindow();
	}
	else if( pNotify->byCancelType == INVALID_BYTE )
		return;
	

	// 아이템 UnLock
	m_aShopItemData[pNotify->byPrivateShopInventorySlotPos].byItemState = sPRIVATESHOP_ITEM_DATA::PRIVATESHOP_GENERAL;
	UpdateShopItem();
}

/**
* \brief 흥정 손님이 즉시 구매를 했을 경우의 결과
* 흥정 대화 상자를 Off하고 아이템을 구입.
*/
void CPrivateShopGui::HandlePrivateShopBusinessImmediateItemBuyingRes( void* pData ) 
{
	SNtlEventPrivateShopBusinessImmediateItemBuyingRes* pResult =
		reinterpret_cast<SNtlEventPrivateShopBusinessImmediateItemBuyingRes*>(pData);

	SetBusiness( FALSE );

	// Simulation Layer에서 모든 처리를 해줌.
	CloseBusinessWindow();

	sPRIVATESHOP_ITEM_DATA& sPrivateShopItemData = pResult->sPrivateShopItemData;
	if (sPrivateShopItemData.byPrivateShopInventorySlotPos != PRIVATESHOP_ITEMPOS_INVALID)
	{
		SetShopItemData(sPrivateShopItemData.byPrivateShopInventorySlotPos, NULL);
	}
...
/**
* \brief 흥정손님이 아이템을 즉시 구매 하였을 경우
* 상점주인과 흥정 손님을 뺀 손님들에게 아이템이 사라진 처리를 해준다.
*/
void CPrivateShopGui::HandlePrivateShopBusinessImmediateItemBuyingNfy( void* pData ) 
{
	SNtlEventPrivateShopBusinessImmediateItemBuyingNfy* pNotify =
		reinterpret_cast<SNtlEventPrivateShopBusinessImmediateItemBuyingNfy*>(pData);

	SetBusiness( FALSE );

	// 상점 주인일 경우
	if( pNotify->hPrivateShopOwner == GetNtlSLGlobal()->GetSobAvatar()->GetSerialID() )
		CloseBusinessWindow();

	sPRIVATESHOP_ITEM_DATA& sPrivateShopItemData = pNotify->sPrivateShopItemData;
	if (sPrivateShopItemData.byPrivateShopInventorySlotPos != PRIVATESHOP_ITEMPOS_INVALID)
	{
		SetShopItemData(sPrivateShopItemData.byPrivateShopInventorySlotPos, NULL);
	}
...
/**
* \brief 흥정 대기 시간의 타임이 경과하였을 경우
* 흥정 손님을 포함한 모든 손님에게 아이템의 흥정 모드를 취소해준다.
*/
void CPrivateShopGui::HandlePrivateShopBusinessWaittimeOutNfy( void* pData ) 
{
	SNtlEventPrivateShopBusinessWaittimeOutNfy* pNotify = 
		reinterpret_cast<SNtlEventPrivateShopBusinessWaittimeOutNfy*>(pData);
...
void CPrivateShopGui::BusinessWait( RwBool bWait )
{
	m_bBusinessWait = bWait;
}

/**
* \brief 흥정 대기 타임 아웃
*/
void CPrivateShopGui::BusinessWaitTimeOut() 
{
	// Todo(흥정 대기상태의 타임아웃 : 주인) 보내는것
	// 1. 상점 주인이 게임 서버에게 타임아웃이 되었다고 알려준다.
	// 2. 주인은 (흥정 수락)대화상자를 닫는다.
	// 3. 해당 아이템 UnLock(아이템 상태 변경 GENERAL)	
	
	BusinessWait( FALSE );
	GetNtlGuiManager()->RemoveUpdateFunc( this );

	// 서버에 타임 아웃 요청
	GetDboGlobal()->GetGamePacketGenerator()->SendPrivateShopBusinessWaittimeOutNfy( );
...
void CPrivateShopGui::OnShopItemClicked(CPrivateShopItemGui* pShopItem, const CKey& Key)
{
	...
	case PRIVATESHOP_BUY:
		{
			// 흥정 중인 아이템은 카트에 넣을 수 없다.
			if( m_aShopItemData[pShopItem->GetPos()].byItemState == sPRIVATESHOP_ITEM_DATA::PRIVATESHOP_BUSINESS_ITEM )
				return;
...
/**
* \brief 손님이 흥정 버튼을 클릭했을 경우
* \param pShopItem		클릭된 아이템의 GUI 슬롯
* \param pComponent		클릭된 Component의 Pointer
*/
void CPrivateShopGui::OnShopItemBusinessClicked( CPrivateShopItemGui* pShopItem, gui::CComponent* pComponent ) 
{
	// 만약에 누가 찜 중인 아이템일 경우
	if( pShopItem->GetItemState() == sPRIVATESHOP_ITEM_DATA::PRIVATESHOP_VISITOR_SELECT_ITEM )
	{
		GetAlarmManager()->AlarmMessage( GAME_PRIVATESHOP_ITEM_SELECTITEM_NOTBUSINESS, TRUE );
		return;
	}

	// 현재 상점이 흥정이 아닌 경우 흥정 요청 패킷을 보낸다.
	RwUInt32 uiOwner			= m_ShopData.hOwner;
		
	// 개인상점의 아이템 위치
	RwUInt8 uiPrivateShopInventorySlotPos	= 
		m_aShopItemData[pShopItem->GetPos()].byPrivateShopInventorySlotPos;
	RwUInt32 uiTblIdx			= pShopItem->GetItemTable()->tblidx;

	// SetShopItemSelect( pShopItem );
				
	// 개인상점 흥정 요청의 패킷을 보낸다.
	GetDboGlobal()->GetGamePacketGenerator()->SendPrivateShopBusinessRequestReq(
		uiOwner, uiPrivateShopInventorySlotPos, uiTblIdx );
}
...
/**
* \brief 흥정에 관련된 창을 모두 닫는다.
*/
void CPrivateShopGui::CloseBusinessWindow() 
{
	...
	if( GetDialogManager()->IsOpenDialog( DIALOG_PRIVATESHOP_TRADEBOX ) )
	{
	...
	case PRIVATESHOP_BUY:
		{
			...
			for (int i = 0; i < SLOT_NUM; ++i)
			{
				...
				if (iPos < NTL_MAX_PRIVATESHOP_INVENTORY && m_aShopItemData[iPos].sItem.itemNo != 0)
				{
					...
					m_aShopItem[i].ShowBusiness( TRUE );
					++iPos;
					continue;
				}
	...
	case PRIVATESHOP_SELL:
		{
			...
			for (int i = 0; i < SLOT_NUM; ++i)
			{
				...
				// 판매자는 흥정 버튼이 보이지 않는다.
				m_aShopItem[i].ShowBusiness( FALSE );
...
void CPrivateShopGui::SetAway(bool bAway)
{
	...
	for (int i = 0; i < SLOT_NUM; ++i)
	{
		m_aShopItem[i].ClickEnableBusiness( !bAway );
	}	
}

void CPrivateShopGui::SetBusiness( bool bBusiness ) 
{
	m_bOwnerBusiness = bBusiness;

	for( int i=0; i < SLOT_NUM; ++i )
	{
		m_aShopItem[i].ClickEnableBusiness( !bBusiness );
	}
}
...
/**
* \brief 현재 떠 있는 흥정 관련 메시지 박스를 닫아주고 포커스를 회복해준다.
*/
void CPrivateShopGui::CloseBusinessMsgBox() 
{
	GetDboGlobal()->GetGameData()->bChatMode = FALSE;

	// 흥정 관련 메시지 박스일 경우
	GetMsgBoxManager()->DeleteMsgBox( MBW_PRIVATESHOP_BUSINESS_WAIT );
	GetMsgBoxManager()->DeleteMsgBox( MBW_PRIVATESHOP_BUSINESS_CONSENT );
	GetMsgBoxManager()->DeleteMsgBox( MBW_PRIVATESHOP_BUSINESS_IMMEDIATE );	
}
