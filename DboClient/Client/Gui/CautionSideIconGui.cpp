...
* \brief Construction
*/
CCautionSideViewUnit::CCautionSideViewUnit( gui::CComponent* pParentGui, RwUInt32 uiStringID, const WCHAR* pwcString, RwReal fLimitTime) 
{
	m_bLife = TRUE;
...
void CCautionSideViewGui::HandleEvents( RWS::CMsg& msg ) 
{
	if( msg.Id == g_EventCautionSideNotify )
	{
		...
		if( pNotify->bActive )
		{
			...
			if( CreateUnit( pNotify->uiStringID, pNotify->pString, pNotify->fLifeTime ) )
			{
				SendUnitSizeToIcon();
				OnSideViewLocate( m_rectIcon );
			}
...
*/
RwBool CCautionSideViewGui::CreateUnit( RwUInt32 uiStringID, const WCHAR* pString, RwReal fLifeTime ) 
{
	// 10개 이상일 경우 하나를 지워준다.
...
* \return 성공여부
*/
RwBool CCautionSideViewGui::DestroyUnit( RwUInt32 uiStringID ) 
{
