...
RWS::CEventId g_EventConnectBoxShow;		// network connect box show on/off
...
RWS::CEventId g_EventCharm;					// 파티 부적 이벤트

RWS::CEventId g_EventCharmVictim;			// 파티 부적 제물 이벤트

RWS::CEventId g_EventPartyInventory;		// 파티 인벤토리 이벤트
...
RWS::CEventId g_EventScouterUpgradePredict;	// 스카우터 업그레이드 예측

RWS::CEventId g_EventESCinNarrationMode;	// 나레이션 모드에서의 ESC키 입력.
...
RWS::CEventId g_EventHoipoiMixItemCheckRes;		///< 만들 아이템 선택 ( 필요한 아이템 수량 체크
RWS::CEventId g_EventHoipoiMixItemMakeRes;		///< 아이템 만들기
RWS::CEventId g_EventHoipoiMixItemMakeEpRes;		///< 아이템 만들기 중 EP 넣기
...
//--------------------------------------------------------
// Game mania time
RWS::CEventId g_EventGmtInfo;
RWS::CEventId g_EventGmtUpdate;
RWS::CEventId g_EventGmtChanged;
RWS::CEventId g_EventGmtResetOpen;
