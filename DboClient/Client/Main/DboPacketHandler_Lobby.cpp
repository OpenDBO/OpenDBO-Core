...
void PacketHandler_CSCharSelectRes(void *pPacket)
{
	...
	if(pCharSelectRes->wResultCode == CHARACTER_SUCCESS)
	{
		...
		if(!pNet->Connect((char*)pConnectData->sGameCon.chServerIP, pConnectData->sGameCon.wServerPort, &pConnectData->sGameCon.hSocket, false, NETCLIENT_ENCODE_RANDKEY) )
...
void PacketHandler_GSGameEnterRes(void *pPacket)
{
	...
	if(pConnectData->sChatCon.byConnState == SConnectAtomic::SOCKET_STATE_DISCONNECT)
	{
		...
		CNtlClientNet *pNet = GetDboGlobal()->GetNetwork(); 
		if(!pNet->Connect((char*)pConnectData->sChatCon.chServerIP, pConnectData->sChatCon.wServerPort, &pConnectData->sChatCon.hSocket, false, NETCLIENT_ENCODE_RANDKEY ) )
			bConnectSuccess = FALSE;
