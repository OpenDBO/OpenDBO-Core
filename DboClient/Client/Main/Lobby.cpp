...
VOID CLobby::SetServerInfo(sDBO_GAME_SERVER_FARM_INFO* pServerInfo)
{
	...
	if( pServerInfo->wszGameServerFarmName )
		memcpy(awcServerName, pServerInfo->wszGameServerFarmName,
		sizeof(pServerInfo->wszGameServerFarmName));
}
...
VOID CLobby::SetChannelInfo(RwUInt8 byCount, sDBO_GAME_SERVER_CHANNEL_INFO* pChannelInfo)
{
	...
	memcpy(&atChannel, pChannelInfo, sizeof(atChannel[0]) * byChannelCount);	
}
...
VOID CLobby::SetCharacterInfo(RwUInt8 byCount, sPC_SUMMARY* pPC_SUMMARY)
{
	...
	{
		memcpy(&atLobbyCharacter[i].tSummary, &pPC_SUMMARY[i], sizeof(atLobbyCharacter[i].tSummary));
	}
}
...
VOID CLobby::SetDeletingCharcterInfo(sDELETE_WAIT_CHARACTER_INFO* pDELETE_WAIT_CHARACTER_INFO)
{
	...
	for( RwUInt8 i = 0 ; i < NTL_MAX_COUNT_USER_CHAR_SLOT ; ++i )
	{
		...
		{
			if( atLobbyCharacter[k].tSummary.charId == pDELETE_WAIT_CHARACTER_INFO[i].charId )
			{
				RwReal fTime_util_Deletion = (RwReal)(NTL_DELETE_CHAR_MAX_WAIT_TICK - pDELETE_WAIT_CHARACTER_INFO[i].dwPastTick) / 1000.f;
...
RwBool CLobby::AddCharacter(sPC_SUMMARY* pSummary)
{
	...
	memcpy(&atLobbyCharacter[byCharacterCount].tSummary, pSummary, sizeof(*pSummary));
...
RwUInt8 CLobby::RemoveCharacter(CHARACTERID charID)
{
	...
	for( RwUInt8 i = 0 ; i < NTL_MAX_COUNT_USER_CHAR_SLOT ; ++i )
	{
		...
		{
			memcpy(&atLobbyCharacter[i-1].tSummary, &atLobbyCharacter[i].tSummary, sizeof(atLobbyCharacter[i].tSummary));
...
VOID CLobby::Rename(CHARACTERID charID, const WCHAR* pwcName)
{
	...
	for( RwUInt8 i = 0 ; i < byCharacterCount ; ++i )
	{
		if( atLobbyCharacter[i].tSummary.charId == charID )
		{
			...
			if( NTL_MAX_SIZE_CHAR_NAME_UNICODE < uiNameLength )
				uiNameLength = NTL_MAX_SIZE_CHAR_NAME_UNICODE;
