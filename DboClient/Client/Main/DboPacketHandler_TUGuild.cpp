...
void PacketHandler_TUGuild_Kick_Out_Res(void *pPacket)
{
	...
	CNtlSLEventGenerator::SLGuildEvent(SLGE_DEL_MEMBER, pResult->kickedOutMemberCharId);
}
...
void PacketHandler_TUGuild_FunctionChange_Nfy(void* pPacket)
{
	...
	{
		// 도장 쟁탈전의 패배 등의 이유로 기존의 기능을 잃어버렸다
	}
	else if( IsGuildFunction((eDBO_GUILD_FUNCTION)pResult->byNewFunc) )
	{
		GetAlarmManager()->AlarmMessage(DST_GUILD_ADD_FUNCTION_NOTIFY);
	}
	else if( IsDojoFunction((eDBO_GUILD_FUNCTION)pResult->byNewFunc) )
	{
		GetAlarmManager()->AlarmMessage(DST_DOJO_ADD_FUNCTION_NOTIFY);
	}
	else
	{
...
void PacketHandler_TUGuild_GiveZenny_Nfy(void* pPacket)
{
	...
	// %s님이 %d 제니를 기부하였습니다
	GetAlarmManager()->FormattedAlarmMessage(DST_GUILD_CONTRIBUTION_ZENNY_OTHERS, FALSE, NULL, pMember->wszMemberName, pResult->dwZenny);
}
...
void PacketHandler_GSGuild_WarehouseZenny_Res(void* pPacket)
{
	...
	{
		// %d 제니를 저금하였습니다
		GetAlarmManager()->FormattedAlarmMessage(DST_GUILD_WAREHOUSE_SAVE_ZENNY, FALSE, NULL, pResult->dwZenny);
	...
	{
		// %d 제니를 출금하였습니다
		GetAlarmManager()->FormattedAlarmMessage(DST_GUILD_WAREHOUSE_ZENNY_DRAW_OUT, FALSE, NULL, pResult->dwZenny);
...
{
	API_GetSLPacketLockManager()->Unlock(GU_DOJO_BANK_HISTORY_RES);

	// 도장 창고 사용 내역을 받았다
	sGU_DOJO_BANK_HISTORY_RES* pResult = (sGU_DOJO_BANK_HISTORY_RES*)pPacket;
	...
	CNtlSLEventGenerator::DojoEvent(DOJO_EVENT_BANK_HISTORY, pResult->dwBalance, (void*)&pResult->byCount, (void*)&pResult->asData);
}
...
void PacketHandler_TUDojo_NoticeChangeNfy(void* pPacket)
{
	...
	if( NTL_MAX_LENGTH_OF_GUILD_NOTICE_UNICODE < pResult->wNoticeLengthInUnicode )
		pResult->wNoticeLengthInUnicode = NTL_MAX_LENGTH_OF_GUILD_NOTICE_UNICODE;

	WCHAR acNotice[NTL_MAX_LENGTH_OF_GUILD_NOTICE_UNICODE + 1];
