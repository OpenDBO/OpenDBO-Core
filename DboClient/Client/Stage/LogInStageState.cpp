...
void CLogInStageState::UpdateLoginReq(RwReal fElapsed)
{
	...
	if(!m_bNotifyShow)
	{
		if(m_fNotifyTime >= 3.0f)
		{
			...
			//WCHAR* pwcText = (WCHAR*)GetDisplayStringManager()->GetString(DST_SERVER_CONNECTING);
			//GetAlarmManager()->AlarmMessage(pwcText, CAlarmManager::ALARM_TYPE_LOBBY_MSGBOX, MBW_POST_QUIT_MESSAGAE,
			//								MBTF_OK, TRUE, fRemainTime);
