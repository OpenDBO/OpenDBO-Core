...
void CGameStageState::Update(RwReal fElapsed)
{
	...
	if ( m_fNPCheckTime < 0.f )
	{
		m_fNPCheckTime = NP_CHECK_TIME;

		GetServiceInterface()->CheckGameMon();
	}

	if ( m_bNPCheck && !GetServiceInterface()->IsEmptySecurityRetData() )
	{
		unsigned int uiRetFlag, uiRetCode;
		sSEC_RET_DATA* pRetData;

		while ( GetServiceInterface()->PopSecurityRetData( uiRetFlag, uiRetCode, &pRetData ) )
		{
			Logic_ProcessSecurityResultCode( uiRetCode, pRetData );

			if ( pRetData )
			{
				if ( pRetData->pData )
				{
					delete [] pRetData->pData;
					pRetData->pData = NULL;
				}
...
void CGameStageState::UpdateServerConnect(RwReal fElapsed)
{
    ...
    else if(m_fNotifyTime * 1000.0f >= GSS_TIME_SERVER_CONNECT)
    {
        ...
        // 기다리는 시간(30초)가 끝나면 기존의 메시지 박스를 지우고, 게임 종료 메시지 박스를 뛰운다.
        GetMsgBoxManager()->DeleteNetConnectBox();
