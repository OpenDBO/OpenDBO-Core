...
RwBool CDBOCheatCommand::OnCreatePC(CNtlTokenizer* pLexer)
{
	...
	wcscpy_s( sPcBrief.awchName, NTL_MAX_SIZE_CHAR_NAME_UNICODE, L"이름1" );
	wcscpy_s( sPcBrief.wszGuildName, NTL_MAX_SIZE_CHAR_NAME_UNICODE, L"길드1" );
...
RwBool CDBOCheatCommand::LoadPalette( CNtlTokenizer* pLexer ) 
{
	if( !Logic_IsDevUser() )
	{
		GetAlarmManager()->AlarmMessage(L"You are not developer");
		return FALSE;
	}
...
RwBool CDBOCheatCommand::OnDestMove(CNtlTokenizer* pLexer)
{
	...
	FILE* pFile = fopen( "destmove.txt", "rt" );
	...
	if ( pFile )
	{
		...
		for ( int i = 0; i < nCnt; ++i )
		{
			...
			fscanf( pFile, "%f ", &pPathList[i].x );
			fscanf( pFile, "%f ", &pPathList[i].z );
