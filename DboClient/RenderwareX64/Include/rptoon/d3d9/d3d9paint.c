...
extern RwBool RpSkinD3DAtomicOnlyVertexUpdate(RpAtomic  *atomic);
...
static D3DMATRIX    D3D9IdentyMatrix =
                 ...
                 const RpToonPaint *paint )
{
    ...
    */
    ...
    RwD3D9SetTransform( D3DTS_TEXTURE0 + 0, &D3D9CameraSpaceLightmatrix );
...
ToonShadeClose(void)
{
    ...
    RwD3D9SetTransform(D3DTS_TEXTURE3, &D3D9IdentyMatrix);
    RwD3D9SetTransform(D3DTS_TEXTURE4, &D3D9IdentyMatrix);
    RwD3D9SetTransform(D3DTS_TEXTURE5, &D3D9IdentyMatrix);
    RwD3D9SetTransform(D3DTS_TEXTURE6, &D3D9IdentyMatrix);

    RwD3D9SetRenderState(D3DRS_TEXTUREFACTOR, 0xFFFFFFFF);
    RwD3D9SetRenderState(D3DRS_ALPHATESTENABLE, FALSE);
    RwD3D9SetRenderState(D3DRS_ALPHAREF, 0);
    RwD3D9SetRenderState(D3DRS_ALPHAFUNC, D3DCMP_ALWAYS);
                 ...
                 const RpToonPaint *paint )
{
    ...
    */
        ...
        mesh->material->texture != NULL)
    {
        ...
        {
            psConstants[1].x = COLORSCALAR * (RwReal)(color->red);
            psConstants[1].y = COLORSCALAR * (RwReal)(color->green);
            psConstants[1].z = COLORSCALAR * (RwReal)(color->blue);
            psConstants[1].w = COLORSCALAR * (RwReal)(color->alpha);
                                    ...
                                    RwMatrix *ltmMatrix)
{
	...
	RwRenderStateGet(rwRENDERSTATEVERTEXALPHAENABLE, (void *)&bAlphaBlend);
	...
	//. 2006. 5. 23. nonstopdj
	//. alpha blend state retore.
	RwRenderStateSet(rwRENDERSTATEVERTEXALPHAENABLE, (void *)bAlphaBlend);
                             ...
                             RwUInt32 flags)
{
    ...
    if (type == rpATOMIC)
    {
        ...
        if(toonGeo == NULL)
        {
            _rxD3D9DefaultRenderCallback(repEntry, object, type, flags);
            RWRETURNVOID();
        }
...
RpAtomic *
RpToonAtomicRender( RpAtomic *atomic )
{
    RpToonGeo *toonGeo;
	...
	RpSkinD3DAtomicOnlyVertexUpdate(atomic);
...
ToonAtomicRender( RpAtomic *atomic )
{
	...
	//. 2006. 5. 23. nonstopdj
	//. we need not defatomicRenderCallback.
	RpSkinD3DAtomicOnlyVertexUpdate(atomic);
    ...
    if (FastSilhouetteVertexShader != NULL)
    {
        ...
        * atomic is used else the resEntry in the geometry */
		...
		if(!repEntry || IsBadReadPtr(repEntry, sizeof(RwResEntry)))
			RWRETURN(atomic);
