...
#define rwAlign(_addr, _alignment) \
    (( (uintptr_t)(_addr) + ((_alignment) - 1) ) & ~((_alignment) - 1))
...
#if defined WIN32
// Critical Section for _freeListList, _masterFreeList
static CRITICAL_SECTION _csFreeList;
#else // defined WIN32
static void* _csFreeList;
#endif // defined WIN32
...
{
    RWFUNCTION(RWSTRING("_rwFreeListModuleOpen"));
    RWASSERT(FALSE == _freeListModuleOpen);

	//@{ 20050513 DDonSS : Threadsafe
	// _freeListList, _masterFreeList Initialize Lock
	CS_MASTERFREELIST_INITLOCK();
	//@} DDonSS
...
_rwFreeListModuleClose ( void )
{
...
#endif /* RWKEEPFREELISTS */
    RWFUNCTION(RWSTRING("_rwFreeListModuleClose"));
    RWASSERT(_freeListModuleOpen);

#ifdef RWKEEPFREELISTS
	//@{ 20050513 DDonSS : Threadsafe
	// _freeListList, _masterFreeList Lock
	CS_MASTERFREELIST_LOCK();
	//@} DDonSS
	...
	//@{ 20050513 DDonSS : Threadsafe
	// _freeListList, _masterFreeList Unlock
	CS_MASTERFREELIST_UNLOCK();
	//@} DDonSS

#endif /* RWKEEPFREELISTS */

	//@{ 20050513 DDonSS : Threadsafe
	// _freeListList, _masterFreeList Delete Lock
	CS_MASTERFREELIST_DELLOCK();
	//@} DDonSS
    ...
    RwUInt32    hint)
{
	...
	//@{ 20050513 DDonSS : Threadsafe
	// FreeList Lock Initialize
	CS_FREELIST_INITLOCK( freeList );
	//@} DDonSS
...
{
    RwLLLink *link, *lastLink;
    RWFUNCTION(RWSTRING("_RwFreeListFree"));
    RWASSERT(freeList);

	//@{ 20050513 DDonSS : Threadsafe
	// FreeList Lock
	CS_FREELIST_LOCK( freeList );
	//@} DDonSS
	...
	//@{ 20050513 DDonSS : Threadsafe
	// FreeList Unlock
	CS_FREELIST_UNLOCK( freeList );

	// FreeList Delete Lock
	CS_FREELIST_DELLOCK( freeList );
	//@} DDonSS
...
_rwFreeListFreeReal(RwFreeList * freeList, void *entry)
{
    ...
    RWMESSAGE((RWSTRING("Entry 0x%08X was not allocated from this FreeList"), (uintptr_t)entry));
    RWASSERT(FALSE);
