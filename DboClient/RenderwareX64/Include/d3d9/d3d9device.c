...
//@{ Jaewon 20040917
D3DTEXTUREOP TextureOpModulation = D3DTOP_MODULATE;
//@} Jaewon
...
*/
    ...
    hr = DXCHECK(IDirect3D9_CreateDevice
                    (_RwDirect3DObject, _RwD3DAdapterIndex,
	...
	// memcheck -gemani
	//hr = DXCHECK(IDirect3D9_CreateDevice
    //                (_RwDirect3DObject, IDirect3D9_GetAdapterCount(_RwDirect3DObject) - 1,
    //                D3DDEVTYPE_REF, WindowHandle,
    //                behaviorFlags, &Present,
    //                &_RwD3DDevice));
...
//@{ Jaewon 20040917
// change the default texture color operation to a specified modulate operation.
// mainly for modulate2x
// 'factor' should be 1, 2 or 4.
void
RwD3D9EngineSetTextureOpModulationFactor(RwInt32 factor)
{
    RWAPIFUNCTION(RWSTRING("RwD3D9EngineSetTextureOpModulationFactor"));

	if(factor == 1)
		TextureOpModulation = D3DTOP_MODULATE;
	else if(factor == 4)
		TextureOpModulation = D3DTOP_MODULATE4X;
	else
		TextureOpModulation = D3DTOP_MODULATE;
                         ...
                         void *cameraIn, RwInt32 in __RWUNUSED__)
{
	...
	RwD3D9SetTransform(D3DTS_VIEW, &_RwD3D9D3D9ViewTransform);			// 2005.3.25 gemani
                       ...
                       void *cameraIn, RwInt32 nIn __RWUNUSED__)
{
	...
	/* 2006. 5. 2. Nonstopdj
	 * if global camera is null pointer then force return true.
	 */
	...
	/*
	 * 2006. 3. 13. Nonstopdj
	 * top level raster validation check.
	 */
	if(raster != NULL)
		topRaster = RwRasterGetParent(raster);
	else
...
RwD3D9GetBackBufferFomat()
{
	RWAPIFUNCTION(RWSTRING("RwD3D9GetBackBufferFomat"));

	return rwD3D9FindRWFormat(Present.BackBufferFormat);
}
