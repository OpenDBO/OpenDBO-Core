...
// 2005.3.29 gemani		�޸� block�� �� á�� ��� �޸� ���� �Ҵ� �÷��� �߰� & ó�� �ڵ� ����
...
struct rwResHeapBlockHeader
{
	...
	RwBool				isdynamicalloc;		// ���� �Ҵ� �Ͽ�����.. 2005.3.29 gemani
    RwUInt32            pad[2]; /* Align this structure to 32 bytes */
};
...
MemoryLogFree(void *memory)
{
    ...
    if (nGLogFileID != NULL)
    {
        ...
        nGAddressBuffer[nGBufferIndex] = (uintptr_t)(memory);
...
MemoryLogAlloc(rwResHeapBlockHeader * targetBlock, RwUInt32 size)
{
    ...
    {
        /* Buffer up the stats since debug_write is Sloooooooooooowwwwwwwwww */
        nGAddressBuffer[nGBufferIndex] = (uintptr_t)(targetBlock + 1);
...
_rwResHeapInit(void *resHeap, RwUInt32 size)
{
	...
	// 2005.3.29 gemani 
	firstBlock->isdynamicalloc = FALSE;
...
_rwResHeapAlloc(void *resHeap, RwUInt32 size)
{
    ...
    if (!targetBlock)
    {
		...
		targetBlock = (rwResHeapBlockHeader *)RwMalloc( size + 32,
                           rwMEMHINTDUR_EVENT | rwID_RESOURCESMODULE);		

		targetBlock->isdynamicalloc = TRUE;
	...
	// 2005.3.29 gemani
	targetBlock->isdynamicalloc = FALSE;
