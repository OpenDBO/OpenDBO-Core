...
#define RwCosec(_x) \
   ( ((RwReal)1) / RwSin((_x)) )
#define ROUNDUP16(x)      (((uintptr_t)(x) + 16 - 1) & ~(16 - 1))
          ...
          RwInt32 __RWUNUSED__ sizeInObject)
{
    ...
    if (numNodes > 0)
    {
        ...
        /* Ensure the interpolator keyframe size is at least as large as the
           current platform's interpolated keyframe size. DFF files authored on
           x86 store maxInterpKeyFrameSize=36, but on x64
           sizeof(RpHAnimInterpFrame)=48 due to pointer fields growing from 4
           to 8 bytes. Without this fix, the interpolator buffer is allocated
           too small and the animation code overflows the heap. */
        if (maxInterpKeyFrameSize < (RwInt32)sizeof(RpHAnimInterpFrame))
        {
            maxInterpKeyFrameSize = (RwInt32)sizeof(RpHAnimInterpFrame);
        }
