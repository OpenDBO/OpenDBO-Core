                         ...
                         RwInt32 numVerts)
{
	...
	/* 2006. 2. 13. Nonstopdj */
	/* Check Vertex Buffer Lock() is Sucess */
    if (UseOwnVertexBuffer)
    {
        if ((CurrentBaseIndex + numVerts) > BUFFER_MAX_VERTEX_INDEX)
        {
            ...
            hRet = IDirect3DVertexBuffer9_Lock(CurrentVertexBuffer,
                                        0,
    ...
    {
        bResult = RwD3D9DynamicVertexBufferLock(sizeof(RwIm2DVertex),
                                      numVerts,
                                      (void**)&CurrentVertexBuffer,
                                      (void**)&bufferMem,
                                      &CurrentBaseIndex);
	...
	if( !bResult || hRet != S_OK )
	{
		/* Current Vertex Buffer Unlock() */
		IDirect3DVertexBuffer9_Unlock(CurrentVertexBuffer);

		/* Vertex Buffer Lock was FAILED */
		RWMESSAGE((RWSTRING("Error:D3D Vertex Buffer Lock() was FAILED. Current Base Index is %d"), CurrentBaseIndex));
		RWRETURN(FALSE);
	}
