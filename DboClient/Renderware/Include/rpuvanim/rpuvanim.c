                ...
                /*  Iterate over each node for that RtAnimIterator */
                {
                    ...
                    for (iNode = 0; iNode < interp->numNodes; ++iNode)
                    {
                        ...
                        if (channel < 2)
                        {
                            ...
                            {
                                RpUVAnimParamKeyFrameData *pd =
                                    &((RpUVAnimInterpFrame *)nodeInterpFrame)->data.param;
 
                                /* Manually build the parametric UV matrix.
                                   Bypasses RwMatrixTranslate / RwMatrixRotate,
                                   which were observed corrupting the matrix on
                                   this x64 build (confirmed via debug log: at
                                   theta=0 -- where rotation should be a no-op
                                   and Pivot+InvPivot cancel to zero -- the
                                   output still came out completely wrong).
 
                                   Replicates, with plain scalar math:
                                     M = M0 * T(pivot) * R(theta) * T(invPivot)
                                   where M0 is the scale/skew/translate base
                                   matrix (s0,s1,skew,x,y), and pivot/invPivot
                                   are the fixed UV-space pivot constants used
                                   to rotate about the texture center. */
                                {
                                    RwReal c = (RwReal)cos(pd->theta);
                                    RwReal s = (RwReal)sin(pd->theta);
 
                                    /* Base linear part + translation, from M0 */
                                    RwReal m00 = pd->s0,  m01 = pd->skew;
                                    RwReal m10 = 0.0f,    m11 = pd->s1;
                                    RwReal tx  = pd->x,   ty  = pd->y;
 
                                    /* Step 1: translate by Pivot */
                                    RwReal wtx = tx + rpUVAnimPivot.x;
                                    RwReal wty = ty + rpUVAnimPivot.y;
 
                                    /* Step 2: rotate linear part and the
                                       translated pivot point by theta.
                                       (If the visual rotation direction ends
                                       up backwards, flip the sign of s here.) */
                                    RwReal l00 = m00 * c + m01 * (-s);
                                    RwReal l01 = m00 * s + m01 * c;
                                    RwReal l10 = m10 * c + m11 * (-s);
                                    RwReal l11 = m10 * s + m11 * c;
 
                                    RwReal rtx = wtx * c + wty * (-s);
                                    RwReal rty = wtx * s + wty * c;
 
                                    /* Step 3: translate by InvPivot */
                                    RwReal finalTx = rtx + rpUVAnimInvPivot.x;
                                    RwReal finalTy = rty + rpUVAnimInvPivot.y;
 
                                    interpMatrix.right.x = l00;
                                    interpMatrix.right.y = l01;
                                    interpMatrix.right.z = 0.0f;
                                    interpMatrix.up.x    = l10;
                                    interpMatrix.up.y    = l11;
                                    interpMatrix.up.z    = 0.0f;
                                    interpMatrix.at.x    = 0.0f;
                                    interpMatrix.at.y    = 0.0f;
                                    interpMatrix.at.z    = 1.0f;
                                    interpMatrix.pos.x   = finalTx;
                                    interpMatrix.pos.y   = finalTy;
                                    interpMatrix.pos.z   = 0.0f;
                                }
 
                                /* TEMP DEBUG: keep until visually confirmed */
                                /*
                                {
                                    FILE *dfp = fopen(
                                        "C:\\dboDevelop\\OpenDBO-Core\\DboClient\\DragonBall\\dbo_param.txt",
                                        "a");
                                    if (dfp)
                                    {
                                        fprintf(dfp,
                                            "PARAM(manual) mat=%p slot=%d node=%d theta=%.4f s0=%.4f s1=%.4f skew=%.4f x=%.4f y=%.4f => right=(%.4f,%.4f) up=(%.4f,%.4f) pos=(%.4f,%.4f)\n",
                                            (void*)material, iSlot, iNode,
                                            pd->theta, pd->s0, pd->s1, pd->skew, pd->x, pd->y,
                                            interpMatrix.right.x, interpMatrix.right.y,
                                            interpMatrix.up.x, interpMatrix.up.y,
                                            interpMatrix.pos.x, interpMatrix.pos.y);
                                        fclose(dfp);
                                    }
                                }
                                */
