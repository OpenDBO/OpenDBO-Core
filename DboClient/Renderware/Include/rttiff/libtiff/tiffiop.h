/* $Header: //RenderWare/RW37Active/rwsdk/tool/tiff/libtiff/tiffiop.h#1 $ */
...
/* Avoid _ftol() calls for MW/CW x86 runtime compatibility */

static __inline int
IntFromFloat(float x)
{
    short savemode;
    short workmode;
    int res;
...
#else
    // x64: use intrinsics or standard conversion
    res = (int)x;
#endif

    return res;
}
#define IntFromFloatMacro(x) IntFromFloat(x)
#endif /* (defined(_MSC_VER)) */

#if (!defined(IntFromFloatMacro))
#define IntFromFloatMacro(x)                \
    ((int)(x))
#endif /* (!defined(IntFromFloatMacro)) */

#if ((!defined(__R5900__)) && (defined(unix) || defined(__unix)))
#include "port.h"
#include "tiffconf.h"
#else
#include "tiffconf.h"
...
/* We need to stop a bunch of typedefs coming out of eekernel.h */
#ifdef __R5900__
#include <sys/types.h>
/* PC centric source assumes that this is 32 bit! However, some of the
   source uses unsigned long directly, so we have to here. It doesn't
   seem to cause any problems, though I'd be inclined to clean up all
   the missmatches if problems emerge */
typedef unsigned long u_long;
typedef unsigned int u_int;
typedef unsigned short u_short;
typedef unsigned char u_char;
